...
namespace vz::font
{
	namespace font_internal
	{
		...
		static std::unordered_map<int32_t, vz::rectpacker::Rect> rect_lookup;
		struct Bitmap
		{
			int width;
			int height;
	...
	Cursor Draw_internal(const T* text, size_t text_length, const Params& params, CommandList cmd)
	{
		...
		if (status.quadCount > 0)
		{
			...
			else
			{
				...
				assert(canvas.GetPhysicalWidth() > 0);
				assert(canvas.GetPhysicalHeight() > 0);
				assert(canvas.GetDPI() > 0);
