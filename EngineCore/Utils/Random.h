...
namespace vz::random
{
	...
	inline static thread_local RNG rng(time(nullptr));

	inline int GetRandom(int minValue, int maxValue)
	{
		return rng.next_int(minValue, maxValue);
	}
	inline int GetRandom(int maxValue)
	{
		return GetRandom(0, maxValue);
	}

	inline uint32_t GetRandom(uint32_t minValue, uint32_t maxValue)
	{
		return rng.next_uint(minValue, maxValue);
	}
	inline uint32_t GetRandom(uint32_t maxValue)
	{
		return GetRandom(0u, maxValue);
	}

	inline uint64_t GetRandom(uint64_t minValue, uint64_t maxValue)
	{
		return rng.next_uint(minValue, maxValue);
	}
	inline uint64_t GetRandom(uint64_t maxValue)
	{
	...
	inline float GetRandom(float maxValue)
	{
