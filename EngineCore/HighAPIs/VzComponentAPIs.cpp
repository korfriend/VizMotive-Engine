...
#define GET_TRANS_COMP(COMP, RET) TransformComponent* COMP = compfactory::GetTransformComponent(componentVID_); \
	...
	void VzSceneComp::GetWorldRotation(float v[4])
	{
		GET_TRANS_COMP(transform, );
		*(XMFLOAT4*)v = transform->GetWorldRotation();
	}
	void VzSceneComp::GetWorldScale(float v[3])
	{
		GET_TRANS_COMP(transform, );
	...
	void VzSceneComp::GetWorldMatrix(float mat[16], const bool rowMajor)
	{
		GET_TRANS_COMP(transform, );
	...
	void VzSceneComp::GetRotation(float v[4])
	{
		GET_TRANS_COMP(transform, );
	...
	void VzSceneComp::GetLocalMatrix(float mat[16], const bool rowMajor)
	{
		GET_TRANS_COMP(transform, );
	...
	void VzSceneComp::SetEulerAngleZXY(const float v[3])
	{
		GET_TRANS_COMP(transform, );
		transform->SetEulerAngleZXY(*(XMFLOAT3*)v);
		UpdateTimeStamp();
	}
	void VzSceneComp::SetEulerAngleZXYInDegree(const float v[3])
	{
		GET_TRANS_COMP(transform, );
	...
	void VzSceneComp::SetMatrix(const float value[16], const bool rowMajor)
	{
		GET_TRANS_COMP(transform, );
		XMFLOAT4X4 mat_in = *(XMFLOAT4X4*)value;
		if (!rowMajor)
		{
