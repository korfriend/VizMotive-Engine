#ifndef SHADERINTEROP_DDGI_H
#define SHADERINTEROP_DDGI_H
...
inline float2 ddgi_probe_color_uv(uint3 probeCoord, float3 direction)
{
	...
	pixel += (encode_oct((half3)normalize(direction)) * 0.5 + 0.5) * DDGI_COLOR_RESOLUTION;
...
inline float2 ddgi_probe_depth_uv(uint3 probeCoord, float3 direction)
{
	...
	pixel += (encode_oct((half3)normalize(direction)) * 0.5 + 0.5) * DDGI_DEPTH_RESOLUTION;
...
half3 ddgi_sample_irradiance(float3 P, half3 N)
{
	...
	half3 alpha = (half3)saturate((P - base_probe_pos) * ddgi_cellsize_rcp());
	...
	for (uint i = 0; i < 8; ++i)
	{
		...
		half3 probe_to_point = (half3)(P - probe_pos + N * 0.001);
		...
		half3 trilinear = lerp((half3)1.0 - alpha, alpha, (half3)offset);
		half weight = 1.0;
		...
		// Smooth backface test
		{
			...
			half3 true_direction_to_probe = (half3)normalize(probe_pos - P);
			...
			weight *= (half)lerp(saturate(dot(dir, N)), sqr(max(0.0001, (dot(true_direction_to_probe, N) + 1.0) * 0.5)) + 0.2, GetScene().ddgi.smooth_backface);
		}
		...
		[branch]
			if (GetScene().ddgi.depth_texture >= 0)
			{
				...
				half2 temp = (half2)bindless_textures[GetScene().ddgi.depth_texture].SampleLevel(sampler_linear_clamp, tex_coord, 0).xy;
				half mean = temp.x;
		...
		half3 probe_irradiance = (half3)bindless_textures[GetScene().ddgi.color_texture].SampleLevel(sampler_linear_clamp, tex_coord, 0).rgb;
