...
float4 main(VertextoPixel input) : SV_TARGET
{
	...
	{
		half4 tex = (half4)bindless_textures[image.texture_base_index].Sample(sam, uvsets.xy);
	...
	{
		mask = (half4)bindless_textures[image.texture_mask_index].Sample(sam, uvsets.zw);
	}
	...
	if (image.texture_background_index >= 0)
	{
		...
		half3 background = (half3)backgroundTexture.Sample(sam, uv_screen).rgb;
	...
	else if (image.flags & IMAGE_FLAG_OUTPUT_COLOR_SPACE_LINEAR)
	{
		...
		color.rgb *= (half)image.hdr_scaling;
	}
	...
	if (image.border_soften > 0)
	{
		...
		color.a *= (half)smoothstep(0, image.border_soften, 1 - edge);
	}
