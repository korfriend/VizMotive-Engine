...
namespace vz::texturehelper
{
	...
	vz::graphics::Texture helperTextures[HELPERTEXTURE_COUNT];
	std::unordered_map<unsigned long, vz::graphics::Texture> colorTextures;
	vz::SpinLock colorlock;
	...
	void Initialize()
	{
		...
		// Logo
		{
			//CreateTexture(helperTextures[HELPERTEXTURE_LOGO], our_engine_logo, 256, 256);
		...
		// Random64x64
		{
			...
			for (int i = 0; i < arraysize(data); i += 4)
			{
				...
				data[i + 1] = vz::random::GetRandom(0, 255);
				data[i + 2] = vz::random::GetRandom(0, 255);
		...
		// BlackCubemap
		{
			...
			vz::Color d[6][width * height] = {}; // 6 images initialized to 0 (transparent black)
		...
		// Blue Noise:
		{
			std::vector<vz::Color> bluenoise(128 * 128); // heap alloc intended (PS5)
			...
			for (int y = 0; y < 128; ++y)
			{
				for (int x = 0; x < 128; ++x)
				{
					...
					bluenoise[x + y * 128] = vz::Color::fromFloat4(XMFLOAT4(f0, f1, f2, f3));
				}
			}
		...
		// Single colors:
		{
			...
			color = vz::Color(127, 127, 255, 255);
		...
		vz::backlog::post("vz::texturehelper Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
	...
	)
	{
		...
		case GradientType::Linear:
		{
			...
			for (uint32_t y = 0; y < height; ++y)
			{
				for (uint32_t x = 0; x < width; ++x)
				{
					...
					const XMVECTOR point_on_line = vz::math::ClosestPointOnLineSegment(a, b, XMLoadFloat2(&uv));
					const float uv_distance = XMVectorGetX(XMVector3Length(point_on_line - a));
					float gradient = saturate(vz::math::InverseLerp(0, distance, uv_distance));
					...
					{
						gradient = vz::math::SmoothStep(0, 1, gradient);
					}
					if (has_flag(flags, GradientFlags::PerlinNoise))
					{
		...
		case GradientType::Circular:
		{
			...
			for (uint32_t y = 0; y < height; ++y)
			{
				...
				{
					const XMFLOAT2 uv = XMFLOAT2((float(x) + 0.5f) / float(width), (float(y) + 0.5f) / float(height));
					const float uv_distance = vz::math::Clamp(XMVectorGetX(XMVector3Length(XMLoadFloat2(&uv) - a)), 0, distance);
					float gradient = saturate(vz::math::InverseLerp(0, distance, uv_distance));
					if (has_flag(flags, GradientFlags::Inverse))
					{
						gradient = 1 - gradient;
		...
		case GradientType::Angular:
		{
			...
			for (uint32_t y = 0; y < height; ++y)
			{
				for (uint32_t x = 0; x < width; ++x)
				{
					...
					float gradient = vz::math::GetAngle(direction, coord) / XM_2PI;
					if (has_flag(flags, GradientFlags::Inverse))
					{
						gradient = 1 - gradient;
					}
	...
	)
	{
		...
		for (uint32_t y = 0; y < height; ++y)
		{
			for (uint32_t x = 0; x < width; ++x)
			{
				...
				const float d = vz::math::Length(uv);
