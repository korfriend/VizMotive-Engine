...
bool ImportModel_OBJ(const std::string& fileName, Scene& scene)
{
	...
	auto CreateNode = [&](const NodeType ntype, const std::string& name, Entity parent_entity = 0u) -> Entity
		{
	...
	{
		// root actor //
		root_entity = CreateNode(ACTOR, name);

		// Load material library:
		for (auto& obj_material : obj_materials)
		{

			Entity material_entity = CreateNode(MATERIAL, obj_material.name);
			...
			auto registerMaterial = [&material, &directory, &textures, &CreateNode](const std::string& obj_mat_name, const MaterialComponent::TextureSlot slot)
				{
					...
					{
						Entity texture_entity = CreateNode(TEXTURE, obj_mat_name);
		...
		if (materials.empty())
		{
			...
			CreateNode(MATERIAL, "OBJImport_defaultMaterial");
		}

		// Load actors, meshes:
		for (auto& shape : obj_shapes)
		{
			Entity actor_entity = CreateNode(ACTOR, shape.name, root_entity);

			Entity mesh_entity = CreateNode(GEOMETRY, shape.name + "_mesh");
