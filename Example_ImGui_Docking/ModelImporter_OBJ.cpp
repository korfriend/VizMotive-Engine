...
public:
		...
		std::map<std::string, int>* matMap, std::string* err)
	{
		...
		if (!vz::helper::FileRead(filepath, filedata))
		{
			std::string ss;
...
void ImportModel_OBJ(const std::string& fileName, Scene& scene)
{
	...
	if (success)
	{
		...
		vz::vector<Entity> materialLibrary = {};
		...
		for (auto& shape : obj_shapes)
		{
			...
			vz::unordered_map<int, int> registered_materialIndices = {};
			vz::unordered_map<size_t, uint32_t> uniqueVertices = {};
			...
			for (size_t i = 0; i < shape.mesh.indices.size(); i += 3)
			{
				...
				for (auto& index : reordered_indices)
				{
					...
					size_t vertexHash = 0;
					vz::helper::hash_combine(vertexHash, index.vertex_index);
					vz::helper::hash_combine(vertexHash, index.normal_index);
					vz::helper::hash_combine(vertexHash, index.texcoord_index);
					vz::helper::hash_combine(vertexHash, materialIndex);
