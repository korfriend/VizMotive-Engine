...
HWND hWnd = NULL;
#endif
vz::graphics::SwapChain myswapChain;
...
bool ImGui_Impl_CreateDeviceObjects()
{
	...
	vz::graphics::GetDevice()->CreateTexture(&textureDesc, &textureData, &fontTexture);
	...
	vz::graphics::GetDevice()->CreateSampler(&samplerDesc, &sampler);
	...
	desc.dss = vz::renderer::GetDepthStencilState(vz::enums::DSSTYPE_DEPTHREAD);
	...
	vz::graphics::GetDevice()->CreatePipelineState(&desc, &imguiPSO);
...
void Example_ImGui::Initialize()
{
	// Compile shaders
	{
		auto shaderPath = vz::renderer::GetShaderSourcePath();
		vz::renderer::SetShaderSourcePath(vz::helper::GetCurrentPath() + "/");

		vz::renderer::LoadShader(ShaderStage::VS, imguiVS, "ImGuiVS.cso");
		vz::renderer::LoadShader(ShaderStage::PS, imguiPS, "ImGuiPS.cso");

		vz::renderer::SetShaderSourcePath(shaderPath);
	}
...
void Example_ImGui::Compose(vz::graphics::CommandList cmd)
{
...
void Example_ImGuiRenderer::Load()
{
	...
	// Load model.
	//vz::scene::LoadModel("../Content/models/bloom_test.wiscene");
...
vz::scene::TransformComponent camera_transform;
float camera_pos[3] = { 4.0, 6.0, -15 }, camera_ang[3] = { 22, -13, 0 };
float font_scale = 1.0;
vz::ecs::Entity highlight_entity = -1;
vz::ecs::Entity subset_entity = -1;
...
void Example_ImGuiRenderer::Update(float dt)
{
	...
	auto* backendData = ImGui_Impl_GetBackendData();
	Scene& scene = vz::scene::GetScene();
	CameraComponent& camera = vz::scene::GetCamera();
	...
	if (ImGui::Begin("DockSpaceWicked", &dockingopen, window_flags))
	{
		...
		if (highlight_entity != INVALID_ENTITY)
		{
			...
			if (object != nullptr)
			{
				...
				//vz::physics::SetSimulationEnabled(false);
				...
				if (obj_tranform != nullptr)
				{
					...
					//const vz::primitive::AABB& aabb = *scene.aabb_objects.GetComponent(highlight_entity);
	...
	ImGui::Begin("Scene");
	{
		...
		if (ImGui::CollapsingHeader(ICON_MD_PLAY_CIRCLE_OUTLINE"  Scene Demo", ImGuiTreeNodeFlags_DefaultOpen)) //ImGuiTreeNodeFlags_None
		{
			...
			{
				//std::string ext = vz::helper::toUpper(vz::helper::GetExtensionFromFileName(wiscene_file_items[current_scene]));
				//if (!ext.compare("GLB")) // binary gltf
				//{
				//	scene.Clear();
				//	Scene scene;
				//	ImportModel_GLTF(wiscene_file_items[current_scene], scene);
				//	vz::scene::GetScene().Merge(scene);
				//}
				//else
				//{
				//	scene.Clear();
				//	vz::scene::LoadModel(wiscene_file_items[current_scene]);
				//}
		...
		if (ImGui::CollapsingHeader( ICON_MD_TEXTURE "  Scene Materials", ImGuiTreeNodeFlags_DefaultOpen)) //ImGuiTreeNodeFlags_None
		{
			...
			vz::scene::Scene* pScene = &vz::scene::GetScene();
			...
			for (int i = 0; i < size; i++)
			{
				for (int a = 0; a < 13; a++) //TEXTURESLOT_COUNT = 13
				{
					...
					{
						vz::Resource& image = pScene->materials[i].textures[a].resource;
						...
						if (image.IsValid())
						{
							...
							if (lpTexture)
							{
								...
								if (ImGui::IsItemHovered())
								{
									...
									std::string filename = vz::helper::GetFileNameFromPath(pScene->materials[i].textures[a].name);
		...
		if (highlight_entity != INVALID_ENTITY)
		{
			...
			if (object != nullptr)
			{
				if (!boundSizing)
				{
					...
					const vz::primitive::AABB& aabb = scene.aabb_objects[scene.objects.GetIndex(highlight_entity)];
					XMFLOAT4X4 hoverBox;
					XMStoreFloat4x4(&hoverBox, aabb.getAsBoxMatrix());
					vz::renderer::DrawBox(hoverBox, XMFLOAT4(1.0f, 0.9f, 0.5f, 0.4f));
				}
			}
		}
		...
		if (!imgui_got_focus_last && !bIsUsingWidget)
		{
			...
			vz::primitive::Ray pickRay = vz::renderer::GetPickRay((long)mouse_x, (long)mouse_y, *this);
			if (ImGui::IsMouseClicked(0))
			{
				PickResult hovered = vz::scene::Pick(pickRay, vz::enums::FILTER_ALL);
	...
	ImGui::Begin("Options");
	{
		...
		if (ImGui::CollapsingHeader(ICON_MD_HIGHLIGHT_ALT "  Selection", ImGuiTreeNodeFlags_DefaultOpen)) //ImGuiTreeNodeFlags_None
		{
			if (object != nullptr)
			{
				...
				if (mesh)
				{
					vz::primitive::AABB aabb = mesh->aabb;
		...
		if (ImGui::CollapsingHeader(ICON_MD_SETTINGS "  Settings", ImGuiTreeNodeFlags_None)) //ImGuiTreeNodeFlags_DefaultOpen
		{
			...
			{
				vz::eventhandler::SetVSync(bVSync);
			}

			bool bPhysics = false;// vz::physics::IsSimulationEnabled();
			if (ImGui::Checkbox("Physics On", &bPhysics))
			{
				//vz::physics::SetSimulationEnabled(bPhysics);
			}
			...
			{
				vz::renderer::SetToDrawGridHelper(bDrawGrid);
			}
			static bool bTemporalAAEnabled = false;
			if (ImGui::Checkbox("Temporal AA", &bTemporalAAEnabled))
			{
				vz::renderer::SetTemporalAAEnabled(bTemporalAAEnabled);
			}
			...
			{
				vz::renderer::SetVXGIEnabled(bVoxelGIEnabled);
			}
		...
		if (ImGui::CollapsingHeader(ICON_MD_SCREENSHOT_MONITOR "  Camera Screenshot", ImGuiTreeNodeFlags_None)) //ImGuiTreeNodeFlags_DefaultOpen
		{
			...
			if (ImGui::Button("Take Screenshot", ImVec2(IMGUIBUTTONWIDE, 0)))
			{
				...
				vz::helper::screenshot(myswapChain, name);

				MaterialComponent::TextureMap tm;
				tm.resource = vz::resourcemanager::Load(name, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
				tm.name = name;
	...
	ImGui::Begin(ICON_MD_BUG_REPORT " Debugger", &bAlwaysOpen, ImGuiWindowFlags_None); //ImGuiWindowFlags_NoScrollbar
	{
		...
		std::vector< vz::graphics::Texture > debug_textures;
	...
	ImGui::Begin(ICON_MD_TEXT_SNIPPET " Wicked Backlog", &bAlwaysOpen, ImGuiWindowFlags_NoScrollbar);
	{
		...
		std::string log = vz::backlog::getText();
		if (log.size() > 0)
		{
			lastlog += log;
			vz::backlog::clear();
	...
	if (!imgui_got_focus_last)
	{
		...
		*/
		...
		if (camControlStart)
		{
			originalMouse = vz::input::GetPointer();
		}

		if (vz::input::Down(vz::input::MOUSE_BUTTON_MIDDLE) || vz::input::Down(vz::input::MOUSE_BUTTON_RIGHT))
		{
			camControlStart = false;
			// Mouse delta from hardware read:
			float xDif = vz::input::GetMouseState().delta_position.x;
			float yDif = vz::input::GetMouseState().delta_position.y;
			xDif = 0.1f * xDif;
			yDif = 0.1f * yDif;
			vz::input::SetPointer(originalMouse);
			vz::input::HidePointer(true);
			camera_ang[0] += yDif;
			camera_ang[1] += xDif;
			if (camera_ang[0] < -89.999f)  camera_ang[0] = -89.999f;
			if (camera_ang[0] > 89.999f)  camera_ang[0] = 89.999f;
...
void Example_ImGuiRenderer::DisplayPerformanceData(bool* p_open)
{
	...
	{
		ImGui::Text("Wicked Engine (%s) - FPS: %.1f", vz::version::GetVersionString(),ImGui::GetIO().Framerate);
	}
	ImGui::End();
}
