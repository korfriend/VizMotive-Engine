...
namespace vz::renderer
{
	...
	void GRenderPath3DDetails::UpdatePerFrameData(Scene& scene, const View& vis, FrameCB& frameCB, float dt)
	{
		...
		{
			/*
			auto isCameraStable = [](const CameraComponent& camera, const CameraComponent& cameraPrevious, float screenWidth, float screenHeight)
				{
					// 1. Check if camera position change is less than or equal to 1 pixel
					const XMFLOAT3& currentEye = camera.GetWorldEye();
					const XMFLOAT3& previousEye = cameraPrevious.GetWorldEye();

					// Convert world space position difference to screen space
					XMVECTOR worldPosDiff = XMVectorSubtract(XMLoadFloat3(&currentEye), XMLoadFloat3(&previousEye));

					// Project position difference to screen space using view-projection matrix
					XMMATRIX viewProj = XMLoadFloat4x4(&camera.GetViewProjection());
					XMVECTOR screenPosDiff = XMVector3TransformNormal(worldPosDiff, viewProj);

					// Convert from NDC [-1,1] to pixel coordinates
					float pixelDiffX = abs(XMVectorGetX(screenPosDiff) * screenWidth * 0.5f);
					float pixelDiffY = abs(XMVectorGetY(screenPosDiff) * screenHeight * 0.5f);

					bool positionStable = std::max(pixelDiffX, pixelDiffY) <= 0.1f;
					
					// 2. Check rotation using quaternions
					const XMFLOAT3X3& currentRot = camera.GetWorldRotation();
					const XMFLOAT3X3& previousRot = cameraPrevious.GetWorldRotation();

					// Convert rotation matrices to quaternions
					XMMATRIX currentRotMat = XMLoadFloat3x3(&currentRot);
					XMMATRIX previousRotMat = XMLoadFloat3x3(&previousRot);

					XMVECTOR currentQuat = XMQuaternionRotationMatrix(currentRotMat);
					XMVECTOR previousQuat = XMQuaternionRotationMatrix(previousRotMat);

					// Calculate the difference quaternion
					// Note: XMQuaternionMultiply(q1, XMQuaternionConjugate(q2)) gives the rotation from q2 to q1
					XMVECTOR diffQuat = XMQuaternionMultiply(currentQuat, XMQuaternionInverse(previousQuat));

					// Extract the angle from the difference quaternion
					// In a difference quaternion, the angle represents the rotation amount between the two orientations
					float angle = 2.0f * acos(std::min(1.0f, fabs(XMVectorGetW(diffQuat))));

					// Small epsilon for rotation comparison
					const float ROTATION_EPSILON = 0.001f; // about 0.057 degrees
					bool rotationStable = angle < ROTATION_EPSILON;

					// 3. For scale, check projection parameters
					bool scaleStable = true;

					// For perspective camera, check FOV and aspect ratio
					if (!camera.IsOrtho()) {
						float currentFov = camera.GetFovVertical();
						float previousFov = cameraPrevious.GetFovVertical();

						float currentW, currentH, previousW, previousH;
						camera.GetWidthHeight(&currentW, &currentH);
						cameraPrevious.GetWidthHeight(&previousW, &previousH);

						float currentAspect = currentW / currentH;
						float previousAspect = previousW / previousH;

						const float PARAMS_EPSILON = 0.0001f;
						scaleStable = (fabs(currentFov - previousFov) < PARAMS_EPSILON) &&
							(fabs(currentAspect - previousAspect) < PARAMS_EPSILON);
					}
					// For orthographic camera, check orthographic size
					else {
						float currentSize = camera.GetOrthoVerticalSize();
						float previousSize = cameraPrevious.GetOrthoVerticalSize();

						const float PARAMS_EPSILON = 0.0001f;
						scaleStable = fabs(currentSize - previousSize) < PARAMS_EPSILON;
					}

					// All conditions must be met
					return positionStable;//&& rotationStable&& scaleStable;
				};

			if (camera->IsSlicer())
			{
				if (isCameraStable(*camera, cameraPrevious, (float)canvasWidth_, (float)canvasHeight_))
					stableFrameCount++;
				else
					stableFrameCount = 0;
			}
