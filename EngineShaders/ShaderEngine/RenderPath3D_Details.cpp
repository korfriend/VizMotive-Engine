...
namespace vz::renderer
{
	...
	bool GRenderPath3DDetails::UpdateGeometryGPUBVH(const Entity geometryEntity, CommandList cmd)
	{
		...
		for (size_t i = 0; i < parts.size(); ++i)
		{
			...
			{
				device->BindComputeShader(&shaders[CSTYPE_BVH_PRIMITIVES_GEOMETRYONLY], cmd);
			...
			{
				device->BindComputeShader(&shaders[CSTYPE_BVH_HIERARCHY], cmd);
				const GPUResource* uavs[] = {
			...
			device->EventBegin("BVH - Propagate AABB", cmd);
			{
				...
				device->BindComputeShader(&shaders[CSTYPE_BVH_PROPAGATEAABB], cmd);
				const GPUResource* uavs[] = {
					&bvhBuffers.bvhNodeBuffer,
					&bvhBuffers.bvhFlagBuffer,
				};
