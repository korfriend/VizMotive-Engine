// Note: this file is modified for VizMotive Engine from the original
...
#ifndef SH_LITE_HLSLI
#define SH_LITE_HLSLI
...
static const half BasisL1 = half(sqrt(3) / (2 * SqrtPi));
static const half BasisL2_MN2 = half(sqrt(15) / (2 * SqrtPi));
static const half BasisL2_MN1 = half(sqrt(15) / (2 * SqrtPi));
static const half BasisL2_M0 = half(sqrt(5) / (4 * SqrtPi));
static const half BasisL2_M1 = half(sqrt(15) / (2 * SqrtPi));
static const half BasisL2_M2 = half(sqrt(15) / (4 * SqrtPi));
...
{
	L1 ret;
	ret.C[0] = (half)f16tof32(C[0]);
	ret.C[1] = (half)f16tof32(C[0] >> 16u);
	ret.C[2] = (half)f16tof32(C[1]);
	ret.C[3] = (half)f16tof32(C[1] >> 16u);
	return ret;
}
