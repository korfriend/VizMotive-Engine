#ifndef VOLUMETRICCLOUDS_HF
#define VOLUMETRICCLOUDS_HF 
...
LayerParameters SampleLayerParameters(VolumetricCloudLayer layer)
{
	...
	float animation = GetEnvironment().volumetric_clouds.animationMultiplier * GetFrame().time;
...
float GetHeightFractionForPoint(AtmosphereParameters atmosphere, float3 pos)
{
	...
	return saturate((distance(pos, planetCenterWorld) - (planetRadius + GetEnvironment().volumetric_clouds.cloudStartHeight)) / GetEnvironment().volumetric_clouds.cloudThickness);
}
...
float3 GetEnvironmentTypeMask(float3 weatherData)
{
...
{
	float3 weatherTypeMask = GetEnvironmentTypeMask(weatherData);
...
{
	float3 albedoFirst = densityFirst * GetEnvironment().volumetric_clouds.layerFirst.albedo;
	albedoFirst = pow(saturate(albedoFirst * GetEnvironment().volumetric_clouds.beerPowder), GetEnvironment().volumetric_clouds.beerPowderPower); // Artistic approach
	albedoFirst *= (1.0 - weatherDataFirst.b);
	
	float3 albedoSecond = densitySecond * GetEnvironment().volumetric_clouds.layerSecond.albedo;
	albedoSecond = pow(saturate(albedoSecond * GetEnvironment().volumetric_clouds.beerPowder), GetEnvironment().volumetric_clouds.beerPowderPower);
...
{
	float3 extinctionFirst = densityFirst * GetEnvironment().volumetric_clouds.layerFirst.extinctionCoefficient;
	float3 extinctionSecond = densitySecond * GetEnvironment().volumetric_clouds.layerSecond.extinctionCoefficient;
	
	return saturate(extinctionFirst + extinctionSecond);
}

#endif // VOLUMETRICCLOUDS_HF
