	...
	bool enableSun, bool darkMode, bool stationary, bool highQuality, bool perPixelNoise, bool receiveShadow)
{
    ...
    {
        float3 sunIlluminance = sunColor;
		totalColor = luminance + GetSunLuminance(worldPosition, worldDirection, sunDirection, (half3)sunIlluminance, atmosphere, texture_transmittancelut);
	}
    else
    {
	...
	if (GetFrame().options & OPTION_BIT_HEIGHT_FOG)
	{
		...
		const float top_radius = bottom_radius + GetEnvironment().fog.height_end;
		...
		if(dist >= 0)
		{
			...
			const float3 offsetO = rayOrigin + rayDirection * GetEnvironment().fog.start;
...
{
	ShaderEnvironment weather = GetEnvironment();
