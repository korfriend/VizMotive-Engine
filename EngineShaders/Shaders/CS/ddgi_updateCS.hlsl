...
void main(uint2 GTid : SV_GroupThreadID, uint2 Gid : SV_GroupID, uint groupIndex : SV_GroupIndex)
{
...
#ifdef DDGI_UPDATE_DEPTH
	float2 result = 0;
	const uint2 pixel_topleft = ddgi_probe_depth_pixel((min16uint3)probeCoord);
	...
	const float3 texel_direction = decode_oct(half2((((GTid.xy % RESOLUTION) + 0.5) / (float2)RESOLUTION) * 2 - 1));
	...
	shared_depths[flatten2D(GTid, DDGI_DEPTH_RESOLUTION)] = pack_half2((half2)result);
	...
	if (groupIndex == 0)
	{
		...
		if(GetScene().voxelgrid.IsValid())
		{
			...
			uint3 coord = voxelgrid.world_to_coord(ddgi_probe_position_rest((min16uint3)probeCoord));
		...
		probeOffset /= probe_limit;
		ddgiProbeBuffer[probeIndex].offset = pack_half3((half3)probeOffset);
	}
#else
	...
	if (groupIndex == 0)
	{
		...
		for (int x = 0; x < RESOLUTION; ++x)
		{
			...
			{
				const float3 direction = decode_oct(half2(((float2(x, y) + 0.5) / (float2)RESOLUTION) * 2 - 1));
		...
		radiance = SH::Multiply(radiance, (half3)rcp(RESOLUTION * RESOLUTION * SPHERE_SAMPLING_PDF));
