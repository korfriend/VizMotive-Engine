...
void main(uint3 DTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID, uint3 Gid : SV_GroupID, uint groupIndex : SV_GroupIndex)
{
	...
	for (uint t = groupIndex; t < TILE_SIZE * TILE_SIZE; t += POSTPROCESS_BLOCKSIZE * POSTPROCESS_BLOCKSIZE)
	{
		...
		const half depth = (half)texture_lineardepth[pixel];
		const half3 color = (half3)input_current[pixel].rgb;
	...
	half3 history = (half3)input_history.SampleLevel(sampler_linear_clamp, prevUV, 0).rgb;
	...
	half subpixelCorrection = (half)(frac(max(abs(velocity.x) * GetCamera().internal_resolution.x, abs(velocity.y) * GetCamera().internal_resolution.y)) * 0.5);
