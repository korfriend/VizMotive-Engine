...
#include "../CommonHF/skyAtmosphere.hlsli"
...
#else
	#define MAX_STEP_COUNT GetEnvironment().volumetric_clouds.maxStepCount
	#define LOD_Min GetEnvironment().volumetric_clouds.LODMin
	#define SHADOW_SAMPLE_COUNT GetEnvironment().volumetric_clouds.shadowSampleCount
	#define GROUND_CONTRIBUTION_SAMPLE_COUNT GetEnvironment().volumetric_clouds.groundContributionSampleCount
#endif // VOLUMETRICCLOUD_CAPTURE
...
void VolumetricShadow(inout ParticipatingMedia participatingMedia, in AtmosphereParameters atmosphere, float3 worldPosition, float3 sunDirection, LayerParameters layerParametersFirst, LayerParameters layerParametersSecond, float lod)
{
	...
	for (float s = 0.0f; s < sampleCount; s += 1.0)
	{
		...
		float shadowSampleT = GetEnvironment().volumetric_clouds.shadowStepLength * t;
		...
		ParticipatingMedia shadowParticipatingMedia = SampleParticipatingMedia(0.0f, shadowExtinction, GetEnvironment().volumetric_clouds.multiScatteringScattering, GetEnvironment().volumetric_clouds.multiScatteringExtinction, 0.0f);
	...
	for (ms = 0; ms < MS_COUNT; ms++)
	{
		participatingMedia.transmittanceToLight[ms] *= exp(-extinctionAccumulation[ms] * GetEnvironment().volumetric_clouds.shadowStepLength);
	}
}
...
void VolumetricGroundContribution(inout float3 environmentLuminance, in AtmosphereParameters atmosphere, float3 worldPosition, float3 sunDirection, float3 sunIlluminance, float3 atmosphereTransmittanceToLight, LayerParameters layerParametersFirst, LayerParameters layerParametersSecond, float lod)
{
	...
	float cloudBottomRadius = planetRadius + GetEnvironment().volumetric_clouds.cloudStartHeight;
...
float3 SampleAmbientLight(float heightFraction)
{
	...
	//float ambientTerm = -cloudDensity * (1.0 - saturate(GetEnvironment().volumetric_clouds.CloudAmbientGroundMultiplier + heightFraction));
	//float isotropicScatteringTopContribution = max(0.0, exp(ambientTerm) - ambientTerm * ExponentialIntegral(ambientTerm));

	float isotropicScatteringTopContribution = saturate(GetEnvironment().volumetric_clouds.ambientGroundMultiplier + heightFraction);
	...
	inout float3 luminance, inout float3 transmittanceToView, inout float depthWeightedSum, inout float depthWeightsSum)
{
	...
	ParticipatingMedia participatingMedia = SampleParticipatingMedia(albedo, extinction, GetEnvironment().volumetric_clouds.multiScatteringScattering, GetEnvironment().volumetric_clouds.multiScatteringExtinction, atmosphereTransmittanceToLight);
	...
	float phaseFunction = DualLobPhase(GetEnvironment().volumetric_clouds.phaseG, GetEnvironment().volumetric_clouds.phaseG2, GetEnvironment().volumetric_clouds.phaseBlend, -cosTheta);
	ParticipatingMediaPhase participatingMediaPhase = SampleParticipatingMediaPhase(phaseFunction, GetEnvironment().volumetric_clouds.multiScatteringEccentricity);
...
float CalculateAtmosphereBlend(float tDepth)
{
	...
	float fogDistance = saturate(tDepth * GetEnvironment().volumetric_clouds.horizonBlendAmount);
    
	float fade = pow(fogDistance, GetEnvironment().volumetric_clouds.horizonBlendPower);
...
{	
	AtmosphereParameters atmosphere = GetEnvironment().atmosphere;
	...
	float stepSize;
    {
		...
		const float cloudBottomRadius = planetRadius + GetEnvironment().volumetric_clouds.cloudStartHeight;
		const float cloudTopRadius = planetRadius + GetEnvironment().volumetric_clouds.cloudStartHeight + GetEnvironment().volumetric_clouds.cloudThickness;
		...
		if (tMax <= tMin || tMin > GetEnvironment().volumetric_clouds.renderDistance)
		{
			cloudColor = float4(0.0, 0.0, 0.0, 0.0);
			cloudDepth = FLT_MAX;
			return;
		...
		const float marchingDistance = min(GetEnvironment().volumetric_clouds.maxMarchingDistance, tMax - tMin);
		tMax = tMin + marchingDistance;

		steps = MAX_STEP_COUNT * saturate((tMax - tMin) * (1.0 / GetEnvironment().volumetric_clouds.inverseDistanceStepCount));
	...
	// Sample layers
	LayerParameters layerParametersFirst = SampleLayerParameters(GetEnvironment().volumetric_clouds.layerFirst);
	LayerParameters layerParametersSecond = SampleLayerParameters(GetEnvironment().volumetric_clouds.layerSecond);
	...
	for (float i = 0.0; i < steps; i++)
	{
		...
		if (!ValidCloudDensityLayers(heightFraction, weatherDataFirst, weatherDataSecond, layerParametersFirst, layerParametersSecond))
		{
			...
			i += GetEnvironment().volumetric_clouds.bigStepMarch - 1;
			t += GetEnvironment().volumetric_clouds.bigStepMarch * stepSize;
			
			continue;
		}
		
		float tProgress = distance(rayOrigin, sampleWorldPosition);
		float lod = round(tProgress / max(GetEnvironment().volumetric_clouds.LODDistance, 0.00001));
		...
		if (saturate(cloudDensityFirst + cloudDensitySecond) > CLOUD_DENSITY_THRESHOLD)
		{
			...
			if (all(transmittanceToView < GetEnvironment().volumetric_clouds.transmittanceThreshold))
			{
				break;
			}
		}
	...
	float grayScaleTransmittance = approxTransmittance < GetEnvironment().volumetric_clouds.transmittanceThreshold ? 0.0 : approxTransmittance;
