...
float4 main(VertexToPixel input) : SV_TARGET
{
	...
	P = P + V * stepSize * dither(inputPosXY);
	...
	for (uint i = 0; i < sampleCount; ++i)
	{
		...
		const half spot_factor = dot((half)L, light.GetDirection());
		...
		if (spot_factor > spot_cutoff)
		{
			...
			if (light.IsCastingShadow())
			{
				...
				{
					attenuation *= shadow_2D(light, shadow_pos.xyz, shadow_uv.xy, 0, inputPosXY);
				}
			}
