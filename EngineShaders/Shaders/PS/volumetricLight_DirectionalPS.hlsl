...
float4 main(VertexToPixel input) : SV_Target
{
	...
	const half scattering = (half)ComputeScattering(saturate(dot(float3(L), -V)));
	...
	P = P + V * min(stepSize * (float)dither(inputPosXY), 10); // limit dithering step to 10 to avoid very large dither on sky
	...
	for (uint i = 0; i < sampleCount; ++i)
	{
		...
		for (uint cascade = 0; cascade < light.GetShadowCascadeCount(); ++cascade)
		{
			...
			{
				shadow *= shadow_2D(light, shadow_pos, shadow_uv.xy, cascade, inputPosXY);
				break;
			}
		}
		...
		shadow *= (half)(g_xColor.y + GetFogAmount(cameraDistance - marchedDistance, P, (float3)0));
		shadow *= scattering;
