...
namespace vulkan_internal
{
	...
	//	These are also defined in vz::shadercompiler.cpp as hard coded compiler arguments for SPIRV, so they need to be the same
	enum
	{
	...
	constexpr VkStencilOp _ConvertStencilOp(StencilOp value)
	{
		switch (value)
		{
		case vz::graphics::StencilOp::KEEP:
			return VK_STENCIL_OP_KEEP;
		case vz::graphics::StencilOp::ZERO:
			return VK_STENCIL_OP_ZERO;
		case vz::graphics::StencilOp::REPLACE:
			return VK_STENCIL_OP_REPLACE;
		case vz::graphics::StencilOp::INCR_SAT:
			return VK_STENCIL_OP_INCREMENT_AND_CLAMP;
		case vz::graphics::StencilOp::DECR_SAT:
			return VK_STENCIL_OP_DECREMENT_AND_CLAMP;
		case vz::graphics::StencilOp::INVERT:
			return VK_STENCIL_OP_INVERT;
		case vz::graphics::StencilOp::INCR:
			return VK_STENCIL_OP_INCREMENT_AND_WRAP;
		case vz::graphics::StencilOp::DECR:
	...
	bool checkExtensionSupport(const char* checkExtension, const vz::vector<VkExtensionProperties>& available_extensions)
	{
	...
	bool ValidateLayers(const vz::vector<const char*>& required,
		const vz::vector<VkLayerProperties>& available)
	{
		...
		void* user_data)
	{
		...
		if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
		{
			...
			vz::backlog::post(ss, vz::backlog::LogLevel::Warning);
		}
		else if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
		{
			...
			ss += callback_data->pMessage;
#if 1
			vz::backlog::post(ss, vz::backlog::LogLevel::Error);
#else
...
//#endif // _DEBUG
	...
	struct Buffer_Vulkan
	{
		...
		vz::vector<BufferSubresource> subresources_srv;
		vz::vector<BufferSubresource> subresources_uav;
	...
	struct Texture_Vulkan
	{
		...
		vz::vector<TextureSubresource> subresources_srv;
		vz::vector<TextureSubresource> subresources_uav;
		vz::vector<TextureSubresource> subresources_rtv;
		vz::vector<TextureSubresource> subresources_dsv;

		vz::vector<SubresourceData> mapped_subresources;
	...
	struct Shader_Vulkan
	{
		...
		vz::vector<VkDescriptorSetLayoutBinding> layoutBindings;
		vz::vector<VkImageViewType> imageViewTypes;

		vz::vector<BindingUsage> bindlessBindings;
		vz::vector<VkDescriptorSet> bindlessSets;
		uint32_t bindlessFirstSet = 0;

		VkPushConstantRange pushconstants = {};

		VkDeviceSize uniform_buffer_sizes[DESCRIPTORBINDER_CBV_COUNT] = {};
		vz::vector<uint32_t> uniform_buffer_dynamic_slots;
	...
	struct BVH_Vulkan
	{
		...
		vz::vector<VkAccelerationStructureGeometryKHR> geometries;
		vz::vector<uint32_t> primitiveCounts;
	...
	struct SwapChain_Vulkan
	{
		...
		vz::vector<VkImage> swapChainImages;
		vz::vector<VkImageView> swapChainImageViews;
		...
		vz::vector<VkSemaphore> swapchainAcquireSemaphores;
	...
	struct VideoDecoder_Vulkan
	{
		...
		vz::vector<VmaAllocation> allocations;
	...
	{
		return vz::helper::GetCacheDirectoryPath() + "/wiPipelineCache_Vulkan";
	}
	...
	)
	{
		...
		vz::vector<VkSurfaceFormatKHR> swapchain_formats(formatCount);
		...
		vz::vector<VkPresentModeKHR> swapchain_presentModes(presentModeCount);
...
using namespace vulkan_internal;
	...
	GraphicsDevice_Vulkan::CopyAllocator::CopyCMD GraphicsDevice_Vulkan::CopyAllocator::allocate(uint64_t staging_size)
	{
		...
		if (!cmd.IsValid())
		{
			...
			uploaddesc.size = vz::math::GetNextPowerOfTwo(staging_size);
	...
	void GraphicsDevice_Vulkan::pso_validate(CommandList cmd)
	{
		...
		if (it == pipelines_global.end())
		{
			...
			if (pipeline == VK_NULL_HANDLE)
			{
				...
				vz::vector<VkVertexInputBindingDescription> bindings;
				vz::vector<VkVertexInputAttributeDescription> attributes;
	...
	// Engine functions
	GraphicsDevice_Vulkan::GraphicsDevice_Vulkan(vz::platform::window_type window, ValidationMode validationMode_, GPUPreference preference)
	{
		vz::Timer timer;
		...
		vz::unordered_map<uint32_t, std::shared_ptr<std::mutex>> queue_lockers;
		...
		if (res != VK_SUCCESS)
		{
			vz::helper::messageBox("volkInitialize failed! ERROR: " + std::to_string(res), "Error!");
			vz::platform::Exit();
		}
		...
		assert(res == VK_SUCCESS);
		vz::vector<VkLayerProperties> availableInstanceLayers(instanceLayerCount);
		...
		assert(res == VK_SUCCESS);
		vz::vector<VkExtensionProperties> availableInstanceExtensions(extensionCount);
		res = vkEnumerateInstanceExtensionProperties(nullptr, &extensionCount, availableInstanceExtensions.data());
		assert(res == VK_SUCCESS);

		vz::vector<const char*> instanceLayers;
		vz::vector<const char*> instanceExtensions;
...
#elif SDL2
		{
			...
			vz::vector<const char *> extensionNames_sdl(extensionCount);
		...
		if (validationMode != ValidationMode::Disabled)
		{
			...
			static const vz::vector<const char*> validationLayerPriorityList[] =
			{
		...
		// Create instance:
		{
			...
			if (res != VK_SUCCESS)
			{
				vz::helper::messageBox("vkCreateInstance failed! ERROR: " + std::to_string(res), "Error!");
				vz::platform::Exit();
			}
		...
		// Enumerating and creating devices:
		{
			...
			{
				assert(0);
				vz::helper::messageBox("Failed to find GPU with Vulkan support!");
				vz::platform::Exit();
			}

			vz::vector<VkPhysicalDevice> devices(deviceCount);
			res = vkEnumeratePhysicalDevices(instance, &deviceCount, devices.data());
			assert(res == VK_SUCCESS);

			const vz::vector<const char*> required_deviceExtensions = {
				VK_KHR_SWAPCHAIN_EXTENSION_NAME,
			};
			vz::vector<const char*> enabled_deviceExtensions;
			...
			auto checkPhysicalDeviceAndFillProperties2 = [&](VkPhysicalDevice dev) {
				...
				assert(res == VK_SUCCESS);
				vz::vector<VkExtensionProperties> available_deviceExtensions(extensionCount);
			...
			{
				assert(0);
				vz::helper::messageBox("Failed to find a suitable GPU!");
				vz::platform::Exit();
			}
			...
			vz::vector<VkDeviceQueueCreateInfo> queueCreateInfos;
			vz::unordered_set<uint32_t> uniqueQueueFamilies = { graphicsFamily,copyFamily,computeFamily };
			if (videoFamily != VK_QUEUE_FAMILY_IGNORED)
			{
			...
			if (res != VK_SUCCESS)
			{
				vz::helper::messageBox("vkCreateDevice failed! ERROR: " + std::to_string(res), "Error!");
				vz::platform::Exit();
			}
		...
		if (res != VK_SUCCESS)
		{
			vz::helper::messageBox("vmaCreateAllocator failed! ERROR: " + std::to_string(res), "Error!");
			vz::platform::Exit();
		}
		...
		if (res != VK_SUCCESS)
		{
			vz::helper::messageBox("Failed to create Vulkan external memory allocator, ERROR: " + std::to_string(res), "Error!");
			vz::platform::Exit();
		}
		...
		for (uint32_t fr = 0; fr < BUFFERCOUNT; ++fr)
		{
			for (int queue = 0; queue < QUEUE_COUNT; ++queue)
			{
				...
				if (res != VK_SUCCESS)
				{
					vz::helper::messageBox("vkCreateFence[FRAME] failed! ERROR: " + std::to_string(res), "Error!");
					vz::platform::Exit();
				}
			}
		}
		...
		// Pipeline Cache
		{
			...
			vz::vector<uint8_t> pipelineData;

			std::string cachePath = GetCachePath(); 
			if (!vz::helper::FileRead(cachePath, pipelineData))
			{
				pipelineData.clear();
			}
		...
		vz::backlog::post("Created GraphicsDevice_Vulkan (" + std::to_string((int)std::round(timer.elapsed())) + " ms)\nAdapter: " + adapterName);
	}
	...
	bool GraphicsDevice_Vulkan::CreateSwapChain(const SwapChainDesc* desc, vz::platform::window_type window, SwapChain* swapchain) const
	{
...
#error WICKEDENGINE VULKAN DEVICE ERROR: PLATFORM NOT SUPPORTED
	...
	bool GraphicsDevice_Vulkan::CreateTexture(const TextureDesc* desc, const SubresourceData* initial_data, Texture* texture) const
	{
			...
			)
		{
			...
			vz::vector<VkVideoFormatPropertiesKHR> video_format_properties(format_property_count);
		...
		if (has_flag(texture->desc.misc_flags, ResourceMiscFlag::SPARSE))
		{
			...
			vz::vector<VkSparseImageMemoryRequirements> sparse_requirements(sparse_requirement_count);
		...
		if (initial_data != nullptr)
		{
			...
			vz::vector<VkBufferImageCopy> copyRegions;
	...
	bool GraphicsDevice_Vulkan::CreateShader(ShaderStage stage, const void* shadercode, size_t shadercode_size, Shader* shader) const
	{
...

		{
			...
			vz::vector<SpvReflectDescriptorBinding*> bindings(binding_count);
			result = spvReflectEnumerateDescriptorBindings(
			...
			vz::vector<SpvReflectBlockVariable*> pushconstants(push_count);
			...
			if (stage == ShaderStage::CS || stage == ShaderStage::LIB)
			{
				...
				{
					vz::helper::hash_combine(internal_state->binding_hash, x.binding);
					vz::helper::hash_combine(internal_state->binding_hash, x.descriptorCount);
					vz::helper::hash_combine(internal_state->binding_hash, x.descriptorType);
					vz::helper::hash_combine(internal_state->binding_hash, x.stageFlags);
					vz::helper::hash_combine(internal_state->binding_hash, internal_state->imageViewTypes[i++]);
				}
				for (auto& x : internal_state->bindlessBindings)
				{
					vz::helper::hash_combine(internal_state->binding_hash, x.used);
					vz::helper::hash_combine(internal_state->binding_hash, x.binding.binding);
					vz::helper::hash_combine(internal_state->binding_hash, x.binding.descriptorCount);
					vz::helper::hash_combine(internal_state->binding_hash, x.binding.descriptorType);
					vz::helper::hash_combine(internal_state->binding_hash, x.binding.stageFlags);
				}
				vz::helper::hash_combine(internal_state->binding_hash, internal_state->pushconstants.offset);
				vz::helper::hash_combine(internal_state->binding_hash, internal_state->pushconstants.size);
				vz::helper::hash_combine(internal_state->binding_hash, internal_state->pushconstants.stageFlags);

				pso_layout_cache_mutex.lock();
				if (pso_layout_cache[internal_state->binding_hash].pipelineLayout == VK_NULL_HANDLE)
				{
					vz::vector<VkDescriptorSetLayout> layouts;
	...
	bool GraphicsDevice_Vulkan::CreateRaytracingPipelineState(const RaytracingPipelineStateDesc* desc, RaytracingPipelineState* rtpso) const
	{
		...
		vz::vector<VkPipelineShaderStageCreateInfo> stages;
		...
		vz::vector<VkRayTracingShaderGroupCreateInfoKHR> groups;
	...
	bool GraphicsDevice_Vulkan::CreateVideoDecoder(const VideoDesc* desc, VideoDecoder* video_decoder) const
	{
		...
		vz::vector<StdVideoH264PictureParameterSet> pps_array_h264(desc->pps_count);
		vz::vector<StdVideoH264ScalingLists> scalinglist_array_h264(desc->pps_count);
		...
		vz::vector<StdVideoH264SequenceParameterSet> sps_array_h264(desc->sps_count);
		vz::vector<StdVideoH264SequenceParameterSetVui> vui_array_h264(desc->sps_count);
		vz::vector<StdVideoH264HrdParameters> hrd_array_h264(desc->sps_count);
		...
		vz::vector<VkVideoSessionMemoryRequirementsKHR> requirements(requirement_count);
		...
		vz::vector<VkBindVideoSessionMemoryInfoKHR> bind_session_memory_infos(requirement_count);
	...
	CommandList GraphicsDevice_Vulkan::BeginCommandList(QUEUE_TYPE queue)
	{
		...
		if (commandlist.GetCommandBuffer() == VK_NULL_HANDLE)
		{
			...
			for (uint32_t buffer = 0; buffer < BUFFERCOUNT; ++buffer)
			{
				...
				switch (queue)
				{
				case vz::graphics::QUEUE_GRAPHICS:
					poolInfo.queueFamilyIndex = graphicsFamily;
					break;
				case vz::graphics::QUEUE_COMPUTE:
					poolInfo.queueFamilyIndex = computeFamily;
					break;
				case vz::graphics::QUEUE_COPY:
					poolInfo.queueFamilyIndex = copyFamily;
					break;
				case vz::graphics::QUEUE_VIDEO_DECODE:
	...
	{
		thread_local vz::vector<VkBindSparseInfo> sparse_infos;
		struct DataPerBind
		{
			...
			vz::vector<VkSparseMemoryBind> memory_binds;
			vz::vector<VkSparseImageMemoryBind> image_memory_binds;
		};
		thread_local vz::vector<DataPerBind> sparse_binds;
	...
	void GraphicsDevice_Vulkan::BindVertexBuffers(const GPUBuffer *const* vertexBuffers, uint32_t slot, uint32_t count, const uint32_t* strides, const uint64_t* offsets, CommandList cmd)
	{
		...
		for (uint32_t i = 0; i < count; ++i)
		{
			vz::helper::hash_combine(hash, strides[i]);
