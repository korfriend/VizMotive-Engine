...
namespace vz
{
	...
	public:
		...
		virtual void Draw(vz::graphics::CommandList cmd) const;
		...
		vz::Resource textureResource;
		vz::Resource maskResource;
		...
		struct Anim
		{
			...
			struct WobbleAnim
			{
				...
				WobbleAnim()
				{
					for (int i = 0; i < 4; ++i)
					{
						corner_angles[i] = vz::random::GetRandom(0, 1000) / 1000.0f * XM_2PI;
						corner_speeds[i] = vz::random::GetRandom(500, 1000) / 1000.0f;
						if (vz::random::GetRandom(0, 1) == 0)
						{
							corner_speeds[i] *= -1;
						}
						corner_angles2[i] = vz::random::GetRandom(0, 1000) / 1000.0f * XM_2PI;
						corner_speeds2[i] = vz::random::GetRandom(500, 1000) / 1000.0f;
						if (vz::random::GetRandom(0, 1) == 0)
						{
							corner_speeds2[i] *= -1;
						}
