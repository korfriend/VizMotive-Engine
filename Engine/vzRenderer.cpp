...
#include "vzSheenLUT.h"
#include "vzShaderCompiler.h"
#include "vzTimer.h"
#include "vzUnorderedMap.h" // leave it here for shader dump!
#include "vzFont.h"
...
using namespace vz::primitive;
using namespace vz::graphics;
using namespace vz::enums;
...
static thread_local vz::vector<GPUBarrier> barrier_stack;
...
vz::vector<std::pair<XMFLOAT4X4, XMFLOAT4>> renderableBoxes;
vz::vector<std::pair<Sphere, XMFLOAT4>> renderableSpheres;
vz::vector<std::pair<Capsule, XMFLOAT4>> renderableCapsules;
vz::vector<RenderableLine> renderableLines;
vz::vector<RenderableLine2D> renderableLines2D;
vz::vector<RenderablePoint> renderablePoints;
vz::vector<RenderableTriangle> renderableTriangles_solid;
vz::vector<RenderableTriangle> renderableTriangles_wireframe;
vz::vector<uint8_t> debugTextStorage; // A stream of DebugText struct + text characters
vz::vector<PaintRadius> paintrads;
vz::vector<const vz::VoxelGrid*> renderableVoxelgrids;
vz::vector<const vz::PathQuery*> renderablePathqueries;

vz::SpinLock deferredMIPGenLock;
vz::vector<std::pair<Texture, bool>> deferredMIPGens;
vz::vector<std::pair<Texture, Texture>> deferredBCQueue;
...
union ObjectRenderingVariant
{
	struct
	{
		uint32_t renderpass : 4;	// vz::enums::RENDERPASS
		uint32_t shadertype : 8;	// MaterialComponent::SHADERTYPE
		uint32_t blendmode : 4;		// vz::enums::BLENDMODE
		uint32_t cullmode : 2;		// vz::graphics::CullMode
...
vz::unordered_map<uint32_t, PipelineState> PSO_object[RENDERPASS_COUNT][MaterialComponent::SHADERTYPE_COUNT];
...
vz::jobsystem::context object_pso_job_ctx[RENDERPASS_COUNT];
PipelineState PSO_object_wire;
PipelineState PSO_object_wire_tessellation;

vz::vector<CustomShader> customShaders;
...
)
{
	...
	if (device != nullptr)
	{
		...
		{
			vz::backlog::post("shader dump doesn't contain shader: " + shaderbinaryfilename, vz::backlog::LogLevel::Error);
		}
#endif // SHADERDUMP_ENABLED
	}

	vz::shadercompiler::RegisterShader(shaderbinaryfilename);

	if (vz::shadercompiler::IsShaderOutdated(shaderbinaryfilename))
	{
		...
		if (output.IsValid())
		{
			...
			{
				vz::backlog::post(output.error_message, vz::backlog::LogLevel::Warning);
			}
			vz::backlog::post("shader compiled: " + shaderbinaryfilename);
			return device->CreateShader(stage, output.shaderdata, output.shadersize, &shader);
		}
		else
		{
			vz::backlog::post("shader compile FAILED: " + shaderbinaryfilename + "\n" + output.error_message, vz::backlog::LogLevel::Error);
...
void LoadShaders()
{
	...
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OBJECT_COMMON_TESSELLATION], "objectVS_common_tessellation.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OBJECT_PREPASS_TESSELLATION], "objectVS_prepass_tessellation.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OBJECT_PREPASS_ALPHATEST_TESSELLATION], "objectVS_prepass_alphatest_tessellation.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OBJECT_SIMPLE_TESSELLATION], "objectVS_simple_tessellation.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_IMPOSTOR], "impostorVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOLUMETRICLIGHT_DIRECTIONAL], "volumetriclight_directionalVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOLUMETRICLIGHT_POINT], "volumetriclight_pointVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOLUMETRICLIGHT_SPOT], "volumetriclight_spotVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_LIGHTVISUALIZER_SPOTLIGHT], "vSpotLightVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_LIGHTVISUALIZER_POINTLIGHT], "vPointLightVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SPHERE], "sphereVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_OCCLUDEE], "occludeeVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SKY], "skyVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOXELIZER], "objectVS_voxelizer.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_VOXEL], "voxelVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_FORCEFIELDVISUALIZER_POINT], "forceFieldPointVisualizerVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_FORCEFIELDVISUALIZER_PLANE], "forceFieldPlaneVisualizerVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_RAYTRACE_SCREEN], "raytrace_screenVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_POSTPROCESS], "postprocessVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_LENSFLARE], "lensFlareVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_DDGI_DEBUG], "ddgi_debugVS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SCREEN], "screenVS.cso"); });

	if (device->CheckCapability(GraphicsDeviceCapability::RENDERTARGET_AND_VIEWPORT_ARRAYINDEX_WITHOUT_GS))
	{
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_ENVMAP], "envMapVS.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_ENVMAP_SKY], "envMap_skyVS.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SHADOW], "shadowVS.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SHADOW_ALPHATEST], "shadowVS_alphatest.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SHADOW_TRANSPARENT], "shadowVS_transparent.cso"); });
	}
	else
	{
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_ENVMAP], "envMapVS_emulation.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_ENVMAP_SKY], "envMap_skyVS_emulation.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SHADOW], "shadowVS_emulation.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SHADOW_ALPHATEST], "shadowVS_alphatest_emulation.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::VS, shaders[VSTYPE_SHADOW_TRANSPARENT], "shadowVS_transparent_emulation.cso"); });

		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_ENVMAP_EMULATION], "envMapGS_emulation.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_ENVMAP_SKY_EMULATION], "envMap_skyGS_emulation.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_SHADOW_EMULATION], "shadowGS_emulation.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_SHADOW_ALPHATEST_EMULATION], "shadowGS_alphatest_emulation.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_SHADOW_TRANSPARENT_EMULATION], "shadowGS_transparent_emulation.cso"); });
	}

	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_IMPOSTOR], "impostorPS.cso"); });

	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_HOLOGRAM], "objectPS_hologram.cso"); });

	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_DEBUG], "objectPS_debug.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PAINTRADIUS], "objectPS_paintradius.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_SIMPLE], "objectPS_simple.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PREPASS], "objectPS_prepass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PREPASS_ALPHATEST], "objectPS_prepass_alphatest.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PREPASS_DEPTHONLY], "objectPS_prepass_depthonly.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_OBJECT_PREPASS_DEPTHONLY_ALPHATEST], "objectPS_prepass_depthonly_alphatest.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_IMPOSTOR_PREPASS], "impostorPS_prepass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_IMPOSTOR_PREPASS_DEPTHONLY], "impostorPS_prepass_depthonly.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_IMPOSTOR_SIMPLE], "impostorPS_simple.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_LIGHTVISUALIZER], "lightVisualizerPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOLUMETRICLIGHT_DIRECTIONAL], "volumetricLight_DirectionalPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOLUMETRICLIGHT_POINT], "volumetricLight_PointPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOLUMETRICLIGHT_SPOT], "volumetricLight_SpotPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_ENVMAP], "envMapPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_ENVMAP_SKY_STATIC], "envMap_skyPS_static.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_ENVMAP_SKY_DYNAMIC], "envMap_skyPS_dynamic.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_CAPTUREIMPOSTOR], "captureImpostorPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_CUBEMAP], "cubeMapPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VERTEXCOLOR], "vertexcolorPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SKY_STATIC], "skyPS_static.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SKY_DYNAMIC], "skyPS_dynamic.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SUN], "sunPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SHADOW_ALPHATEST], "shadowPS_alphatest.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SHADOW_TRANSPARENT], "shadowPS_transparent.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_SHADOW_WATER], "shadowPS_water.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOXELIZER], "objectPS_voxelizer.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_VOXEL], "voxelPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_FORCEFIELDVISUALIZER], "forceFieldVisualizerPS.cso"); });
	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_RENDERLIGHTMAP], "renderlightmapPS_rtapi.cso", ShaderModel::SM_6_5); });
	}
	else
	{
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_RENDERLIGHTMAP], "renderlightmapPS.cso"); });
	}
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_RAYTRACE_DEBUGBVH], "raytrace_debugbvhPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_DOWNSAMPLEDEPTHBUFFER], "downsampleDepthBuffer4xPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL], "upsample_bilateralPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_POSTPROCESS_OUTLINE], "outlinePS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_LENSFLARE], "lensFlarePS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_DDGI_DEBUG], "ddgi_debugPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_UPSAMPLE], "volumetricCloud_upsamplePS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_COPY_DEPTH], "copyDepthPS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::PS, shaders[PSTYPE_COPY_STENCIL_BIT], "copyStencilBitPS.cso"); });

	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_VOXELIZER], "objectGS_voxelizer.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::GS, shaders[GSTYPE_VOXEL], "voxelGS.cso"); });

	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LUMINANCE_PASS1], "luminancePass1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LUMINANCE_PASS2], "luminancePass2CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SHADINGRATECLASSIFICATION], "shadingRateClassificationCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SHADINGRATECLASSIFICATION_DEBUG], "shadingRateClassificationCS_DEBUG.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_TILEFRUSTUMS], "tileFrustumsCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LIGHTCULLING], "lightCullingCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LIGHTCULLING_DEBUG], "lightCullingCS_DEBUG.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LIGHTCULLING_ADVANCED], "lightCullingCS_ADVANCED.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_LIGHTCULLING_ADVANCED_DEBUG], "lightCullingCS_ADVANCED_DEBUG.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_RESOLVEMSAADEPTHSTENCIL], "resolveMSAADepthStencilCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_OFFSETPREV], "vxgi_offsetprevCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_TEMPORAL], "vxgi_temporalCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_SDF_JUMPFLOOD], "vxgi_sdf_jumpfloodCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_RESOLVE_DIFFUSE], "vxgi_resolve_diffuseCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VXGI_RESOLVE_SPECULAR], "vxgi_resolve_specularCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_TRANSMITTANCELUT], "skyAtmosphere_transmittanceLutCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_MULTISCATTEREDLUMINANCELUT], "skyAtmosphere_multiScatteredLuminanceLutCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_SKYVIEWLUT], "skyAtmosphere_skyViewLutCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_SKYLUMINANCELUT], "skyAtmosphere_skyLuminanceLutCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKYATMOSPHERE_CAMERAVOLUMELUT], "skyAtmosphere_cameraVolumeLutCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAIN2D_UNORM4], "generateMIPChain2DCS_unorm4.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAIN2D_FLOAT4], "generateMIPChain2DCS_float4.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAIN3D_UNORM4], "generateMIPChain3DCS_unorm4.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAIN3D_FLOAT4], "generateMIPChain3DCS_float4.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAINCUBE_UNORM4], "generateMIPChainCubeCS_unorm4.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAINCUBE_FLOAT4], "generateMIPChainCubeCS_float4.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAINCUBEARRAY_UNORM4], "generateMIPChainCubeArrayCS_unorm4.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_GENERATEMIPCHAINCUBEARRAY_FLOAT4], "generateMIPChainCubeArrayCS_float4.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC1], "blockcompressCS_BC1.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC3], "blockcompressCS_BC3.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC4], "blockcompressCS_BC4.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC5], "blockcompressCS_BC5.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC6H], "blockcompressCS_BC6H.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_BLOCKCOMPRESS_BC6H_CUBEMAP], "blockcompressCS_BC6H_cubemap.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_FILTERENVMAP], "filterEnvMapCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_COPYTEXTURE2D_UNORM4], "copytexture2D_unorm4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_COPYTEXTURE2D_FLOAT4], "copytexture2D_float4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_COPYTEXTURE2D_UNORM4_BORDEREXPAND], "copytexture2D_unorm4_borderexpandCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_COPYTEXTURE2D_FLOAT4_BORDEREXPAND], "copytexture2D_float4_borderexpandCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SKINNING], "skinningCS.cso"); });
	
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_PAINT_TEXTURE], "paint_textureCS.cso"); });

	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_FLOAT1], "blur_gaussian_float1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_FLOAT3], "blur_gaussian_float3CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_FLOAT4], "blur_gaussian_float4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_UNORM1], "blur_gaussian_unorm1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_UNORM4], "blur_gaussian_unorm4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_FLOAT1], "blur_gaussian_wide_float1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_FLOAT3], "blur_gaussian_wide_float3CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_FLOAT4], "blur_gaussian_wide_float4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_UNORM1], "blur_gaussian_wide_unorm1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_GAUSSIAN_WIDE_UNORM4], "blur_gaussian_wide_unorm4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_FLOAT1], "blur_bilateral_float1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_FLOAT3], "blur_bilateral_float3CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_FLOAT4], "blur_bilateral_float4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_UNORM1], "blur_bilateral_unorm1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_UNORM4], "blur_bilateral_unorm4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_FLOAT1], "blur_bilateral_wide_float1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_FLOAT3], "blur_bilateral_wide_float3CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_FLOAT4], "blur_bilateral_wide_float4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_UNORM1], "blur_bilateral_wide_unorm1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLUR_BILATERAL_WIDE_UNORM4], "blur_bilateral_wide_unorm4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSAO], "ssaoCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_HBAO], "hbaoCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_PREPAREDEPTHBUFFERS1], "msao_preparedepthbuffers1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_PREPAREDEPTHBUFFERS2], "msao_preparedepthbuffers2CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_INTERLEAVE], "msao_interleaveCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO], "msaoCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_BLURUPSAMPLE], "msao_blurupsampleCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_BLURUPSAMPLE_BLENDOUT], "msao_blurupsampleCS_blendout.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_BLURUPSAMPLE_PREMIN], "msao_blurupsampleCS_premin.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MSAO_BLURUPSAMPLE_PREMIN_BLENDOUT], "msao_blurupsampleCS_premin_blendout.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_TILEMAXROUGHNESS_HORIZONTAL], "ssr_tileMaxRoughness_horizontalCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_TILEMAXROUGHNESS_VERTICAL], "ssr_tileMaxRoughness_verticalCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_DEPTHHIERARCHY], "ssr_depthHierarchyCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_RAYTRACE], "ssr_raytraceCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_RAYTRACE_EARLYEXIT], "ssr_raytraceCS_earlyexit.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_RAYTRACE_CHEAP], "ssr_raytraceCS_cheap.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_RESOLVE], "ssr_resolveCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_TEMPORAL], "ssr_temporalCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SSR_BILATERAL], "ssr_bilateralCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_LIGHTSHAFTS], "lightShaftsCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_TILEMAXCOC_HORIZONTAL], "depthoffield_tileMaxCOC_horizontalCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_TILEMAXCOC_VERTICAL], "depthoffield_tileMaxCOC_verticalCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_NEIGHBORHOODMAXCOC], "depthoffield_neighborhoodMaxCOCCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_PREPASS], "depthoffield_prepassCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_PREPASS_EARLYEXIT], "depthoffield_prepassCS_earlyexit.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_MAIN], "depthoffield_mainCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_MAIN_EARLYEXIT], "depthoffield_mainCS_earlyexit.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_MAIN_CHEAP], "depthoffield_mainCS_cheap.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_POSTFILTER], "depthoffield_postfilterCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DEPTHOFFIELD_UPSAMPLE], "depthoffield_upsampleCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_TILEMAXVELOCITY_HORIZONTAL], "motionblur_tileMaxVelocity_horizontalCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_TILEMAXVELOCITY_VERTICAL], "motionblur_tileMaxVelocity_verticalCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_NEIGHBORHOODMAXVELOCITY], "motionblur_neighborhoodMaxVelocityCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR], "motionblurCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_EARLYEXIT], "motionblurCS_earlyexit.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_MOTIONBLUR_CHEAP], "motionblurCS_cheap.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_BLOOMSEPARATE], "bloomseparateCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_AERIALPERSPECTIVE], "aerialPerspectiveCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_AERIALPERSPECTIVE_CAPTURE], "aerialPerspectiveCS_capture.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_AERIALPERSPECTIVE_CAPTURE_MSAA], "aerialPerspectiveCS_capture_MSAA.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_SHAPENOISE], "volumetricCloud_shapenoiseCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_DETAILNOISE], "volumetricCloud_detailnoiseCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_CURLNOISE], "volumetricCloud_curlnoiseCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_WEATHERMAP], "volumetricCloud_weathermapCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_RENDER], "volumetricCloud_renderCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_RENDER_CAPTURE], "volumetricCloud_renderCS_capture.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_RENDER_CAPTURE_MSAA], "volumetricCloud_renderCS_capture_MSAA.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_REPROJECT], "volumetricCloud_reprojectCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_SHADOW_RENDER], "volumetricCloud_shadow_renderCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_VOLUMETRICCLOUDS_SHADOW_FILTER], "volumetricCloud_shadow_filterCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FXAA], "fxaaCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_TEMPORALAA], "temporalaaCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SHARPEN], "sharpenCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_TONEMAP], "tonemapCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UNDERWATER], "underwaterCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR_UPSCALING], "fsr_upscalingCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR_SHARPEN], "fsr_sharpenCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_AUTOGEN_REACTIVE_PASS], "ffx-fsr2/ffx_fsr2_autogen_reactive_pass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_COMPUTE_LUMINANCE_PYRAMID_PASS], "ffx-fsr2/ffx_fsr2_compute_luminance_pyramid_pass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_PREPARE_INPUT_COLOR_PASS], "ffx-fsr2/ffx_fsr2_prepare_input_color_pass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_RECONSTRUCT_PREVIOUS_DEPTH_PASS], "ffx-fsr2/ffx_fsr2_reconstruct_previous_depth_pass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_DEPTH_CLIP_PASS], "ffx-fsr2/ffx_fsr2_depth_clip_pass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_LOCK_PASS], "ffx-fsr2/ffx_fsr2_lock_pass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_ACCUMULATE_PASS], "ffx-fsr2/ffx_fsr2_accumulate_pass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_FSR2_RCAS_PASS], "ffx-fsr2/ffx_fsr2_rcas_pass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_CHROMATIC_ABERRATION], "chromatic_aberrationCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL_FLOAT1], "upsample_bilateral_float1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL_UNORM1], "upsample_bilateral_unorm1CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL_FLOAT4], "upsample_bilateral_float4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL_UNORM4], "upsample_bilateral_unorm4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_UPSAMPLE_BILATERAL_UINT4], "upsample_bilateral_uint4CS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_DOWNSAMPLE4X], "downsample4xCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_LINEARDEPTH], "lineardepthCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_NORMALSFROMDEPTH], "normalsfromdepthCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_SCREENSPACESHADOW], "screenspaceshadowCS.cso"); });

	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTDIFFUSE], "rtdiffuseCS.cso", ShaderModel::SM_6_5); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTDIFFUSE_SPATIAL], "rtdiffuse_spatialCS.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTDIFFUSE_TEMPORAL], "rtdiffuse_temporalCS.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTDIFFUSE_BILATERAL], "rtdiffuse_bilateralCS.cso"); });

		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTREFLECTION], "rtreflectionCS.cso", ShaderModel::SM_6_5); });

		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTSHADOW], "rtshadowCS.cso", ShaderModel::SM_6_5); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTSHADOW_DENOISE_TILECLASSIFICATION], "rtshadow_denoise_tileclassificationCS.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTSHADOW_DENOISE_FILTER], "rtshadow_denoise_filterCS.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTSHADOW_DENOISE_TEMPORAL], "rtshadow_denoise_temporalCS.cso"); });

		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTAO], "rtaoCS.cso", ShaderModel::SM_6_5); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTAO_DENOISE_TILECLASSIFICATION], "rtao_denoise_tileclassificationCS.cso"); });
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_POSTPROCESS_RTAO_DENOISE_FILTER], "rtao_denoise_filterCS.cso"); });

	}

	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_COVERAGE], "surfel_coverageCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_INDIRECTPREPARE], "surfel_indirectprepareCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_UPDATE], "surfel_updateCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_GRIDOFFSETS], "surfel_gridoffsetsCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_BINNING], "surfel_binningCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_INTEGRATE], "surfel_integrateCS.cso"); });
	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_RAYTRACE], "surfel_raytraceCS_rtapi.cso", ShaderModel::SM_6_5); });
	}
	else
	{
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_RAYTRACE], "surfel_raytraceCS.cso"); });
	}
	...
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VISIBILITY_RESOLVE], "visibility_resolveCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VISIBILITY_RESOLVE_MSAA], "visibility_resolveCS_MSAA.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VISIBILITY_SKY], "visibility_skyCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VISIBILITY_VELOCITY], "visibility_velocityCS.cso"); });

	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_RAYTRACE], "ddgi_raytraceCS_rtapi.cso", ShaderModel::SM_6_5); });
	}
	else
	{
		vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_RAYTRACE], "ddgi_raytraceCS.cso"); });
	}
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_UPDATE], "ddgi_updateCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_UPDATE_DEPTH], "ddgi_updateCS_depth.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_BASECOLORMAP], "terrainVirtualTextureUpdateCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_NORMALMAP], "terrainVirtualTextureUpdateCS_normalmap.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_SURFACEMAP], "terrainVirtualTextureUpdateCS_surfacemap.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_MESHLET_PREPARE], "meshlet_prepareCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_IMPOSTOR_PREPARE], "impostor_prepareCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VIRTUALTEXTURE_TILEREQUESTS], "virtualTextureTileRequestsCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VIRTUALTEXTURE_TILEALLOCATE], "virtualTextureTileAllocateCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_VIRTUALTEXTURE_RESIDENCYUPDATE], "virtualTextureResidencyUpdateCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_WIND], "vzndCS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_YUV_TO_RGB], "yuv_to_rgbCS.cso"); });

	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::HS, shaders[HSTYPE_OBJECT], "objectHS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::HS, shaders[HSTYPE_OBJECT_PREPASS], "objectHS_prepass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::HS, shaders[HSTYPE_OBJECT_PREPASS_ALPHATEST], "objectHS_prepass_alphatest.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::HS, shaders[HSTYPE_OBJECT_SIMPLE], "objectHS_simple.cso"); });

	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::DS, shaders[DSTYPE_OBJECT], "objectDS.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::DS, shaders[DSTYPE_OBJECT_PREPASS], "objectDS_prepass.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::DS, shaders[DSTYPE_OBJECT_PREPASS_ALPHATEST], "objectDS_prepass_alphatest.cso"); });
	vz::jobsystem::Execute(ctx, [](vz::jobsystem::JobArgs args) { LoadShader(ShaderStage::DS, shaders[DSTYPE_OBJECT_SIMPLE], "objectDS_simple.cso"); });

	vz::jobsystem::Dispatch(ctx, MaterialComponent::SHADERTYPE_COUNT, 1, [](vz::jobsystem::JobArgs args) {
	...
	vz::jobsystem::Dispatch(ctx, RENDERPASS_COUNT, 1, [](vz::jobsystem::JobArgs args) {
	...
	vz::jobsystem::Dispatch(ctx, LightComponent::LIGHTTYPE_COUNT, 1, [](vz::jobsystem::JobArgs args) {
		PipelineStateDesc desc;
	...
	vz::jobsystem::Dispatch(ctx, SKYRENDERING_COUNT, 1, [](vz::jobsystem::JobArgs args) {
		PipelineStateDesc desc;
	...
	vz::jobsystem::Dispatch(ctx, DEBUGRENDERING_COUNT, 1, [](vz::jobsystem::JobArgs args) {
		PipelineStateDesc desc;
	...
	for (uint32_t renderPass = 0; renderPass < RENDERPASS_COUNT; ++renderPass)
	{
		...
		vz::jobsystem::Wait(object_pso_job_ctx[renderPass]);
		for (uint32_t shaderType = 0; shaderType < MaterialComponent::SHADERTYPE_COUNT; ++shaderType)
		{
			vz::jobsystem::Execute(object_pso_job_ctx[renderPass], [=](vz::jobsystem::JobArgs args) {
...
const GPUBuffer& GetIndexBufferForQuads(uint32_t max_quad_count)
{
	...
	if (required_max_index < 65536u)
	{
		...
		if (!indexBufferForQuads16.IsValid() || indexBufferForQuads16.desc.size / indexBufferForQuads16.desc.stride < required_index_count)
		{
			...
			device->SetName(&indexBufferForQuads16, "vz::renderer::indexBufferForQuads16bit");
		}
		return indexBufferForQuads16;
	}

	// 32-bit request below:
	max_quad_count = vz::math::GetNextPowerOfTwo(max_quad_count); // reduce allocations by making larger fitting allocations
	const size_t required_index_count = max_quad_count * 6u;
	...
	if (!indexBufferForQuads32.IsValid() || indexBufferForQuads32.desc.size / indexBufferForQuads32.desc.stride < required_index_count)
	{
		...
		device->SetName(&indexBufferForQuads32, "vz::renderer::indexBufferForQuads32bit");
	}
...
void ReloadShaders()
{
	...
	vz::eventhandler::FireEvent(vz::eventhandler::EVENT_RELOAD_SHADERS, 0);
}
...
void Initialize()
{
	...
	static vz::eventhandler::Handle handle2 = vz::eventhandler::Subscribe(vz::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); });
	LoadShaders();

	vz::backlog::post("vz::renderer Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
}
...
inline void CreateDirLightShadowCams(const LightComponent& light, CameraComponent camera, SHCAM* shcams, size_t shcam_count, const vz::rectpacker::Rect& shadow_rect)
{
...
void Workaround(const int bug , CommandList cmd)
{
	if (bug == 1)
	{
		...
		vz::jobsystem::Wait(object_pso_job_ctx[RENDERPASS_VOXELIZE]);
		ObjectRenderingVariant variant = {};
...
void UpdateVisibility(Visibility& vis)
{
	...
	vz::jobsystem::context ctx;
	auto range = vz::profiler::BeginRangeCPU("Frustum Culling");
	...
	if (vis.flags & Visibility::ALLOW_LIGHTS)
	{
		...
		vz::jobsystem::Dispatch(ctx, light_loop, groupSize, [&](vz::jobsystem::JobArgs args) {
	...
	if (vis.flags & Visibility::ALLOW_OBJECTS)
	{
		...
		vz::jobsystem::Dispatch(ctx, object_loop, groupSize, [&](vz::jobsystem::JobArgs args) {
			...
			if ((aabb.layerMask & vis.layerMask) && vis.frustum.CheckBoxFast(aabb))
			{
				...
				if ((vis.flags & Visibility::ALLOW_REQUEST_REFLECTION) && object.IsRequestPlanarReflection() && !occluded)
				{
					...
					float dist = vz::math::DistanceEstimated(vis.camera->Eye, object.center);
					vis.locker.lock();
	...
	if (vis.flags & Visibility::ALLOW_DECALS)
	{
		...
		vz::jobsystem::Dispatch(ctx, decal_loop, groupSize, [&](vz::jobsystem::JobArgs args) {
	...
	if (vis.flags & Visibility::ALLOW_EMITTERS)
	{
		vz::jobsystem::Execute(ctx, [&](vz::jobsystem::JobArgs args) {
			...
			{
				const vz::EmittedParticleSystem& emitter = vis.scene->emitters[i];
	...
	{
		auto range = vz::profiler::BeginRangeCPU("Shadowmap packing");
		...
		while (iterative_scaling > 0.03f)
		{
			...
			for (uint32_t lightIndex : vis.visibleLights)
			{
				...
				const float dist = vz::math::Distance(vis.camera->Eye, light.position);
			...
			if (!vis.shadow_packer.rects.empty())
			{
				if (vis.shadow_packer.pack(8192))
				{
					for (auto& rect : vis.shadow_packer.rects)
					{
						...
						vz::rectpacker::Rect& lightrect = vis.visibleLightShadowRects[lightIndex];
						const LightComponent& light = vis.scene->lights[lightIndex];
						if (rect.was_packed)
						{
							lightrect = rect;
	...
	vz::profiler::EndRange(range); // Frustum Culling
}
...
)
{
	...
	frameCB.texture_random64x64_index = device->GetDescriptorIndex(vz::texturehelper::getRandom64x64(), SubresourceType::SRV);
	frameCB.texture_bluenoise_index = device->GetDescriptorIndex(vz::texturehelper::getBlueNoise(), SubresourceType::SRV);
...
)
{
	...
	auto prof_updatebuffer_cpu = vz::profiler::BeginRangeCPU("Update Buffers (CPU)");
	auto prof_updatebuffer_gpu = vz::profiler::BeginRangeGPU("Update Buffers (GPU)", cmd);
	...
	// Fill Entity Array with decals + envprobes + lights in the frustum:
	{
		...
		for (uint32_t lightIndex : vis.visibleLights)
		{
			...
			const vz::rectpacker::Rect& shadow_rect = vis.visibleLightShadowRects[lightIndex];
	...
	vz::profiler::EndRange(prof_updatebuffer_cpu);
	vz::profiler::EndRange(prof_updatebuffer_gpu);

	BindCommonResources(cmd);

	{
		auto range = vz::profiler::BeginRangeGPU("Wind", cmd);
...

	{
		...
		auto range = vz::profiler::BeginRangeGPU("Skinning and Morph", cmd);
		...
		vz::profiler::EndRange(range); // Skinning and Morph
	...
	if (vis.scene->impostors.GetCount() > 0 && vis.scene->objects.GetCount() > 0 && vis.scene->impostorBuffer.IsValid())
	{
		...
		auto range = vz::profiler::BeginRangeGPU("Impostor prepare", cmd);
	...
	if(vis.scene->instanceArraySize > 0 && vis.scene->meshletBuffer.IsValid())
	{
		...
		auto range = vz::profiler::BeginRangeGPU("Meshlet prepare", cmd);
...
)
{
	...
	{
		auto range = vz::profiler::BeginRangeGPU("EmittedParticles - Simulate", cmd);
		for (uint32_t emitterIndex : vis.visibleEmitters)
		{
			const vz::EmittedParticleSystem& emitter = vis.scene->emitters[emitterIndex];
	...
	if (vis.scene->weather.IsOceanEnabled())
	{
		...
		if (!occluded)
		{
			auto range = vz::profiler::BeginRangeGPU("Ocean - Simulate", cmd);
...
void UpdateRaytracingAccelerationStructures(const Scene& scene, CommandList cmd)
{
	if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
	{
		...
		// BLAS:
		{
			auto rangeCPU = vz::profiler::BeginRangeCPU("BLAS Update (CPU)");
			auto range = vz::profiler::BeginRangeGPU("BLAS Update (GPU)", cmd);
			...
			{
				const vz::EmittedParticleSystem& emitter = scene.emitters[i];
			...
			device->EventEnd(cmd);
			vz::profiler::EndRange(range);
			vz::profiler::EndRange(rangeCPU);
		}

		// TLAS:
		{
			auto rangeCPU = vz::profiler::BeginRangeCPU("TLAS Update (CPU)");
			auto range = vz::profiler::BeginRangeGPU("TLAS Update (GPU)", cmd);
...
void OcclusionCulling_Render(const CameraComponent& camera, const Visibility& vis, CommandList cmd)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Occlusion Culling Render", cmd);
	...
	vz::profiler::EndRange(range); // Occlusion Culling Render
}
...
)
{
	auto range = distortion ?
		vz::profiler::BeginRangeGPU("EmittedParticles - Render (Distortion)", cmd) :
		vz::profiler::BeginRangeGPU("EmittedParticles - Render", cmd);
	...
	if (emitterCount > 0)
	{
		...
		static thread_local vz::vector<uint32_t> emitterSortingHashes;
		emitterSortingHashes.resize(emitterCount);
		for (size_t i = 0; i < emitterCount; ++i)
		{
			const uint32_t emitterIndex = vis.visibleEmitters[i];
			const vz::EmittedParticleSystem& emitter = vis.scene->emitters[emitterIndex];
			float distance = vz::math::DistanceEstimated(emitter.center, vis.camera->Eye);
		...
		for (size_t i = 0; i < emitterCount; ++i)
		{
			...
			if (distortion && emitter.shaderType == vz::EmittedParticleSystem::SOFT_DISTORTION)
			{
				emitter.Draw(material, cmd);
			}
			else if (!distortion && (emitter.shaderType == vz::EmittedParticleSystem::SOFT || emitter.shaderType == vz::EmittedParticleSystem::SOFT_LIGHTING || emitter.shaderType == vz::EmittedParticleSystem::SIMPLE || IsWireRender()))
			{
				emitter.Draw(material, cmd);
			}
		}
...
)
{
	...
	static thread_local vz::vector<uint64_t> distance_sorter;
	distance_sorter.clear();
	for (size_t i = 0; i < scene.sprites.GetCount(); ++i)
	{
		const vz::Sprite& sprite = scene.sprites[i];
		if (sprite.IsHidden())
			continue;
		...
		{
			sorter.bits.distance = XMConvertFloatToHalf(vz::math::DistanceEstimated(transform->GetPosition(), camera.Eye));
		}
	...
	if (!distortion)
	{
		...
		{
			const vz::SpriteFont& font = scene.fonts[i];
			if (font.IsHidden())
				continue;
			DistanceSorter sorter = {};
			sorter.bits.id = uint32_t(i);
	...
	for (auto& x : distance_sorter)
	{
		...
		case SPRITE:
		{
			const vz::Sprite& sprite = scene.sprites[sorter.bits.id];
			vz::image::Params params = sprite.params;
			...
			if (transform != nullptr)
			{
				...
				{
					float scale = 0.05f * vz::math::Distance(transform->GetPosition(), camera.Eye);
			...
			vz::image::Draw(sprite.GetTexture(), params, cmd);
		}
		break;
		case FONT:
		{
			const vz::SpriteFont& font = scene.fonts[sorter.bits.id];
			vz::font::Params params = font.params;
			...
			params.customProjection = &M;
			vz::font::Draw(font.GetText().c_str(), font.GetCurrentTextLength(), params, cmd);
		}
		break;
		}
	}
		...
		for (int type = LightComponent::POINT; type < LightComponent::LIGHTTYPE_COUNT; ++type)
		{
			...
			for (uint32_t lightIndex : vis.visibleLights)
			{
				...
				if (light.GetType() == type && light.IsVisualizerEnabled())
				{
					...
					{
						const float sphere_volume = std::max(1.0f, vz::math::SphereVolume(light.radius));
	...
	for (uint32_t lightIndex : vis.visibleLights)
	{
		...
		if (!light.lensFlareRimTextures.empty())
		{
			...
			if (light.GetType() == LightComponent::DIRECTIONAL)
			{
				...
				{
					device->BindResource(vz::texturehelper::getWhite(), 0, cmd);
				}
				else
				{
...
)
{
	...
	if ((!vis.visibleLights.empty() || vis.scene->weather.rain_amount > 0) && shadowMapAtlas.IsValid())
	{
		...
		auto range_cpu = vz::profiler::BeginRangeCPU("Shadowmap Rendering");
		auto range_gpu = vz::profiler::BeginRangeGPU("Shadowmap Rendering", cmd);
		...
		vz::profiler::EndRange(range_gpu);
		vz::profiler::EndRange(range_cpu);
		device->EventEnd(cmd);
	}
}
...
)
{
	...
	for (uint32_t instanceIndex : vis.visibleObjects)
	{
		...
		const float distance = vz::math::Distance(vis.camera->Eye, object.center);
...
)
{
	...
	if (!debugTextStorage.empty())
	{
		...
		static thread_local vz::vector<DebugTextSorter> sorted_texts;
		sorted_texts.clear();
		size_t offset = 0;
		while(offset < debugTextStorage.size())
		{
			...
			offset += x.text_len + 1;
			x.distance = vz::math::Distance(x.params.position, camera.Eye);
		...
		for (auto& x : sorted_texts)
		{
			...
			params.color = vz::Color::fromFloat4(x.params.color);
			params.intensity = x.params.color.w > 1 ? x.params.color.w : 1;
			params.h_align = vz::font::WIFALIGN_CENTER;
			params.v_align = vz::font::WIFALIGN_CENTER;
			params.softness = 0.0f;
			params.shadowColor = vz::Color::Black();
			...
			vz::font::Draw(x.text, x.text_len, params, cmd);
		}
	...
	const Texture* weatherMapSecond)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Volumetric Clouds Shadow", cmd);
...
void ComputeSkyAtmosphereTextures(CommandList cmd)
{
	...
	auto range = vz::profiler::BeginRangeGPU("SkyAtmosphere Textures", cmd);
...
void RefreshEnvProbes(const Visibility& vis, CommandList cmd)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Environment Probe Refresh", cmd);
	...
	auto render_probe = [&](const EnvironmentProbeComponent& probe, const AABB& probe_aabb) {
		...
		static vz::unordered_map<uint32_t, Texture> render_textures;
		static std::mutex locker;
		{
			...
			if (!envrenderingDepthBuffer.IsValid())
			{
				desc.mip_levels = 1;
				desc.bind_flags = BindFlag::DEPTH_STENCIL | BindFlag::SHADER_RESOURCE;
				desc.format = vz::renderer::format_depthbuffer_envprobe;
				desc.layout = ResourceState::SHADER_RESOURCE;
				...
				info += "\n\tMemory = " + vz::helper::GetMemorySizeText(ComputeTextureMemorySizeInBytes(desc)) + "\n";
				vz::backlog::post(info);
			}
			...
			if (!envrenderingColorBuffer.IsValid())
			{
				...
				desc.format = vz::renderer::format_rendertarget_envprobe;
				desc.layout = ResourceState::SHADER_RESOURCE;
				desc.misc_flags = ResourceMiscFlag::TEXTURECUBE;
				desc.sample_count = 1;
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("VXGI - Voxelize", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("VXGI - Resolve", cmd);
...
{
	auto range = vz::profiler::BeginRangeGPU("Entity Culling", cmd);
...
void BlockCompress(const Texture& texture_src, const Texture& texture_bc, CommandList cmd, uint32_t dst_slice_offset)
{
	...
	Texture bc_raw_dest;
	{
		...
		if (!bc_raw->IsValid() || bc_raw->desc.width < desc.width || bc_raw->desc.height < desc.height || bc_raw->desc.array_size < desc.array_size)
		{
			...
			bc_raw_desc.width = vz::math::GetNextPowerOfTwo(bc_raw_desc.width);
			bc_raw_desc.height = vz::math::GetNextPowerOfTwo(bc_raw_desc.height);
			...
			info += "\n\tMemory = " + vz::helper::GetMemorySizeText(total_size) + "\n";
			vz::backlog::post(info);
		}
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("RayTraceScene", cmd);

	const TextureDesc& desc = output.GetDesc();

	BindCommonResources(cmd);

	const XMFLOAT4& halton = vz::math::GetHaltonSequence(accumulation_sample);
	RaytracingCB cb = {};
...
void RefreshLightmaps(const Scene& scene, CommandList cmd)
{
	...
	{
		auto range = vz::profiler::BeginRangeGPU("Lightmap Processing", cmd);
		...
		for (uint32_t requestIndex = 0; requestIndex < lightmap_request_count; ++requestIndex)
		{
			...
			if (object.IsLightmapRenderRequested())
			{
				...
				cb.xTraceResolution_rcp.x = 1.0f / cb.xTraceResolution.x;
				cb.xTraceResolution_rcp.y = 1.0f / cb.xTraceResolution.y;
				XMFLOAT4 halton = vz::math::GetHaltonSequence(object.lightmapIterationCount); // for jittering the rasterization (good for eliminating atlas border artifacts)
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Luminance", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Bloom", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("ComputeShadingRateClassification", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Visibility_Prepare", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Visibility_Surface", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Visibility_Surface_Reduced", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Visibility_Shade", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Visibility_Velocity", cmd);
...
)
{
	...
	auto prof_range = vz::profiler::BeginRangeGPU("SurfelGI - Coverage", cmd);
	...
	vz::profiler::EndRange(prof_range);
	device->EventEnd(cmd);
}
void SurfelGI(
	const SurfelGIResources& res,
...
)
{
	...
	auto prof_range = vz::profiler::BeginRangeGPU("SurfelGI", cmd);
...
)
{
	...
	auto prof_range = vz::profiler::BeginRangeGPU("DDGI", cmd);
	...
	// Raytracing:
	{
		...
		MiscCB cb = {};
		float angle = vz::random::GetRandom(0.0f, 1.0f) * XM_2PI;
		XMVECTOR axis = XMVectorSet(
			vz::random::GetRandom(-1.0f, 1.0f),
			vz::random::GetRandom(-1.0f, 1.0f),
			vz::random::GetRandom(-1.0f, 1.0f),
			0
		);
...
)
{
	...
	auto prof_range = vz::profiler::BeginRangeGPU("SSAO", cmd);
...
)
{
	...
	auto prof_range = vz::profiler::BeginRangeGPU("HBAO", cmd);
	...
	)
{
	...
	auto prof_range = vz::profiler::BeginRangeGPU("MSAO", cmd);
...
)
{
	...
	auto prof_range = vz::profiler::BeginRangeGPU("RTAO", cmd);
...
)
{
	...
	auto profilerRange = vz::profiler::BeginRangeGPU("RTDiffuse", cmd);
	...
	vz::profiler::EndRange(profilerRange);
	device->EventEnd(cmd);
}
...
)
{
	...
	auto profilerRange = vz::profiler::BeginRangeGPU("RTReflection", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("SSR", cmd);
...
)
{
	...
	auto prof_range = vz::profiler::BeginRangeGPU("RTShadow", cmd);
...
)
{
	...
	auto prof_range = vz::profiler::BeginRangeGPU("ScreenSpaceShadow", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("LightShafts", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Depth of Field", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Outline", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("MotionBlur", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Aerial Perspective", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Volumetric Clouds", cmd);
	...
	if (jitterEnabled)
	{
		...
		vz::renderer::BindCameraCB(camera_clouds, camera_previous_clouds, camera_reflection, cmd);
	}
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Volumetric Clouds Upsample", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("FXAA", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Temporal AA Resolve", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Postprocess_FSR", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Postprocess_FSR2", cmd);
	...
	fsr2_constants.deltaTime = vz::math::saturate(dt);
	...
	if (resetAccumulation)
	{
		...
		uint32_t clear_lock_pk = vz::math::Pack_R11G11B10_FLOAT(XMFLOAT3(clearValuesLockStatus[0], clearValuesLockStatus[1], clearValuesLockStatus[2]));
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU("Underwater", cmd);
...
)
{
	...
	auto range = vz::profiler::BeginRangeGPU(debug_name, cmd);
...
Ray GetPickRay(long cursorX, long cursorY, const vz::Canvas& canvas, const CameraComponent& camera)
{
