...
namespace vz::audio
{
	...
	struct AudioInternal
	{
		...
		AudioInternal()
		{
			...
			{
				vz::backlog::post("Failed to create XAudio2 mastering voice!");
				return;
			}
			...
			// Reverb setup:
			{
				...
				{
					vz::backlog::post("vz::audio [XAudio2] Reverb Submix was not created successfully!", vz::backlog::LogLevel::Warning);
				}
			}
			...
			if (SUCCEEDED(hr))
			{
				vz::backlog::post("vz::audio Initialized [XAudio2] (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
			}
		}
		~AudioInternal()
		{
	...
	struct SoundInstanceInternal : public IXAudio2VoiceCallback
	{
		...
		vz::vector<float> outputMatrix;
		vz::vector<float> channelAzimuths;
	...
	bool CreateSound(const std::string& filename, Sound* sound)
	{
		...
		bool success = vz::helper::FileRead(filename, filedata);
		if (!success)
		{
			return false;
		}
...
namespace vz::audio
{
	...
	struct AudioInternal{
		...
		AudioInternal(){
			...
			if (success)
			{
				vz::backlog::post("vz::audio Initialized [FAudio] (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
			}
		}
		~AudioInternal(){
			if(reverbSubmix != nullptr)
