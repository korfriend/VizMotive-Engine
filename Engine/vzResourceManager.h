...
namespace vz
{
	...
	struct Resource
	{
		...
		const vz::vector<uint8_t>& GetFileData() const;
		const vz::graphics::Texture& GetTexture() const;
		const vz::audio::Sound& GetSound() const;
		const std::string& GetScript() const;
		const vz::video::Video& GetVideo() const;
		int GetTextureSRGBSubresource() const;
		int GetFontStyle() const;

		void SetFileData(const vz::vector<uint8_t>& data);
		void SetFileData(vz::vector<uint8_t>&& data);
		...
		void SetSound(const vz::audio::Sound& sound);
		void SetScript(const std::string& script);
		void SetVideo(const vz::video::Video& script);
