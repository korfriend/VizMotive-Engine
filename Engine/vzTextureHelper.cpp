...
namespace vz::texturehelper
{
	...
	vz::unordered_map<unsigned long, vz::graphics::Texture> colorTextures;
	...
	void Initialize()
	{
		...
		// Blue Noise:
		{
			vz::vector<vz::Color> bluenoise(128 * 128);
		...
		// Water ripple:
		{
			...
			vz::vector<SubresourceData> initdata(desc.mip_levels);
		...
		case GradientType::Linear:
		{
			...
			for (uint32_t y = 0; y < height; ++y)
			{
				for (uint32_t x = 0; x < width; ++x)
				{
					...
					float gradient = vz::math::saturate(vz::math::InverseLerp(0, distance, uv_distance));
					...
					gradient = vz::math::saturate(gradient);
					if (has_flag(flags, GradientFlags::R16Unorm))
					{
	...
	{
		vz::vector<uint8_t> data(width * height);
		for (uint32_t y = 0; y < height; ++y)
		{
			for (uint32_t x = 0; x < width; ++x)
			{
