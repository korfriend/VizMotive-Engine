...
#pragma comment(lib,"dxgi.lib")
	...
	struct RootSignatureOptimizer
	{
		...
		vz::vector<RootParameterStatistics> root_stats;
	...
	struct Resource_DX12
	{
		...
		vz::vector<SingleDescriptor> subresources_srv;
		vz::vector<SingleDescriptor> subresources_uav;
		SingleDescriptor uav_raw;

		D3D12_GPU_VIRTUAL_ADDRESS gpu_address = 0;

		UINT64 total_size = 0;
		vz::vector<D3D12_PLACED_SUBRESOURCE_FOOTPRINT> footprints;
		vz::vector<UINT64> rowSizesInBytes;
		vz::vector<UINT> numRows;
	...
	struct Texture_DX12 : public Resource_DX12
	{
		...
		vz::vector<SingleDescriptor> subresources_rtv;
		vz::vector<SingleDescriptor> subresources_dsv;
	...
	struct PipelineState_DX12
	{
		...
		vz::vector<uint8_t> shadercode;
		vz::vector<D3D12_INPUT_ELEMENT_DESC> input_elements;
	...
	struct BVH_DX12 : public Resource_DX12
	{
		...
		vz::vector<D3D12_RAYTRACING_GEOMETRY_DESC> geometries;
	...
	struct RTPipelineState_DX12
	{
		...
		vz::vector<std::wstring> export_strings;
		vz::vector<D3D12_EXPORT_DESC> exports;
		vz::vector<D3D12_DXIL_LIBRARY_DESC> library_descs;
		vz::vector<std::wstring> group_strings;
		vz::vector<D3D12_HIT_GROUP_DESC> hitgroup_descs;
	...
	struct SwapChain_DX12
	{
		...
		vz::vector<ComPtr<ID3D12Resource>> backBuffers;
		vz::vector<D3D12_CPU_DESCRIPTOR_HANDLE> backbufferRTV;
...
using namespace dx12_internal;
	...
	bool GraphicsDevice_DX12::CreateRaytracingPipelineState(const RaytracingPipelineStateDesc* desc, RaytracingPipelineState* rtpso) const
	{
		...
		vz::vector<D3D12_STATE_SUBOBJECT> subobjects;
	...
	bool GraphicsDevice_DX12::CreateVideoDecoder(const VideoDesc* desc, VideoDecoder* video_decoder) const
	{
		...
		vz::vector<GUID> profiles(video_decode_profile_count.ProfileCount);
		...
		vz::vector<DXGI_FORMAT> formats(video_decode_format_count.FormatCount);
	...
	void GraphicsDevice_DX12::SparseUpdate(QUEUE_TYPE queue, const SparseUpdateCommand* commands, uint32_t command_count)
	{
		...
		thread_local vz::vector<D3D12_TILED_RESOURCE_COORDINATE> tiled_resource_coordinates;
		thread_local vz::vector<D3D12_TILE_REGION_SIZE> tiled_region_sizes;
		thread_local vz::vector<D3D12_TILE_RANGE_FLAGS> tile_range_flags;
		thread_local vz::vector<uint32_t> range_start_offsets;
