...
namespace vz
{
	...
	// this should always be only INCREMENTED and only if a new serialization is implemented somewhere!
		...
		if (!fileName.empty())
		{
			...
			if (readMode)
			{
				if (vz::helper::FileRead(fileName, DATA))
				{
					...
					{
						vz::helper::messageBox("The archive version (" + std::to_string(version) + ") is no longer supported!", "Error!");
						Close();
					}
					if (version > __archiveVersion)
					{
						vz::helper::messageBox("The archive version (" + std::to_string(version) + ") is higher than the program's (" + std::to_string(__archiveVersion) + ")!", "Error!");
						Close();
					}
				}
			}
