...
namespace vz
{
	struct VoxelGrid
	{
		...
		vz::vector<uint64_t> voxels; // 1 array element stores 4 * 4 * 4 = 64 voxels
		...
		void inject_aabb(const vz::primitive::AABB& aabb, bool subtract = false);
		void inject_sphere(const vz::primitive::Sphere& sphere, bool subtract = false);
		void inject_capsule(const vz::primitive::Capsule& capsule, bool subtract = false);
		...
		vz::primitive::AABB get_aabb() const;
		void from_aabb(const vz::primitive::AABB& aabb);
		...
		bool is_visible(const XMFLOAT3& observer, const vz::primitive::AABB& subject) const;
...
//#define DEBUG_VOXEL_OCCLUSION
		...
		mutable vz::vector<XMUINT3> debug_subject_coords;
		mutable vz::vector<XMUINT3> debug_visible_coords;
		mutable vz::vector<XMUINT3> debug_occluded_coords;
