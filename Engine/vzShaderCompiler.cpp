...
#if __has_include("vzShaderCompiler_XBOX.h")
#include "vzShaderCompiler_XBOX.h"
#define SHADERCOMPILER_XBOX_INCLUDED
#endif // __has_include("vzShaderCompiler_XBOX.h")

#if __has_include("vzShaderCompiler_PS5.h")
#include "vzShaderCompiler_PS5.h"
#define SHADERCOMPILER_PS5_INCLUDED
#endif // __has_include("vzShaderCompiler_PS5.h") 
...
namespace vz::shadercompiler
{
		...
		InternalState_DXC(const std::string& modifier = "")
		{
			...
			if (dxcompiler != nullptr)
			{
				...
				if (DxcCreateInstance != nullptr)
				{
					...
					assert(SUCCEEDED(hr));
					vz::backlog::post("vz::shadercompiler: loaded " + library + " (version: " + std::to_string(major) + "." + std::to_string(minor) + ")");
				}
			}
			else
			{
				vz::backlog::post("vz::shadercompiler: could not load library " + library, vz::backlog::LogLevel::Error);
#ifdef PLATFORM_LINUX
				vz::backlog::post(dlerror(), vz::backlog::LogLevel::Error); // print dlopen() error detail: https://linux.die.net/man/3/dlerror
#endif // PLATFORM_LINUX
			}
	...
	void Compile_DXCompiler(const CompilerInput& input, CompilerOutput& output)
	{
		...
		vz::vector<uint8_t> shadersourcedata;
		...
		{
			vz::shadercompiler::xbox::AddArguments(input, args);
		}
		...
		vz::vector<const wchar_t*> args_raw;
	...
	void Compile_D3DCompiler(const CompilerInput& input, CompilerOutput& output)
	{
		...
		struct IncludeHandler : public ID3DInclude
		{
			const CompilerInput* input = nullptr;
			CompilerOutput* output = nullptr;
			vz::vector<vz::vector<uint8_t>> filedatas;
			...
			HRESULT Open(D3D_INCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID* ppData, UINT* pBytes) override
			{
				for (auto& x : input->include_directories)
				{
					...
					if (!vz::helper::FileExists(filename))
						continue;
					vz::vector<uint8_t>& filedata = filedatas.emplace_back();
	...
	void Compile(const CompilerInput& input, CompilerOutput& output)
	{
		...
		case ShaderFormat::PS5:
			vz::shadercompiler::ps5::Compile(input, output);
			break;
	...
	constexpr const char* shadermetaextension = "vzshadermeta";
	bool SaveShaderAndMetadata(const std::string& shaderfilename, const CompilerOutput& output)
	{
		...
		vz::Archive dependencyLibrary(vz::helper::ReplaceExtension(shaderfilename, shadermetaextension), false);
		if (dependencyLibrary.IsOpen())
		{
			std::string rootdir = dependencyLibrary.GetSourceDirectory();
			vz::vector<std::string> dependencies = output.dependencies;
	...
	bool IsShaderOutdated(const std::string& shaderfilename)
	{
		...
		vz::Archive dependencyLibrary(dependencylibrarypath);
		if (dependencyLibrary.IsOpen())
		{
			std::string rootdir = dependencyLibrary.GetSourceDirectory();
	...
	std::mutex locker;
	vz::unordered_set<std::string> registered_shaders;
