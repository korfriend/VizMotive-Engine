...
namespace vz
{
	...
	void SpriteFont::Update(float dt)
	{
		...
		if (anim.typewriter.time > 0)
		{
			...
			size_t text_length_prev = std::min(text_length, size_t(vz::math::Lerp(float(std::min(text_length, anim.typewriter.character_start)), float(text_length + 1), anim.typewriter.elapsed / anim.typewriter.time)));
			anim.typewriter.elapsed += dt;
			size_t text_length_next = std::min(text_length, size_t(vz::math::Lerp(float(std::min(text_length, anim.typewriter.character_start)), float(text_length + 1), anim.typewriter.elapsed / anim.typewriter.time)));

			if (anim.typewriter.soundinstance.IsValid())
			{
				if (!anim.typewriter.IsFinished() && vz::audio::IsEnded(&anim.typewriter.soundinstance) && text_length_prev != text_length_next && text[text_length_next - 1] != ' ' && anim.typewriter.soundinstance.IsValid())
				{
					vz::audio::Stop(&anim.typewriter.soundinstance);
					if (!IsHidden())
					{
						vz::audio::Play(&anim.typewriter.soundinstance);
					}
				}
				vz::audio::ExitLoop(&anim.typewriter.soundinstance);
			}
	...
	void SpriteFont::Draw(CommandList cmd) const
	{
		...
		vz::font::Draw(text.c_str(), GetCurrentTextLength(), params, cmd);
	}

	XMFLOAT2 SpriteFont::TextSize() const
	{
		return vz::font::TextSize(text, params);
	}
	float SpriteFont::TextWidth() const
	{
		return vz::font::TextWidth(text, params);
	}
	float SpriteFont::TextHeight() const
	{
		return vz::font::TextHeight(text, params);
	}

	void SpriteFont::SetText(const std::string& value)
	{
		vz::helper::StringConvert(value, text);
	}
	...
	{
		std::string retVal;
		vz::helper::StringConvert(text, retVal);
		return retVal;
	}
	...
	size_t SpriteFont::GetCurrentTextLength() const
	{
		...
		if (anim.typewriter.time > 0)
		{
			text_length = std::min(text_length, size_t(vz::math::Lerp(float(std::min(text_length, anim.typewriter.character_start)), float(text_length + 1), anim.typewriter.elapsed / anim.typewriter.time)));
		}
		return text_length;
	}

	void SpriteFont::Serialize(vz::Archive& archive, vz::ecs::EntitySerializer& seri)
	{
			...
			if (!fontStyleName.empty())
			{
				vz::jobsystem::Execute(seri.ctx, [&](vz::jobsystem::JobArgs args) {
					if (!fontStyleName.empty())
					{
						...
						fontStyleResource = vz::resourcemanager::Load(fontStyleName, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
						params.style = fontStyleResource.GetFontStyle();
					}
				});
			}
		...
		else
		{
			...
			archive << textA;
			archive << vz::helper::GetPathRelative(dir, fontStyleName);
