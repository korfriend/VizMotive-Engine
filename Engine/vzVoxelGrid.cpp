...
namespace vz
{
	...
	void VoxelGrid::inject_aabb(const vz::primitive::AABB& aabb, bool subtract)
	{
		const XMVECTOR CENTER = XMLoadFloat3(&center);
		const XMVECTOR RESOLUTION = XMLoadUInt3(&resolution);
		const XMVECTOR RESOLUTION_RCP = XMLoadFloat3(&resolution_rcp);
	...
	void VoxelGrid::inject_sphere(const vz::primitive::Sphere& sphere, bool subtract)
	{
		const XMVECTOR CENTER = XMLoadFloat3(&center);
		const XMVECTOR RESOLUTION = XMLoadUInt3(&resolution);
		const XMVECTOR RESOLUTION_RCP = XMLoadFloat3(&resolution_rcp);
	...
	void VoxelGrid::inject_capsule(const vz::primitive::Capsule& capsule, bool subtract)
	{
		const XMVECTOR CENTER = XMLoadFloat3(&center);
		const XMVECTOR RESOLUTION = XMLoadUInt3(&resolution);
		const XMVECTOR RESOLUTION_RCP = XMLoadFloat3(&resolution_rcp);
	...
	void VoxelGrid::Serialize(vz::Archive& archive, vz::ecs::EntitySerializer& seri)
	{
	...
	void VoxelGrid::debugdraw(const XMFLOAT4X4& ViewProjection, CommandList cmd) const
	{
		...
		vz::renderer::DrawBox(boxmat, debug_color_extent);
