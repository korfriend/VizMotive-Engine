...
namespace vz
{
	...
	void HairParticleSystem::CreateFromMesh(const vz::scene::MeshComponent& mesh)
	{
	...
	void HairParticleSystem::CreateRenderData()
	{
		...
		if (particleCount > 0)
		{
			...
			primitiveBuffer = vz::renderer::GetIndexBufferForQuads(particleCount);
		}
		...
		if (!vertex_lengths.empty())
		{
			auto pack_lengths = [&](void* dest) {
				...
				{
					uint8_t packed = uint8_t(vz::math::Clamp(vertex_lengths[i], 0, 1) * 255.0f);
	...
	void HairParticleSystem::InitializeGPUDataIfNeeded(vz::graphics::CommandList cmd)
	{
	...
	void HairParticleSystem::Draw(const MaterialComponent& material, vz::enums::RENDERPASS renderPass, CommandList cmd) const
	{
	...
	void HairParticleSystem::Serialize(vz::Archive& archive, vz::ecs::EntitySerializer& seri)
	{
	...
	namespace HairParticleSystem_Internal
	{
		void LoadShaders()
		{
			vz::renderer::LoadShader(ShaderStage::VS, vs, "hairparticleVS.cso");

			vz::renderer::LoadShader(ShaderStage::PS, ps_simple, "hairparticlePS_simple.cso");
			vz::renderer::LoadShader(ShaderStage::PS, ps_prepass, "hairparticlePS_prepass.cso");
			vz::renderer::LoadShader(ShaderStage::PS, ps_prepass_depthonly, "hairparticlePS_prepass_depthonly.cso");
			vz::renderer::LoadShader(ShaderStage::PS, ps_shadow, "hairparticlePS_shadow.cso");
			vz::renderer::LoadShader(ShaderStage::PS, ps, "hairparticlePS.cso");

			vz::renderer::LoadShader(ShaderStage::CS, cs_simulate, "hairparticle_simulateCS.cso");
	...
	void HairParticleSystem::Initialize()
	{
		...
		static vz::eventhandler::Handle handle = vz::eventhandler::Subscribe(vz::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { HairParticleSystem_Internal::LoadShaders(); });
		HairParticleSystem_Internal::LoadShaders();

		vz::backlog::post("vz::HairParticleSystem Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
}
