...
namespace vz
{
	...
	void EmittedParticleSystem::CreateSelfBuffers()
	{
		...
		if (particleBuffer.desc.size < MAX_PARTICLES * sizeof(Particle))
		{
			...
			primitiveBuffer = vz::renderer::GetIndexBufferForQuads(MAX_PARTICLES);
		}
	...
	void EmittedParticleSystem::UpdateCPU(const TransformComponent& transform, float dt)
	{
		...
		const uint32_t oldest_stat_index = vz::graphics::GetDevice()->GetBufferIndex();
	...
	void EmittedParticleSystem::UpdateGPU(uint32_t instanceIndex, const MeshComponent* mesh, CommandList cmd) const
	{
		...
		if (!IsPaused() && dt > 0)
		{
			...
			cb.xEmitterRandomness = vz::random::GetRandom(0.0f, 1.0f);
			...
			if (IsSPHEnabled())
			{
				auto range = vz::profiler::BeginRangeGPU("SPH - Simulation", cmd);
		...
		if (IsSorted())
		{
			vz::gpusortlib::Sort(MAX_PARTICLES, distanceBuffer, counterBuffer, PARTICLECOUNTER_OFFSET_CULLEDCOUNT, culledIndirectionBuffer, cmd);
		}
	...
	void EmittedParticleSystem::Draw(const MaterialComponent& material, CommandList cmd) const
	{
		...
		if (vz::renderer::IsWireRender())
		{
			device->BindPipelineState(&PSO_wire, cmd);
		}
		else
		{
			const vz::enums::BLENDMODE blendMode = material.GetBlendMode();
	...
	namespace EmittedParticleSystem_Internal
	{
		void LoadShaders()
		{
			vz::renderer::LoadShader(ShaderStage::VS, vertexShader, "emittedparticleVS.cso");

			if (ALLOW_MESH_SHADER && vz::graphics::GetDevice()->CheckCapability(GraphicsDeviceCapability::MESH_SHADER))
			{
				vz::renderer::LoadShader(ShaderStage::MS, meshShader, "emittedparticleMS.cso");
			}

			vz::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SOFT], "emittedparticlePS_soft.cso");
			vz::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SOFT_DISTORTION], "emittedparticlePS_soft_distortion.cso");
			vz::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SIMPLE], "emittedparticlePS_simple.cso");
			vz::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SOFT_LIGHTING], "emittedparticlePS_soft_lighting.cso");

			vz::renderer::LoadShader(ShaderStage::CS, kickoffUpdateCS, "emittedparticle_kickoffUpdateCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, finishUpdateCS, "emittedparticle_finishUpdateCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, emitCS, "emittedparticle_emitCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, emitCS_VOLUME, "emittedparticle_emitCS_volume.cso");
			vz::renderer::LoadShader(ShaderStage::CS, emitCS_FROMMESH, "emittedparticle_emitCS_FROMMESH.cso");
			vz::renderer::LoadShader(ShaderStage::CS, sphpartitionCS, "emittedparticle_sphpartitionCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, sphcellallocationCS, "emittedparticle_sphcellallocationCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, sphbinningCS, "emittedparticle_sphbinningCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, sphdensityCS, "emittedparticle_sphdensityCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, sphforceCS, "emittedparticle_sphforceCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, simulateCS, "emittedparticle_simulateCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, simulateCS_SORTING, "emittedparticle_simulateCS_SORTING.cso");
			vz::renderer::LoadShader(ShaderStage::CS, simulateCS_DEPTHCOLLISIONS, "emittedparticle_simulateCS_DEPTHCOLLISIONS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, simulateCS_SORTING_DEPTHCOLLISIONS, "emittedparticle_simulateCS_SORTING_DEPTHCOLLISIONS.cso");
	...
	void EmittedParticleSystem::Initialize()
	{
		...
		static vz::eventhandler::Handle handle = vz::eventhandler::Subscribe(vz::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { EmittedParticleSystem_Internal::LoadShaders(); });
		EmittedParticleSystem_Internal::LoadShaders();

		vz::backlog::post("vz::EmittedParticleSystem Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}


	void EmittedParticleSystem::Serialize(vz::Archive& archive, vz::ecs::EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
			archive >> (uint32_t&)shaderType;
			vz::ecs::SerializeEntity(archive, meshID, seri);
