...
namespace vz::scene
{
	...
	void MaterialComponent::WriteShaderMaterial(ShaderMaterial* dest) const
	{
		...
		material.emissive_r11g11b10 = vz::math::Pack_R11G11B10_FLOAT(XMFLOAT3(emissiveColor.x * emissiveColor.w, emissiveColor.y * emissiveColor.w, emissiveColor.z * emissiveColor.w));
		material.specular_r11g11b10 = vz::math::Pack_R11G11B10_FLOAT(XMFLOAT3(specularColor.x * specularColor.w, specularColor.y * specularColor.w, specularColor.z * specularColor.w));
		...
		material.sheenColor_r11g11b10 = vz::math::Pack_R11G11B10_FLOAT(XMFLOAT3(sheenColor.x, sheenColor.y, sheenColor.z));
		...
		{
			material.anisotropy_strength = vz::math::Clamp(anisotropy_strength, 0, 0.99f);
		...
		material.stencilRef = vz::renderer::CombineStencilrefs(engineStencilRef, userStencilRef);
		...
		{
			material.sampler_descriptor = device->GetDescriptorIndex(vz::renderer::GetSampler(vz::enums::SAMPLER_OBJECTSHADER));
		}
		else
		{
	...
	void MaterialComponent::WriteTextures(const vz::graphics::GPUResource** dest, int count) const
	{
	...
	{
		if (IsCustomShader() && customShaderID < (int)vz::renderer::GetCustomShaders().size())
		{
			auto& customShader = vz::renderer::GetCustomShaders()[customShaderID];
	...
	vz::resourcemanager::Flags MaterialComponent::GetTextureSlotResourceFlags(TEXTURESLOT slot)
	{
		vz::resourcemanager::Flags flags = vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA;
		if (!IsPreferUncompressedTexturesEnabled())
		{
			flags |= vz::resourcemanager::Flags::IMPORT_BLOCK_COMPRESSED;
		}
		switch (slot)
		{
		case NORMALMAP:
		case CLEARCOATNORMALMAP:
			flags |= vz::resourcemanager::Flags::IMPORT_NORMALMAP;
			break;
		default:
			break;
		}
	...
	void MaterialComponent::CreateRenderData(bool force_recreate)
	{
		...
		for (uint32_t slot = 0; slot < TEXTURESLOT_COUNT; ++slot)
		{
			...
			{
				vz::resourcemanager::Flags flags = GetTextureSlotResourceFlags(TEXTURESLOT(slot));
				textureslot.resource = vz::resourcemanager::Load(textureslot.name, flags);
			}
		}
	}
	uint32_t MaterialComponent::GetStencilRef() const
	{
		return vz::renderer::CombineStencilrefs(engineStencilRef, userStencilRef);
	}
	...
	void MeshComponent::CreateRenderData()
	{
		...
		for (size_t i = 0; i < vertex_positions.size(); ++i)
		{
			...
			_min = vz::math::Min(_min, pos);
			_max = vz::math::Max(_max, pos);
		}
		aabb = AABB(_min, _max);
		...
		if (!vertex_uvset_0.empty() || !vertex_uvset_1.empty())
		{
			...
			{
				uv_range_max = vz::math::Max(uv_range_max, uv0_stream[i]);
				uv_range_max = vz::math::Max(uv_range_max, uv1_stream[i]);
				uv_range_min = vz::math::Min(uv_range_min, uv0_stream[i]);
				uv_range_min = vz::math::Min(uv_range_min, uv1_stream[i]);
			}
		}
	...
	void MeshComponent::BuildBVH()
	{
		...
		for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset; ++subsetIndex)
		{
			...
			for (uint32_t triangleIndex = 0; triangleIndex < triangleCount; ++triangleIndex)
			{
				...
				AABB aabb = vz::primitive::AABB(vz::math::Min(p0, vz::math::Min(p1, p2)), vz::math::Max(p0, vz::math::Max(p1, p2)));
	...
	void MeshComponent::ComputeNormals(COMPUTE_NORMALS compute)
	{
		...
		if (compute != COMPUTE_NORMALS_SMOOTH_FAST)
		{
			...
			vz::vector<uint32_t> newIndexBuffer;
			vz::vector<XMFLOAT3> newPositionsBuffer;
			vz::vector<XMFLOAT3> newNormalsBuffer;
			vz::vector<XMFLOAT2> newUV0Buffer;
			vz::vector<XMFLOAT2> newUV1Buffer;
			vz::vector<XMFLOAT2> newAtlasBuffer;
			vz::vector<XMUINT4> newBoneIndicesBuffer;
			vz::vector<XMFLOAT4> newBoneWeightsBuffer;
			vz::vector<uint32_t> newColorsBuffer;
	...
	size_t MeshComponent::GetMemoryUsageBVH() const
	{
		return
			...
			bvh_leaf_aabbs.size() * sizeof(vz::primitive::AABB);
	}
	...
	void ObjectComponent::SaveLightmap()
	{
		if (lightmap.IsValid() && has_flag(lightmap.desc.bind_flags, BindFlag::RENDER_TARGET))
		{
			...
			bool success = vz::helper::saveTextureToMemory(lightmap, lightmapTextureData);
			assert(success);

#ifdef OPEN_IMAGE_DENOISE
			if (success)
			{
				vz::vector<uint8_t> texturedata_dst(lightmapTextureData.size());
			...
			vz::texturehelper::CreateTexture(lightmap, lightmapTextureData.data(), lightmapWidth, lightmapHeight, lightmap.desc.format);
			vz::graphics::GetDevice()->SetName(&lightmap, "lightmap");
		}
	}
	void ObjectComponent::CompressLightmap()
	{
		...
		else
		{
			...
			vz::renderer::BlockCompress(lightmap, bc6tex, cmd);
			vz::helper::saveTextureToMemory(bc6tex, lightmapTextureData); // internally waits for GPU completion
			lightmap.desc = desc;
		}
	}
	...
	void EnvironmentProbeComponent::CreateRenderData()
	{
		...
		{
			resource = vz::resourcemanager::Load(textureName);
		}
		if (resource.IsValid())
		{
		...
		resolution = vz::math::GetNextPowerOfTwo(resolution);
	...
	AnimationComponent::AnimationChannel::PathDataType AnimationComponent::AnimationChannel::GetPathDataType() const
	{
		switch (path)
		{
		case vz::scene::AnimationComponent::AnimationChannel::Path::TRANSLATION:
			return PathDataType::Float3;
		case vz::scene::AnimationComponent::AnimationChannel::Path::ROTATION:
			return PathDataType::Float4;
		case vz::scene::AnimationComponent::AnimationChannel::Path::SCALE:
			return PathDataType::Float3;
		case vz::scene::AnimationComponent::AnimationChannel::Path::WEIGHTS:
			return PathDataType::Weights;

		case vz::scene::AnimationComponent::AnimationChannel::Path::LIGHT_COLOR:
			return PathDataType::Float3;
		case vz::scene::AnimationComponent::AnimationChannel::Path::LIGHT_INTENSITY:
		case vz::scene::AnimationComponent::AnimationChannel::Path::LIGHT_RANGE:
		case vz::scene::AnimationComponent::AnimationChannel::Path::LIGHT_INNERCONE:
		case vz::scene::AnimationComponent::AnimationChannel::Path::LIGHT_OUTERCONE:
			return PathDataType::Float;

		case vz::scene::AnimationComponent::AnimationChannel::Path::SOUND_PLAY:
		case vz::scene::AnimationComponent::AnimationChannel::Path::SOUND_STOP:
			return PathDataType::Event;
		case vz::scene::AnimationComponent::AnimationChannel::Path::SOUND_VOLUME:
			return PathDataType::Float;

		case vz::scene::AnimationComponent::AnimationChannel::Path::EMITTER_EMITCOUNT:
			return PathDataType::Float;

		case vz::scene::AnimationComponent::AnimationChannel::Path::CAMERA_FOV:
		case vz::scene::AnimationComponent::AnimationChannel::Path::CAMERA_FOCAL_LENGTH:
		case vz::scene::AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SIZE:
			return PathDataType::Float;
		case vz::scene::AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SHAPE:
			return PathDataType::Float2;

		case vz::scene::AnimationComponent::AnimationChannel::Path::SCRIPT_PLAY:
		case vz::scene::AnimationComponent::AnimationChannel::Path::SCRIPT_STOP:
			return PathDataType::Event;

		case vz::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_COLOR:
		case vz::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_EMISSIVE:
		case vz::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_TEXMULADD:
			return PathDataType::Float4;
		case vz::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_ROUGHNESS:
		case vz::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_REFLECTANCE:
		case vz::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_METALNESS:
			return PathDataType::Float;
	...
	void SoftBodyPhysicsComponent::CreateFromMesh(const MeshComponent& mesh)
	{
		...
		if (physicsToGraphicsVertexMapping.empty())
		{
			...
			vz::unordered_map<size_t, uint32_t> uniquePositions;
	...
	void CameraComponent::Lerp(const CameraComponent& a, const CameraComponent& b, float t)
	{
		...
		width = vz::math::Lerp(a.width, b.width, t);
		height = vz::math::Lerp(a.height, b.height, t);
		zNearP = vz::math::Lerp(a.zNearP, b.zNearP, t);
		zFarP = vz::math::Lerp(a.zFarP, b.zFarP, t);
		fov = vz::math::Lerp(a.fov, b.fov, t);
		focal_length = vz::math::Lerp(a.focal_length, b.focal_length, t);
		aperture_size = vz::math::Lerp(a.aperture_size, b.aperture_size, t);
		aperture_shape = vz::math::Lerp(a.aperture_shape, b.aperture_shape, t);
	}

	void ScriptComponent::CreateFromFile(const std::string& filename)
	{
		this->filename = filename;
		resource = vz::resourcemanager::Load(filename, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
	...
	{
		_flags |= PLAYING;
		vz::audio::Play(&soundinstance);
	}
	void SoundComponent::Stop()
	{
		_flags &= ~PLAYING;
		vz::audio::Stop(&soundinstance);
	}
	void SoundComponent::SetLooped(bool value)
	{
		...
		{
			_flags |= LOOPED;
			vz::audio::CreateSoundInstance(&soundResource.GetSound(), &soundinstance);
		}
		else
		{
			_flags &= ~LOOPED;
			vz::audio::ExitLoop(&soundinstance);
		}
	}
