...
namespace vz::scene
{
	...
	struct MaterialComponent
	{
		...
		inline static const vz::vector<std::string> shaderTypeDefines[] = {
			{}, // SHADERTYPE_PBR,
		...
		vz::enums::STENCILREF engineStencilRef = vz::enums::STENCILREF_DEFAULT;
		uint8_t userStencilRef = 0;
		vz::enums::BLENDMODE userBlendMode = vz::enums::BLENDMODE_OPAQUE;
		...
		vz::graphics::ShadingRate shadingRate = vz::graphics::ShadingRate::RATE_1X1;
	...
	struct MeshComponent
	{
		...
		struct MeshSubset
		{
			vz::ecs::Entity materialID = vz::ecs::INVALID_ENTITY;
		...
		vz::vector<MeshSubset> subsets;

		float tessellationFactor = 0.0f;
		vz::ecs::Entity armatureID = vz::ecs::INVALID_ENTITY;

		struct MorphTarget
		{
			vz::vector<XMFLOAT3> vertex_positions;
			vz::vector<XMFLOAT3> vertex_normals;
			vz::vector<uint32_t> sparse_indices_positions; // optional, these can be used to target vertices indirectly
			vz::vector<uint32_t> sparse_indices_normals; // optional, these can be used to target vertices indirectly
			float weight = 0;
		...
		vz::vector<MorphTarget> morph_targets;
		...
		vz::vector<vz::graphics::RaytracingAccelerationStructure> BLASes; // one BLAS per LOD
		enum BLAS_STATE
		{
		...
		vz::vector<vz::primitive::AABB> bvh_leaf_aabbs;
		vz::BVH bvh;
		...
		inline vz::graphics::IndexBufferFormat GetIndexFormat() const { return vz::graphics::GetIndexBufferFormat((uint32_t)vertex_positions.size()); }
		inline size_t GetIndexStride() const { return GetIndexFormat() == vz::graphics::IndexBufferFormat::UINT32 ? sizeof(uint32_t) : sizeof(uint16_t); }
		inline bool IsSkinned() const { return armatureID != vz::ecs::INVALID_ENTITY; }
		...
		vz::primitive::Sphere GetBoundingSphere() const;
		...
		struct Vertex_POS10
		{
			...
			constexpr void FromFULL(const vz::primitive::AABB& aabb, XMFLOAT3 pos, uint8_t wind)
			{
				pos = vz::math::InverseLerp(aabb._min, aabb._max, pos); // UNORM remap
				x = uint32_t(vz::math::saturate(pos.x) * 1023.0f);
				y = uint32_t(vz::math::saturate(pos.y) * 1023.0f);
				z = uint32_t(vz::math::saturate(pos.z) * 1023.0f);
				w = uint32_t((float(wind) / 255.0f) * 3);
			}
			inline XMVECTOR LoadPOS(const vz::primitive::AABB& aabb) const
			{
				XMFLOAT3 v = GetPOS(aabb);
				return XMLoadFloat3(&v);
			}
			constexpr XMFLOAT3 GetPOS(const vz::primitive::AABB& aabb) const
			{
				XMFLOAT3 v = XMFLOAT3(
					float(x) / 1023.0f,
					float(y) / 1023.0f,
				...
				return vz::math::Lerp(aabb._min, aabb._max, v);
			}
			constexpr uint8_t GetWind() const
			{
				return uint8_t((float(w) / 3.0f) * 255);
			}
			static constexpr vz::graphics::Format FORMAT = vz::graphics::Format::R10G10B10A2_UNORM;
		};
		struct Vertex_POS16
		{
			uint16_t x = 0;
			...
			static constexpr vz::graphics::Format FORMAT = vz::graphics::Format::R16G16B16A16_UNORM;
		};
		struct Vertex_POS32
		{
			float x = 0;
			...
			static constexpr vz::graphics::Format FORMAT = vz::graphics::Format::R32G32B32_FLOAT;
		};
		struct Vertex_POS32W
		{
			...
			static constexpr vz::graphics::Format FORMAT = vz::graphics::Format::R32G32B32A32_FLOAT;
		};
		vz::graphics::Format position_format = Vertex_POS16::FORMAT; // CreateRenderData() will choose the appropriate format
		...
		struct Vertex_TEX
		{
			...
			{
				x = uint16_t(vz::math::InverseLerp(uv_range_min.x, uv_range_max.x, uv.x) * 65535.0f);
				y = uint16_t(vz::math::InverseLerp(uv_range_min.y, uv_range_max.y, uv.y) * 65535.0f);
			}
			static constexpr vz::graphics::Format FORMAT = vz::graphics::Format::R16G16_UNORM;
		};
		struct Vertex_UVS
		{
			Vertex_TEX uv0;
		...
		{
			uint32_t color = 0;
			static constexpr vz::graphics::Format FORMAT = vz::graphics::Format::R8G8B8A8_UNORM;
		};
		struct Vertex_NOR
		{
			int8_t x = 0;
			...
			static constexpr vz::graphics::Format FORMAT = vz::graphics::Format::R8G8B8A8_SNORM;
		};
		struct Vertex_TAN
		{
			int8_t x = 0;
	...
	struct ObjectComponent	// Actor
	{
		...
		{
			uint8_t value = 0;
			static constexpr vz::graphics::Format FORMAT = vz::graphics::Format::R8_UNORM;
		};
	};
	...
	struct SoftBodyPhysicsComponent
	{
		...
		float friction = 0.5f;
		float restitution = 0.0f;
		vz::vector<uint32_t> physicsToGraphicsVertexMapping; // maps graphics vertex index to physics vertex index of the same position
		vz::vector<uint32_t> graphicsToPhysicsVertexMapping; // maps a physics vertex index to first graphics vertex index of the same position
		vz::vector<float> weights; // weight per physics vertex controlling the mass. (0: disable weight (no physics, only animation), 1: default weight)

		// Non-serialized attributes:
		std::shared_ptr<void> physicsobject = nullptr; // You can set to null to recreate the physics object the next time phsyics system will be running.
		XMFLOAT4X4 worldMatrix = vz::math::IDENTITY_MATRIX;
		vz::vector<MeshComponent::Vertex_POS32> vertex_positions_simulation; // graphics vertices after simulation (world space)
		vz::vector<MeshComponent::Vertex_NOR> vertex_normals_simulation;
		vz::vector<XMFLOAT4>vertex_tangents_tmp;
		vz::vector<MeshComponent::Vertex_TAN> vertex_tangents_simulation;
		vz::primitive::AABB aabb;
	...
	struct ArmatureComponent
	{
		...
		vz::vector<vz::ecs::Entity> boneCollection;
		vz::vector<XMFLOAT4X4> inverseBindMatrices;

		// Non-serialized attributes:
		vz::primitive::AABB aabb;
		uint32_t gpuBoneOffset = 0;
		vz::vector<ShaderTransform> boneData;
	...
	struct LightComponent
	{
		...
		vz::vector<float> cascade_distances = { 8,80,800 };
		vz::vector<std::string> lensFlareNames;
		...
		vz::vector<vz::Resource> lensFlareRimTextures;
		...
		inline void BackCompatSetEnergy(float energy)
		{
			switch (type)
			{
			case vz::scene::LightComponent::POINT:
				intensity = energy * 20;
				break;
			case vz::scene::LightComponent::SPOT:
	...
	struct EnvironmentProbeComponent
	{
		...
		vz::Resource resource; // if texture is coming from an asset
		XMFLOAT3 position;
		float range;
	...
	struct AnimationDataComponent
	{
		...
		vz::vector<float> keyframe_times;
		vz::vector<float> keyframe_data;
	...
	struct AnimationComponent
	{
		...
		struct AnimationChannel
		{
			...
			vz::ecs::Entity target = vz::ecs::INVALID_ENTITY;
			int samplerIndex = -1;
			int retargetIndex = -1;
		...
		struct AnimationSampler
		{
			...
			vz::ecs::Entity data = vz::ecs::INVALID_ENTITY;
		...
		{
			vz::ecs::Entity source = vz::ecs::INVALID_ENTITY;
			XMFLOAT4X4 dstRelativeMatrix = vz::math::IDENTITY_MATRIX;
			XMFLOAT4X4 srcRelativeParentMatrix = vz::math::IDENTITY_MATRIX;
		};

		vz::vector<AnimationChannel> channels;
		vz::vector<AnimationSampler> samplers;
		vz::vector<RetargetSourceData> retargets;

		// Non-serialzied attributes:
		vz::vector<float> morph_weights_temp;
		float last_update_time = 0;

		// Root Motion
		XMFLOAT3 rootTranslationOffset;
		XMFLOAT4 rootRotationOffset;
		vz::ecs::Entity rootMotionBone;
		...
		inline vz::ecs::Entity GetRootMotionBone() const { return rootMotionBone; }
		inline void SetRootMotionBone(vz::ecs::Entity _rootMotionBone) { rootMotionBone = _rootMotionBone; }
	...
	struct WeatherComponent
	{
		...
		vz::Ocean::OceanParameters oceanParameters;
		...
		vz::Resource skyMap;
		vz::Resource colorGradingMap;
		vz::Resource volumetricCloudsWeatherMapFirst;
		vz::Resource volumetricCloudsWeatherMapSecond;
	...
	struct SoundComponent
	{
		...
		std::string filename;
		vz::Resource soundResource;
		vz::audio::SoundInstance soundinstance;
		float volume = 1;
	...
	struct VideoComponent
	{
		...
		std::string filename;
		vz::Resource videoResource;
		vz::video::VideoInstance videoinstance;
	...
	struct InverseKinematicsComponent
	{
		...
		vz::ecs::Entity target = vz::ecs::INVALID_ENTITY; // which entity to follow (must have a transform component)
	...
	struct ColliderComponent
	{
		...
		// Non-serialized attributes:
		vz::primitive::Sphere sphere;
		vz::primitive::Capsule capsule;
		vz::primitive::Plane plane;
		uint32_t layerMask = ~0u;
	...
	struct ScriptComponent
	{
		...
		// Non-serialized attributes:
		vz::vector<uint8_t> script; // compiled script binary data
		vz::Resource resource;
	...
	struct ExpressionComponent
	{
		...
		struct Expression
		{
			...
			vz::vector<MorphTargetBinding> morph_target_bindings;
		...
		vz::vector<Expression> expressions;
	...
	struct HumanoidComponent
	{
		...
		vz::ecs::Entity bones[size_t(HumanoidBone::Count)] = {};
