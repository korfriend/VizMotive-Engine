...
namespace vz
{
	namespace ocean_internal
	{
		...
		vz::fftgenerator::CSFFT512x512_Plan m_fft_plan;


		void LoadShaders()
		{
			vz::renderer::LoadShader(ShaderStage::CS, updateSpectrumCS, "oceanSimulatorCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, updateDisplacementMapCS, "oceanUpdateDisplacementMapCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, updateGradientFoldingCS, "oceanUpdateGradientFoldingCS.cso");

			vz::renderer::LoadShader(ShaderStage::VS, oceanSurfVS, "oceanSurfaceVS.cso");

			vz::renderer::LoadShader(ShaderStage::PS, oceanSurfPS, "oceanSurfacePS.cso");
			vz::renderer::LoadShader(ShaderStage::PS, wireframePS, "oceanSurfaceSimplePS.cso");
...
#define GRAV_ACCEL	981.0f	// The acceleration of gravity, cm/s^2
	...
	void Ocean::Create(const OceanParameters& params)
	{
		...
		vz::vector<XMFLOAT2> h0_data(height_map_size);
		vz::vector<float> omega_data(height_map_size);
		...
		vz::vector<char> zero_data(3 * output_size * sizeof(float) * 2);
	...
	void Ocean::UpdateDisplacementMap(const OceanParameters& params, CommandList cmd) const
	{
		...
		vz::renderer::GenerateMipChain(gradientMap, vz::renderer::MIPGENFILTER_LINEAR, cmd);
	...
	void Ocean::Render(const CameraComponent& camera, const OceanParameters& params, CommandList cmd) const
	{
		...
		bool wire = vz::renderer::IsWireRender();
		...
		{
			vz::vector<uint32_t> index_data(index_count);
			size_t counter = 0;
	...
	void Ocean::Initialize()
	{
		...
		static vz::eventhandler::Handle handle = vz::eventhandler::Subscribe(vz::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); vz::fftgenerator::LoadShaders(); });

		LoadShaders();
		vz::fftgenerator::LoadShaders();
		fft512x512_create_plan(m_fft_plan, 3);

		vz::backlog::post("vz::Ocean Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
