...
namespace vz::profiler
{
	...
	vz::Color background_color = vz::Color(20, 20, 20, 230);
	vz::Color text_color = vz::Color::White();
	...
	vz::unordered_map<size_t, Range> ranges;
	...
	void BeginFrame()
	{
		...
		if (!initialized)
		{
			...
			{
				vz::backlog::post("[vz::profiler] Superluminal Performance API loaded");
			}
#endif // PERFORMANCEAPI_ENABLED
		}
	...
	range_id BeginRangeCPU(const char* name)
	{
		...
		range_id id = vz::helper::string_hash(name);
		...
		{
			vz::helper::hash_combine(id, differentiator++);
		}
	...
	void LoadShaders()
	{
		...
		desc.rs = vz::renderer::GetRasterizerState(vz::enums::RSTYPE_WIRE_SMOOTH);
	...
	vz::unordered_map<std::string, Hits> time_cache_cpu;
	vz::unordered_map<std::string, Hits> time_cache_gpu;
	void DrawData(
	...
	)
	{
		...
		vz::font::Params params = vz::font::Params(x, y + (graph_size.y + graph_padding_y) * 2, vz::font::WIFONTSIZE_DEFAULT - 6, vz::font::WIFALIGN_LEFT, vz::font::WIFALIGN_TOP, text_color);

		// Background:
		vz::image::Params fx;
		fx.pos.x = x - 10;
		fx.pos.y = y - 10;
		fx.siz.x = std::max(graph_size.x, (float)vz::font::TextWidth(ss.str(), params)) + 200;
		fx.siz.y = (float)vz::font::TextHeight(ss.str(), params) + (graph_size.y + graph_padding_y) * 2 + 20;
		...
		vz::font::Draw(ss.str(), params, cmd);


		// Graph:
		{
			GraphicsDevice* device = vz::graphics::GetDevice();
			vz::graphics::GraphicsDevice::MemoryUsage gpu_memory_usage = device->GetMemoryUsage();
			vz::helper::MemoryUsage cpu_memory_usage = vz::helper::GetMemoryUsage();

			static bool shaders_loaded = false;
			if (!shaders_loaded)
			{
				shaders_loaded = true;
				static vz::eventhandler::Handle handle = vz::eventhandler::Subscribe(vz::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); });
				LoadShaders();
			}
			...
			vz::font::Draw("16.6 ms", params, cmd);
			...
			params.color = vz::Color::fromFloat4(XMFLOAT4(1, 0.2f, 0.2f, 1));
			ss.str("");
			ss.clear();
			...
			params.color = vz::Color::fromFloat4(XMFLOAT4(0.2f, 1, 0.2f, 1));
			ss.str("");
			ss.clear();
			...
			params.color = vz::Color::fromFloat4(XMFLOAT4(1, 0.2f, 1, 1));
			ss.str("");
			ss.clear();
			...
			params.color = vz::Color::fromFloat4(XMFLOAT4(1, 1, 0.2f, 1));
			ss.str("");
			ss.clear();
			...
			vz::font::Draw("60 frames", params, cmd);
			params.position.x = x + graph_size.x;
			vz::font::Draw("current frame", params, cmd);
