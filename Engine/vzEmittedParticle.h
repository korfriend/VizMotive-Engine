...
namespace vz
{
	...
	public:
		...
		vz::graphics::GPUBuffer statisticsReadbackBuffer[vz::graphics::GraphicsDevice::GetBufferCount()];

		vz::graphics::GPUBuffer particleBuffer;
		vz::graphics::GPUBuffer aliveList[2];
		vz::graphics::GPUBuffer deadList;
		vz::graphics::GPUBuffer distanceBuffer; // for sorting
		vz::graphics::GPUBuffer sphGridCells;  // for SPH
		vz::graphics::GPUBuffer sphParticleCells;  // for SPH
		vz::graphics::GPUBuffer densityBuffer; // for SPH
		vz::graphics::GPUBuffer counterBuffer;
		vz::graphics::GPUBuffer indirectBuffers; // kickoffUpdate, simulation, draw
		vz::graphics::GPUBuffer constantBuffer;
		vz::graphics::GPUBuffer generalBuffer;
		vz::scene::MeshComponent::BufferView vb_pos;
		vz::scene::MeshComponent::BufferView vb_nor;
		vz::scene::MeshComponent::BufferView vb_uvs;
		vz::scene::MeshComponent::BufferView vb_col;
		vz::graphics::GPUBuffer primitiveBuffer; // raytracing
		vz::graphics::GPUBuffer culledIndirectionBuffer; // rasterization
		vz::graphics::GPUBuffer culledIndirectionBuffer2; // rasterization
	...
	public:
		void UpdateCPU(const vz::scene::TransformComponent& transform, float dt);
		void Burst(int num);
		void Restart();

		// Must have a transform and material component, but mesh is optional
		void UpdateGPU(uint32_t instanceIndex, const vz::scene::MeshComponent* mesh, vz::graphics::CommandList cmd) const;
		void Draw(const vz::scene::MaterialComponent& material, vz::graphics::CommandList cmd) const;
