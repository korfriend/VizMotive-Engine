...
namespace vz::physics
{
	...
	namespace bullet
	{
		...
		class DebugDraw final : public btIDebugDraw
		{
			...
			{
				vz::backlog::post(warningString);
			}
			void draw3dText(const btVector3& location, const char* textString) override
			{
				vz::renderer::DebugTextParams params;
				params.position.x = location.x();
				params.position.y = location.y();
				params.position.z = location.z();
				params.scaling = 0.6f;
				params.flags |= vz::renderer::DebugTextParams::CAMERA_FACING;
				params.flags |= vz::renderer::DebugTextParams::CAMERA_SCALING;
				vz::renderer::DrawDebugText(textString, params);
			}
		...
		RigidBody& GetRigidBody(vz::scene::RigidBodyPhysicsComponent& physicscomponent)
		{
		...
		SoftBody& GetSoftBody(vz::scene::SoftBodyPhysicsComponent& physicscomponent)
		{
			if (physicscomponent.physicsobject == nullptr)
			{
	...
	void Initialize()
	{
		...
		vz::backlog::post("vz::physics Initialized [Bullet] (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
	...
	)
	{
		...
		case RigidBodyPhysicsComponent::CollisionShape::CONVEX_HULL:
			...
			{
				vz::backlog::post("Convex Hull physics requested, but no MeshComponent provided!");
				assert(0);
			}
			break;
		...
		case RigidBodyPhysicsComponent::CollisionShape::TRIANGLE_MESH:
			...
			{
				vz::backlog::post("Triangle Mesh physics requested, but no MeshComponent provided!");
				assert(0);
			}
			break;
		}
	...
	)
	{
		...
		vz::vector<btScalar> btVerts(vCount * 3);
...
#undef IDX
	...
	)
	{
		...
		auto range = vz::profiler::BeginRangeCPU("Physics");
		...
		for (size_t i = 0; i < scene.humanoids.GetCount(); ++i)
		{
			...
			if (humanoid.ragdoll != nullptr)
			{
				...
				if (!vz::math::float_equal(ragdoll.scale, scale))
				{
		...
		vz::jobsystem::Dispatch(ctx, (uint32_t)scene.rigidbodies.GetCount(), 256, [&](vz::jobsystem::JobArgs args) {
		...
		vz::jobsystem::Dispatch(ctx, (uint32_t)scene.softbodies.GetCount(), 1, [&](vz::jobsystem::JobArgs args) {
			...
			if (physicscomponent.physicsobject != nullptr)
			{
				...
				for (size_t ind = 0; ind < physicscomponent.weights.size(); ++ind)
				{
					...
					if (weight == 0)
					{
						...
						XMFLOAT3 position = mesh.vertex_positions[graphicsInd];
						XMVECTOR P = armature == nullptr ? XMLoadFloat3(&position) : vz::scene::SkinVertex(mesh, *armature, graphicsInd);
						P = XMVector3Transform(P, worldMatrix);
						XMStoreFloat3(&position, P);
		...
		for (int i = 0; i < dynamicsWorld.getCollisionObjectArray().size(); ++i)
		{
			...
			else
			{
				...
				if (softbody != nullptr)
				{
					...
					physicscomponent->aabb = vz::primitive::AABB(XMFLOAT3(aabb_min.x(), aabb_min.y(), aabb_min.z()), XMFLOAT3(aabb_max.x(), aabb_max.y(), aabb_max.z()));
		...
		vz::profiler::EndRange(range); // Physics
	}
	...
	constexpr int to_internal(ActivationState state)
	{
		...
		{
		default:
		case vz::physics::ActivationState::Active:
			return ACTIVE_TAG;
		case vz::physics::ActivationState::Inactive:
	...
	)
	{
		...
		btSoftRigidDynamicsWorld& dynamicsWorld = ((PhysicsScene*)scene.physics_scene.get())->dynamicsWorld;
		float tmin = vz::math::Clamp(ray.TMin, 0, 1000000);
		float tmax = vz::math::Clamp(ray.TMax, 0, 1000000);
