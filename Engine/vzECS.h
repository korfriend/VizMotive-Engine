...
namespace vz::ecs
{
	...
	struct EntitySerializer
	{
		...
		vz::unordered_map<uint64_t, Entity> remap;
		...
		vz::unordered_set<std::string> resource_registration; // register for resource manager serialization
	...
	inline void SerializeEntity(vz::Archive& archive, Entity& entity, EntitySerializer& seri)
	{
		...
		{
			// Entities are always serialized as uint64_t for back-compact
			uint64_t mem;
			archive >> mem;
...
#pragma GCC diagnostic pop
	...
	private:
		...
		vz::unordered_map<Entity, size_t> lookup;
	...
	public:
		...
		vz::unordered_map<std::string, LibraryEntry> entries;
