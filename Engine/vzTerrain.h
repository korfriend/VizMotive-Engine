...
{
	template <>
	struct hash<vz::terrain::Chunk>
	{
		inline size_t operator()(const vz::terrain::Chunk& chunk) const
		{
...
namespace vz::terrain
{
	...
	struct VirtualTextureAtlas
	{
		struct Map
		{
			...
			vz::graphics::Texture texture_raw_block;
		};
		Map maps[3];
		vz::graphics::GPUBuffer tile_pool;
		...
		vz::vector<Tile> free_tiles;

		struct Residency
		{
			vz::graphics::Texture feedbackMap;
			vz::graphics::Texture residencyMap;
			vz::graphics::GPUBuffer requestBuffer;
			vz::graphics::GPUBuffer allocationBuffer;
			vz::graphics::GPUBuffer allocationBuffer_CPU_readback[vz::graphics::GraphicsDevice::GetBufferCount() + 1];
			vz::graphics::GPUBuffer pageBuffer;
			vz::graphics::GPUBuffer pageBuffer_CPU_upload[vz::graphics::GraphicsDevice::GetBufferCount() + 1];
			bool data_available_CPU[vz::graphics::GraphicsDevice::GetBufferCount() + 1] = {};
		...
		vz::unordered_map<uint32_t, vz::vector<std::shared_ptr<Residency>>> free_residencies; // per resolution residencies
	...
	struct VirtualTexture
	{
		...
		vz::vector<VirtualTextureAtlas::Tile> tiles;
		...
		mutable vz::vector<UpdateRequest> update_requests;
		vz::graphics::Texture region_weights_texture;
	};

	struct ChunkData
	{
		vz::ecs::Entity entity = vz::ecs::INVALID_ENTITY;
		vz::ecs::Entity grass_entity = vz::ecs::INVALID_ENTITY;
		vz::ecs::Entity props_entity = vz::ecs::INVALID_ENTITY;
		const XMFLOAT3* mesh_vertex_positions = nullptr;
		float prop_density_current = 1;
		vz::HairParticleSystem grass;
		float grass_density_current = 1;
		vz::vector<vz::Color> region_weights;
		vz::graphics::Texture region_weights_texture;
	...
	struct Prop
	{
		vz::vector<uint8_t> data; // serialized component data storage
	...
	struct Terrain
	{
		...
		vz::ecs::Entity terrainEntity = vz::ecs::INVALID_ENTITY;
		vz::scene::Scene* scene = nullptr;
		vz::scene::MaterialComponent material_Base;
		vz::scene::MaterialComponent material_Slope;
		vz::scene::MaterialComponent material_LowAltitude;
		vz::scene::MaterialComponent material_HighAltitude;
		vz::scene::MaterialComponent material_GrassParticle;
		vz::scene::WeatherComponent weather;
		vz::HairParticleSystem grass_properties;
		vz::unordered_map<Chunk, ChunkData> chunks;
		Chunk center_chunk = {};
		vz::noise::Perlin perlin_noise;
		vz::vector<Prop> props;
		...
		vz::vector<vz::graphics::GPUBarrier> virtual_texture_barriers_before_update;
		vz::vector<vz::graphics::GPUBarrier> virtual_texture_barriers_after_update;
		vz::vector<vz::graphics::GPUBarrier> virtual_texture_barriers_before_allocation;
		vz::vector<vz::graphics::GPUBarrier> virtual_texture_barriers_after_allocation;
		vz::vector<const VirtualTexture*> virtual_textures_in_use;
		vz::graphics::Sampler sampler;
		...
		vz::vector<std::shared_ptr<Modifier>> modifiers;
		vz::vector<Modifier*> modifiers_to_remove;
		...
		void Generation_Update(const vz::scene::CameraComponent& camera);
		...
		void UpdateVirtualTexturesGPU(vz::graphics::CommandList cmd) const;
		void CopyVirtualTexturePageStatusGPU(vz::graphics::CommandList cmd) const;
		void AllocateVirtualTextureTileRequestsGPU(vz::graphics::CommandList cmd) const;
		void WritebackTileRequestsGPU(vz::graphics::CommandList cmd) const;
	...
	struct Modifier
	{
		...
		constexpr void Blend(float& height, float value)
		{
			...
			case BlendMode::Normal:
				height = vz::math::Lerp(height, value, weight);
				break;
	...
	struct VoronoiModifier : public Modifier
	{
		...
		void Apply(const XMFLOAT2& world_pos, float& height) override
		{
			...
			vz::noise::voronoi::Result res = vz::noise::voronoi::compute(p.x, p.y, (float)seed);
			float weight = std::pow(1 - vz::math::saturate((res.distance - shape) * fade), std::max(0.0001f, falloff));
			Blend(height, weight);
		}
	};
