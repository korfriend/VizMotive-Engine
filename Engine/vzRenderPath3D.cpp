...
namespace vz
{
	...
	void RenderPath3D::ResizeBuffers()
	{
		...
		{
			TextureDesc desc;
			desc.format = vz::renderer::format_idbuffer;
			desc.bind_flags = BindFlag::RENDER_TARGET | BindFlag::SHADER_RESOURCE;
			if (getMSAASampleCount() > 1)
			{
		...
		if (device->CheckCapability(GraphicsDeviceCapability::VARIABLE_RATE_SHADING_TIER2) &&
			vz::renderer::GetVariableRateShadingClassification())
		{
		...
		vz::renderer::CreateVisibilityResources(visibilityResources, internalResolution);
		vz::renderer::CreateTiledLightResources(tiledLightResources, internalResolution);
		vz::renderer::CreateScreenSpaceShadowResources(screenspaceshadowResources, internalResolution);
	...
	void RenderPath3D::Update(float dt)
	{
		...
		if (getSceneUpdateEnabled())
		{
			if (vz::renderer::GetSurfelGIEnabled() ||
				vz::renderer::GetDDGIEnabled() ||
				(hw_raytrace && vz::renderer::GetRaytracedShadowsEnabled()) ||
			...
			scene->camera = *camera;
			scene->Update(dt * vz::renderer::GetGameSpeed());
		}
		...
		visibility_main.flags = vz::renderer::Visibility::ALLOW_EVERYTHING;
		if (!getOcclusionCullingEnabled())
		{
			visibility_main.flags &= ~vz::renderer::Visibility::ALLOW_OCCLUSION_CULLING;
		}
		vz::renderer::UpdateVisibility(visibility_main);
		...
		vz::renderer::UpdatePerFrameData(
			*scene,
			visibility_main,
			frameCB,
		...
		else if (vz::renderer::GetTemporalAAEnabled())
		{
			const XMFLOAT4& halton = vz::math::GetHaltonSequence(vz::graphics::GetDevice()->GetFrameCount() % 256);
			...
			{
				vz::renderer::CreateTemporalAAResources(temporalAAResources, internalResolution);
			}
		}
		else
		{
		...
		if (!vz::renderer::GetRaytracedShadowsEnabled())
		{
		...
		if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
		{
			...
			{
				vz::renderer::CreateRTShadowResources(rtshadowResources, internalResolution);
			}
		}
		else
		{
		...
		if (scene->weather.IsRealisticSky() && scene->weather.IsRealisticSkyAerialPerspective())
		{
			...
			{
				vz::renderer::CreateAerialPerspectiveResources(aerialperspectiveResources, internalResolution);
			}
			if (getReflectionsEnabled() && depthBuffer_Reflection.IsValid())
			{
				if (!aerialperspectiveResources_reflection.texture_output.IsValid())
				{
					vz::renderer::CreateAerialPerspectiveResources(aerialperspectiveResources_reflection, XMUINT2(depthBuffer_Reflection.desc.width, depthBuffer_Reflection.desc.height));
				}
			}
			else
			{
		...
		if (scene->weather.IsVolumetricClouds())
		{
			...
			{
				vz::renderer::CreateVolumetricCloudResources(volumetriccloudResources, internalResolution);
			}
			if (getReflectionsEnabled() && depthBuffer_Reflection.IsValid())
			{
				if (!volumetriccloudResources_reflection.texture_cloudRender.IsValid())
				{
					vz::renderer::CreateVolumetricCloudResources(volumetriccloudResources_reflection, XMUINT2(depthBuffer_Reflection.desc.width, depthBuffer_Reflection.desc.height));
				}
			}
			else
			{
		...
		if (vz::renderer::GetSurfelGIEnabled())
		{
			...
			{
				vz::renderer::CreateSurfelGIResources(surfelGIResources, internalResolution);
			}
		}
		...
		if (vz::renderer::GetVXGIEnabled())
		{
			...
			{
				vz::renderer::CreateVXGIResources(vxgiResources, internalResolution);
			}
		}
		else
		{
		...
		if (
			...
			vz::renderer::GetTemporalAAEnabled() ||
			getSSREnabled() ||
			getRaytracedReflectionEnabled() ||
			getRaytracedDiffuseEnabled() ||
			vz::renderer::GetRaytracedShadowsEnabled() ||
			getAO() == AO::AO_RTAO ||
			vz::renderer::GetVariableRateShadingClassification() ||
			getFSR2Enabled()
			)
		...
		if (vz::renderer::GetScreenSpaceShadowsEnabled() || vz::renderer::GetRaytracedShadowsEnabled())
		{
			if (!rtShadow.IsValid())
			{
				TextureDesc desc;
		...
		if (vz::renderer::GetVXGIReflectionsEnabled())
		{
		...
		if (getSceneUpdateEnabled() && scene->videos.GetCount() > 0)
		{
			...
			{
				const vz::scene::VideoComponent& video = scene->videos[i];
				if (vz::video::IsDecodingRequired(&video.videoinstance, dt))
				{
			...
			for (size_t i = 0; i < scene->videos.GetCount(); ++i)
			{
				vz::scene::VideoComponent& video = scene->videos[i];
				vz::video::UpdateVideo(&video.videoinstance, dt, video_cmd);
			}
		}
	}
	...
	void RenderPath3D::Render() const
	{
		...
		vz::renderer::ProcessDeferredTextureRequests(cmd); // Execute it first thing in the frame here, on main thread, to not allow other thread steal it and execute on different command list!
		...
		vz::jobsystem::Execute(ctx, [this, cmd](vz::jobsystem::JobArgs args) {
			...
			if (vz::renderer::GetDDGIEnabled() && getSceneUpdateEnabled())
			{
				vz::renderer::DDGI(
					*scene,
					cmd
		...
		vz::jobsystem::Execute(ctx, [this, cmd](vz::jobsystem::JobArgs args) {
			...
			if (getOcclusionCullingEnabled())
			{
				vz::renderer::OcclusionCulling_Reset(visibility_main, cmd); // must be outside renderpass!
			}

			vz::renderer::RefreshImpostors(*scene, cmd);
			...
			auto range = vz::profiler::BeginRangeGPU("Z-Prepass", cmd);
			...
			vz::renderer::DrawScene(
				...
				cmd,
				vz::renderer::DRAWSCENE_OPAQUE |
				vz::renderer::DRAWSCENE_FOREGROUND_ONLY |
				vz::renderer::DRAWSCENE_MAINCAMERA
			);
			...
			if (getOcclusionCullingEnabled())
			{
				vz::renderer::OcclusionCulling_Render(*camera, visibility_main, cmd);
			}
			...
			if (getOcclusionCullingEnabled())
			{
				vz::renderer::OcclusionCulling_Resolve(visibility_main, cmd); // must be outside renderpass!
			}
		...
		vz::jobsystem::Execute(ctx, [this, cmd](vz::jobsystem::JobArgs args) {
			...
			{
				vz::scene::VideoComponent& video = scene->videos[i];
				vz::video::ResolveVideoToRGB(&video.videoinstance, cmd);
			}
			...
			vz::renderer::Visibility_Prepare(
				visibilityResources,
				rtPrimitiveID_render,
				cmd
			);

			vz::renderer::ComputeTiledLightCulling(
				tiledLightResources,
				visibility_main,
				debugUAV,
				cmd
			...
			if (visibility_shading_in_compute)
			{
				vz::renderer::Visibility_Surface(
					visibilityResources,
					rtMain,
					cmd
				);
				...
				getRaytracedReflectionEnabled() ||
				getRaytracedDiffuseEnabled() ||
				vz::renderer::GetScreenSpaceShadowsEnabled() ||
				vz::renderer::GetRaytracedShadowsEnabled() ||
				vz::renderer::GetVXGIEnabled()
				)
			{
				// These post effects require surface normals and/or roughness
				vz::renderer::Visibility_Surface_Reduced(
					visibilityResources,
					cmd
				);
			}
			...
			if (rtVelocity.IsValid())
			{
				vz::renderer::Visibility_Velocity(
					visibilityResources,
					rtVelocity,
					cmd
				);
			...
			if (vz::renderer::GetVariableRateShadingClassification() && device->CheckCapability(GraphicsDeviceCapability::VARIABLE_RATE_SHADING_TIER2))
			{
				vz::renderer::ComputeShadingRateClassification(
					rtShadingRate,
					debugUAV,
					cmd
				);
			...
			if (vz::renderer::GetScreenSpaceShadowsEnabled())
			{
				vz::renderer::Postprocess_ScreenSpaceShadow(
					screenspaceshadowResources,
			...
			if (vz::renderer::GetRaytracedShadowsEnabled())
			{
				vz::renderer::Postprocess_RTShadow(
					rtshadowResources,
					*scene,
					tiledLightResources.entityTiles,
					rtLinearDepth,
		...
		if (getShadowsEnabled())
		{
			cmd = device->BeginCommandList();
			vz::jobsystem::Execute(ctx, [this, cmd](vz::jobsystem::JobArgs args) {
				vz::renderer::DrawShadowmaps(visibility_main, cmd);
				});
		}

		if (vz::renderer::GetVXGIEnabled() && getSceneUpdateEnabled())
		{
			cmd = device->BeginCommandList();
			vz::jobsystem::Execute(ctx, [cmd, this](vz::jobsystem::JobArgs args) {
				vz::renderer::VXGI_Voxelize(visibility_main, cmd);
			});
		}
		...
		if (getSceneUpdateEnabled())
		{
			...
			vz::jobsystem::Execute(ctx, [cmd, this](vz::jobsystem::JobArgs args) {
				...
				vz::renderer::RefreshLightmaps(*scene, cmd);
				vz::renderer::RefreshEnvProbes(visibility_main, cmd);
			});
		}
		...
		if (visibility_main.IsRequestedPlanarReflections())
		{
			...
			vz::jobsystem::Execute(ctx, [cmd, this](vz::jobsystem::JobArgs args) {
				...
				auto range = vz::profiler::BeginRangeGPU("Planar Reflections Z-Prepass", cmd);
				...
				vz::profiler::EndRange(range); // Planar Reflections
				device->EventEnd(cmd);
			...
			vz::jobsystem::Execute(ctx, [cmd, this](vz::jobsystem::JobArgs args) {
				...
				if (scene->weather.IsVolumetricClouds())
				{
					vz::renderer::Postprocess_VolumetricClouds(
						...
						camera_reflection_previous,
						camera_reflection,
						vz::renderer::GetTemporalAAEnabled() || getFSR2Enabled(),
				...
				auto range = vz::profiler::BeginRangeGPU("Planar Reflections", cmd);
				...
				if (scene->weather.IsRealisticSky() && scene->weather.IsRealisticSkyAerialPerspective())
				{
					...
					vz::image::Draw(&aerialperspectiveResources_reflection.texture_output, fx, cmd);
					device->EventEnd(cmd);
				}
				...
				if (scene->weather.IsVolumetricClouds())
				{
					...
					fx.enableFullScreen();
					fx.blendFlag = BLENDMODE_PREMULTIPLIED;
					vz::image::Draw(&volumetriccloudResources_reflection.texture_reproject[volumetriccloudResources_reflection.frame % 2], fx, cmd);
					device->EventEnd(cmd);
				}

				vz::renderer::DrawSoftParticles(visibility_reflection, false, cmd);
				vz::renderer::DrawSpritesAndFonts(*scene, camera_reflection, false, cmd);
		...
		vz::jobsystem::Execute(ctx, [this, cmd](vz::jobsystem::JobArgs args) {
			...
			{
				vz::renderer::Postprocess_RTReflection(
					rtreflectionResources,
					*scene,
					rtSSR,
					cmd,
			...
			if (getRaytracedDiffuseEnabled())
			{
				vz::renderer::Postprocess_RTDiffuse(
					rtdiffuseResources,
					*scene,
					rtRaytracedDiffuse,
					cmd,
			...
			if (vz::renderer::GetRaytracedShadowsEnabled() || vz::renderer::GetScreenSpaceShadowsEnabled())
			{
			...
			if (getOutlineEnabled())
			{
				...
				params.stencilRefMode = vz::image::STENCILREFMODE_ENGINE;
				params.stencilComp = vz::image::STENCILMODE_EQUAL;
				params.stencilRef = vz::enums::STENCILREF_OUTLINE;
				vz::image::Draw(&rtLinearDepth, params, cmd);
				params.stencilRef = vz::enums::STENCILREF_CUSTOMSHADER_OUTLINE;
				vz::image::Draw(&rtLinearDepth, params, cmd);
				device->RenderPassEnd(cmd);
				device->EventEnd(cmd);
			}
			...
			{
				auto range = vz::profiler::BeginRangeGPU("Opaque Scene", cmd);
				...
				vz::renderer::DrawSky(*scene, cmd);
				vz::profiler::EndRange(range); // Opaque Scene
			}
	...
	void RenderPath3D::Compose(CommandList cmd) const
	{
		...
		vz::image::Draw(GetLastPostprocessRT(), fx, cmd);
		device->EventEnd(cmd);

		if (
			vz::renderer::GetDebugLightCulling() ||
			vz::renderer::GetVariableRateShadingClassificationDebug() ||
			vz::renderer::GetSurfelGIDebugEnabled()
			)
		{
			fx.enableFullScreen();
			fx.blendFlag = BLENDMODE_PREMULTIPLIED;
			vz::image::Draw(&debugUAV, fx, cmd);
		}
	...
	void RenderPath3D::RenderOutline(CommandList cmd) const
	{
		if (getOutlineEnabled())
		{
			vz::renderer::Postprocess_Outline(
				rtOutlineSource,
				cmd,
				getOutlineThreshold(),
				getOutlineThickness(),
		...
		if (getLightShaftsEnabled() && XMVectorGetX(XMVector3Dot(sunDirection, camera->GetAt())) > 0)
		{
			...
			// Render sun stencil cutout:
			{
				...
				if (scene->weather.IsVolumetricClouds())
				{
					...
					vz::image::Draw(&volumetriccloudResources.texture_cloudMask, fx, cmd);
					device->EventEnd(cmd);
				}
			...
			// Radial blur on the sun:
			{
					...
					camera->GetProjection(), camera->GetView(), XMMatrixIdentity());
				{
					...
					vz::renderer::Postprocess_LightShafts(
	...
	void RenderPath3D::RenderVolumetrics(CommandList cmd) const
	{
		...
		{
			auto range = vz::profiler::BeginRangeGPU("Volumetric Lights", cmd);
			...
			vz::renderer::DrawVolumeLights(visibility_main, cmd);

			device->RenderPassEnd(cmd);

			vz::renderer::Postprocess_Blur_Bilateral(
				rtVolumetricLights[0],
				rtLinearDepth,
				rtVolumetricLights[1],
				rtVolumetricLights[0],
	...
	void RenderPath3D::RenderSceneMIPChain(CommandList cmd) const
	{
		...
		auto range = vz::profiler::BeginRangeGPU("Scene MIP Chain", cmd);
		device->EventBegin("RenderSceneMIPChain", cmd);

		vz::renderer::Postprocess_Downsample4x(rtMain, rtSceneCopy, cmd);

		vz::renderer::MIPGEN_OPTIONS mipopt;
		mipopt.gaussian_temp = &rtSceneCopy_tmp;
		vz::renderer::GenerateMipChain(rtSceneCopy, vz::renderer::MIPGENFILTER_GAUSSIAN, cmd, mipopt);
	...
	void RenderPath3D::RenderTransparents(CommandList cmd) const
	{
		...
		if (!scene->waterRipples.empty())
		{
			...
			vz::renderer::DrawWaterRipples(visibility_main, cmd);
			device->RenderPassEnd(cmd);
		}
		...
		// Transparent scene
		{
			auto range = vz::profiler::BeginRangeGPU("Transparent Scene", cmd);
			...
			vz::renderer::DrawScene(
				...
				vz::renderer::DRAWSCENE_TESSELLATION |
				vz::renderer::DRAWSCENE_OCCLUSIONCULLING |
				vz::renderer::DRAWSCENE_OCEAN |
				vz::renderer::DRAWSCENE_MAINCAMERA
			);

			device->EventEnd(cmd);
			vz::profiler::EndRange(range); // Transparent Scene
		}
		...
		// Distortion particles:
		{
			...
			vz::renderer::DrawSoftParticles(visibility_main, true, cmd);
			vz::renderer::DrawSpritesAndFonts(*scene, *camera, true, cmd);
	...
	void RenderPath3D::RenderPostprocessChain(CommandList cmd) const
	{
		...
		// 1.) HDR post process chain
		{
			...
			else if (vz::renderer::GetTemporalAAEnabled() && !vz::renderer::GetTemporalAADebugEnabled() && temporalAAResources.IsValid())
			{
				vz::renderer::Postprocess_TemporalAA(
					temporalAAResources,
					*rt_read,
					cmd
				);
			...
			{
				vz::renderer::Postprocess_Underwater(
			...
			for (auto& x : custom_post_processes)
			{
				...
				{
					vz::renderer::Postprocess_Custom(
						x.computeshader,
						rt_first == nullptr ? *rt_read : *rt_first,
						*rt_write,
						cmd,
			...
			{
				vz::renderer::Postprocess_DepthOfField(
					depthoffieldResources,
					rt_first == nullptr ? *rt_read : *rt_first,
					*rt_write,
					cmd,
			...
			{
				vz::renderer::Postprocess_MotionBlur(
					motionblurResources,
					rt_first == nullptr ? *rt_read : *rt_first,
					*rt_write,
					cmd,
		...
		// 3.) LDR post process chain
		{
			...
			{
				vz::renderer::Postprocess_Sharpen(*rt_read, *rt_write, cmd, getSharpenFilterAmount());
			...
			if (getFXAAEnabled())
			{
				vz::renderer::Postprocess_FXAA(*rt_read, *rt_write, cmd);
			...
			{
				vz::renderer::Postprocess_Chromatic_Aberration(*rt_read, *rt_write, cmd, getChromaticAberrationAmount());
			...
			// GUI Background blurring:
			{
				...
				vz::renderer::Postprocess_Downsample4x(*rt_read, rtGUIBlurredBackground[0], cmd);
			...
			{
				vz::renderer::Postprocess_FSR(*rt_read, rtFSR[1], rtFSR[0], cmd, getFSRSharpness());
	...
	void RenderPath3D::setAO(AO value)
	{
		...
		case RenderPath3D::AO_HBAO:
			...
			vz::renderer::CreateSSAOResources(ssaoResources, internalResolution);
			break;
		case RenderPath3D::AO_MSAO:
			desc.width = internalResolution.x;
			desc.height = internalResolution.y;
			vz::renderer::CreateMSAOResources(msaoResources, internalResolution);
			break;
		case RenderPath3D::AO_RTAO:
			desc.width = internalResolution.x / 2;
			desc.height = internalResolution.y / 2;
			vz::renderer::CreateRTAOResources(rtaoResources, internalResolution);
			break;
		default:
			break;
		}
	...
	void RenderPath3D::setSSREnabled(bool value)
	{
		...
		if (value)
		{
			...
			vz::renderer::CreateSSRResources(ssrResources, internalResolution);
		}
		else
		{
			ssrResources = {};
	...
	void RenderPath3D::setRaytracedReflectionsEnabled(bool value)
	{
		...
		if (value)
		{
			...
			vz::renderer::CreateRTReflectionResources(rtreflectionResources, internalResolution);
		}
		else
		{
			rtreflectionResources = {};
	...
	void RenderPath3D::setRaytracedDiffuseEnabled(bool value)
	{
		...
		if (value)
		{
			...
			vz::renderer::CreateRTDiffuseResources(rtdiffuseResources, internalResolution);
		}
		else
		{
			rtRaytracedDiffuse = {};
	...
	void RenderPath3D::setFSR2Enabled(bool value)
	{
		...
		if (fsr2Enabled)
		{
			...
			vz::renderer::CreateFSR2Resources(fsr2Resources, GetInternalResolution(), displayResolution);
	...
	{
		vz::graphics::SamplerDesc desc = vz::renderer::GetSampler(vz::enums::SAMPLER_OBJECTSHADER)->GetDesc();
		switch (preset)
		{
		default:
		...
		vz::renderer::ModifyObjectSampler(desc);
	}
	void RenderPath3D::setMotionBlurEnabled(bool value)
	{
		...
		if (value)
		{
			...
			vz::renderer::CreateMotionBlurResources(motionblurResources, resolution);
		}
		else
		{
			motionblurResources = {};
	...
	void RenderPath3D::setDepthOfFieldEnabled(bool value)
	{
		...
		if (value)
		{
			...
			vz::renderer::CreateDepthOfFieldResources(depthoffieldResources, resolution);
		}
		else
		{
			depthoffieldResources = {};
	...
	void RenderPath3D::setEyeAdaptionEnabled(bool value)
	{
		...
		if (value)
		{
			vz::renderer::CreateLuminanceResources(luminanceResources, GetInternalResolution());
		}
		else
		{
			luminanceResources = {};
	...
	void RenderPath3D::setReflectionsEnabled(bool value)
	{
		...
		if (value)
		{
			...
			vz::renderer::CreateTiledLightResources(tiledLightResources_planarReflection, XMUINT2(depthBuffer_Reflection.desc.width, depthBuffer_Reflection.desc.height));
		}
		else
		{
			rtReflection = {};
	...
	void RenderPath3D::setBloomEnabled(bool value)
	{
		...
		if (value)
		{
			vz::renderer::CreateBloomResources(bloomResources, GetInternalResolution());
		}
		else
		{
			bloomResources = {};
