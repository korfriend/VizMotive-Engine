...
#define KEY_UP(vk_code) (!KEY_DOWN(vk_code))
	...
	vz::platform::window_type window = nullptr;
	vz::Canvas canvas;
	...
	vz::vector<Controller> controllers;
	...
	void Initialize()
	{
		...
		vz::input::rawinput::Initialize();
		vz::input::sdlinput::Initialize();

		vz::backlog::post("vz::input Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
		initialized.store(true);
	}

	void Update(vz::platform::window_type _window, vz::Canvas _canvas)
	{
		window = _window;
		canvas = _canvas;
		if (!initialized.load())
...
#if defined(_WIN32)
		vz::input::rawinput::GetMouseState(&mouse); // currently only the relative data can be used from this
		vz::input::rawinput::GetKeyboardState(&keyboard); 
...
#elif SDL2
		vz::input::sdlinput::GetMouseState(&mouse);
		mouse.position.x /= canvas.GetDPIScaling();
		mouse.position.y /= canvas.GetDPIScaling();
		vz::input::sdlinput::GetKeyboardState(&keyboard);
		//TODO controllers
		//TODO touch
#endif
		...
		{
			if (vz::input::rawinput::GetControllerState(nullptr, i))
			{
				int slot = -1;
				for (int j = 0; j < (int)controllers.size(); ++j)
				{
		...
		for (int i = 0; i < vz::input::sdlinput::GetMaxControllerCount(); ++i)
		{
			if (vz::input::sdlinput::GetControllerState(nullptr, i))
			{
				int slot = -1;
				for (int j = 0; j < (int)controllers.size(); ++j)
				{
		...
		for (auto& controller : controllers)
		{
			...
			switch (controller.deviceType)
			{
				...
				case Controller::SDLINPUT:
					connected = vz::input::sdlinput::GetControllerState(&controller.state, controller.deviceIndex);
					break;
	...
	bool Down(BUTTON button, int playerindex)
	{
		...
		else if (playerindex == 0) // keyboard or mouse
		{
			...
			switch (button)
			{
			case vz::input::MOUSE_BUTTON_LEFT:
				if (mouse.left_button_press) 
					return true;
				return false;
			case vz::input::MOUSE_BUTTON_RIGHT:
				if (mouse.right_button_press) 
					return true;
				return false;
			case vz::input::MOUSE_BUTTON_MIDDLE:
				if (mouse.middle_button_press) 
					return true;
				return false;
#ifdef _WIN32
			case vz::input::KEYBOARD_BUTTON_UP:
				keycode = VK_UP;
				break;
			case vz::input::KEYBOARD_BUTTON_DOWN:
				keycode = VK_DOWN;
				break;
			case vz::input::KEYBOARD_BUTTON_LEFT:
				keycode = VK_LEFT;
				break;
			case vz::input::KEYBOARD_BUTTON_RIGHT:
				keycode = VK_RIGHT;
				break;
			case vz::input::KEYBOARD_BUTTON_SPACE:
				keycode = VK_SPACE;
				break;
			case vz::input::KEYBOARD_BUTTON_RSHIFT:
				keycode = VK_RSHIFT;
				break;
			case vz::input::KEYBOARD_BUTTON_LSHIFT:
				keycode = VK_LSHIFT;
				break;
			case vz::input::KEYBOARD_BUTTON_F1: 
				keycode = VK_F1;
				break;
			case vz::input::KEYBOARD_BUTTON_F2:
				keycode = VK_F2;
				break;
			case vz::input::KEYBOARD_BUTTON_F3:
				keycode = VK_F3;
				break;
			case vz::input::KEYBOARD_BUTTON_F4:
				keycode = VK_F4;
				break;
			case vz::input::KEYBOARD_BUTTON_F5:
				keycode = VK_F5;
				break;
			case vz::input::KEYBOARD_BUTTON_F6:
				keycode = VK_F6;
				break;
			case vz::input::KEYBOARD_BUTTON_F7:
				keycode = VK_F7;
				break;
			case vz::input::KEYBOARD_BUTTON_F8:
				keycode = VK_F8;
				break;
			case vz::input::KEYBOARD_BUTTON_F9:
				keycode = VK_F9;
				break;
			case vz::input::KEYBOARD_BUTTON_F10:
				keycode = VK_F10;
				break;
			case vz::input::KEYBOARD_BUTTON_F11:
				keycode = VK_F11;
				break;
			case vz::input::KEYBOARD_BUTTON_F12:
				keycode = VK_F12;
				break;
			case vz::input::KEYBOARD_BUTTON_ENTER:
				keycode = VK_RETURN;
				break;
			case vz::input::KEYBOARD_BUTTON_ESCAPE:
				keycode = VK_ESCAPE;
				break;
			case vz::input::KEYBOARD_BUTTON_HOME:
				keycode = VK_HOME;
				break;
			case vz::input::KEYBOARD_BUTTON_LCONTROL:
				keycode = VK_LCONTROL;
				break;
			case vz::input::KEYBOARD_BUTTON_RCONTROL:
				keycode = VK_RCONTROL;
				break;
			case vz::input::KEYBOARD_BUTTON_DELETE:
				keycode = VK_DELETE;
				break;
			case vz::input::KEYBOARD_BUTTON_BACKSPACE:
				keycode = VK_BACK;
				break;
			case vz::input::KEYBOARD_BUTTON_PAGEDOWN:
				keycode = VK_NEXT;
				break;
			case vz::input::KEYBOARD_BUTTON_PAGEUP:
				keycode = VK_PRIOR;
				break;
	...
	void SetControllerFeedback(const ControllerFeedback& data, int playerindex)
	{
		if (playerindex < (int)controllers.size())
		{
			...
			{
				vz::input::sdlinput::SetControllerFeedback(data, controller.deviceIndex);
			}
		}
	}
	...
	const vz::vector<Touch>& GetTouches()
	{
		return touches;
	}
