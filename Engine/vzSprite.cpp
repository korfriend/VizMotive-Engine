...
namespace vz
{
	...
	Sprite::Sprite(const std::string& newTexture, const std::string& newMask)
	{
		if (!newTexture.empty())
		{
			...
			textureResource = vz::resourcemanager::Load(newTexture);
		}
		if (!newMask.empty())
		{
			maskName = newMask;
			maskResource = vz::resourcemanager::Load(newMask);
	...
	void Sprite::Draw(CommandList cmd) const
	{
		if (IsHidden())
			return;
		vz::image::Draw(GetTexture(), params, cmd);
	}
	...
	void Sprite::Update(float dt)
	{
		...
		if (anim.wobbleAnim.amount.x > 0 || anim.wobbleAnim.amount.y > 0)
		{
			...
			vz::image::Params default_params; // contains corners in idle positions
			for (int i = 0; i < 4; ++i)
			{
	...
	const vz::graphics::Texture* Sprite::GetTexture() const
	{
		return textureResource.IsValid() ? &textureResource.GetTexture() : vz::texturehelper::getWhite();
	}

	void Sprite::Serialize(vz::Archive& archive, vz::ecs::EntitySerializer& seri)
	{
			...
			if (!textureName.empty() || !maskName.empty())
			{
				vz::jobsystem::Execute(seri.ctx, [&](vz::jobsystem::JobArgs args) {
					if (!textureName.empty())
					{
						...
						textureResource = vz::resourcemanager::Load(textureName, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
					}
					if (!maskName.empty())
					{
						maskName = dir + maskName;
						maskResource = vz::resourcemanager::Load(maskName, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
					}
				});
			}
		}
		else
		{
			...
			archive << vz::helper::GetPathRelative(dir, maskName);
