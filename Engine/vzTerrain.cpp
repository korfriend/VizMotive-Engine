...
namespace vz::terrain
{
	...
	struct Generator
	{
		vz::scene::Scene scene; // The background generation thread can safely add things to this, it will be merged into the main scene when it is safe to do so
		vz::jobsystem::context workload;
	...
	void Terrain::Generation_Update(const CameraComponent& camera)
	{
		...
		for (auto it = chunks.begin(); it != chunks.end();)
		{
			...
			if (vz::renderer::GetOcclusionCullingEnabled())
			{
			...
			{
				vz::HairParticleSystem* grass = scene->hairs.GetComponent(chunk_data.grass_entity);
				if (grass != nullptr)
				{
			...
			if (IsPhysicsEnabled())
			{
				...
				if (rigidbody != nullptr)
				{
					...
					else
					{
						...
						{
							vz::physics::SetActivationState(*rigidbody, vz::physics::ActivationState::Active);
						}
						else
						{
							//scene->rigidbodies.Remove(chunk_data.entity);
							vz::physics::SetActivationState(*rigidbody, vz::physics::ActivationState::Inactive);
						}
					}
				}
				else
		...
		vz::jobsystem::Execute(generator->workload, [=](vz::jobsystem::JobArgs args) {
			...
			auto request_chunk = [&](int offset_x, int offset_z)
			{
				...
				if (it == chunks.end() || it->second.entity == INVALID_ENTITY)
				{
					...
					object.filterMask |= vz::enums::FILTER_NAVIGATION_MESH;
					...
					vz::HairParticleSystem grass = grass_properties;
					...
					vz::jobsystem::Dispatch(ctx, vertexCount, chunk_width, [&](vz::jobsystem::JobArgs args) {
						...
						for (int i = 0; i < arraysize(corners); ++i)
						{
							...
							height = vz::math::Lerp(bottomLevel, topLevel, height);
						...
						const float slope_amount = 1.0f - vz::math::saturate(normal.y);
						...
						const float region_low_altitude = bottomLevel == 0 ? 0 : std::pow(vz::math::saturate(vz::math::InverseLerp(0, bottomLevel, height)), region2);
						const float region_high_altitude = topLevel == 0 ? 0 : std::pow(vz::math::saturate(vz::math::InverseLerp(0, topLevel, height)), region3);

						XMFLOAT4 materialBlendWeights(region_base, 0, 0, 0);
						materialBlendWeights = vz::math::Lerp(materialBlendWeights, XMFLOAT4(0, 1, 0, 0), region_slope);
						materialBlendWeights = vz::math::Lerp(materialBlendWeights, XMFLOAT4(0, 0, 1, 0), region_low_altitude);
						materialBlendWeights = vz::math::Lerp(materialBlendWeights, XMFLOAT4(0, 0, 0, 1), region_high_altitude);
						...
						chunk_data.region_weights[index] = vz::Color::fromFloat4(materialBlendWeights);
					...
					vz::jobsystem::Wait(ctx); // wait until chunk's vertex buffer is fully generated
					...
					vz::jobsystem::Wait(ctx); // wait until mesh.CreateRenderData() async task finishes
				...
				if (dist <= 1 && IsGrassEnabled())
				{
					...
					if (it != chunks.end() && it->second.entity != INVALID_ENTITY)
					{
						...
						if (chunk_data.grass_entity == INVALID_ENTITY && chunk_data.grass.meshID != INVALID_ENTITY)
						{
							...
							vz::HairParticleSystem& grass = generator->scene.hairs.Create(chunk_data.grass_entity);
				...
				if (dist <= prop_generation)
				{
					...
					if (it != chunks.end() && it->second.entity != INVALID_ENTITY)
					{
						...
						if (prop_density > 0 && chunk_data.props_entity == INVALID_ENTITY && chunk_data.mesh_vertex_positions != nullptr)
						{
							...
							vz::random::RNG rng((uint32_t)chunk.compute_hash() ^ seed);
							...
							for (const auto& prop : props)
							{
								...
								for (int i = 0; i < gen_count; ++i)
								{
									...
									{
										vz::Archive archive = vz::Archive(prop.data.data());
										EntitySerializer seri;
										Entity entity = generator->scene.Entity_Serialize(
											archive,
											seri,
											INVALID_ENTITY,
											vz::scene::Scene::EntitySerializeFlags::RECURSIVE |
											vz::scene::Scene::EntitySerializeFlags::KEEP_INTERNAL_ENTITY_REFERENCES
										);
										...
										transform->translation_local.y += vz::math::Lerp(prop.min_y_offset, prop.max_y_offset, rng.next_float());
										const float scaling = vz::math::Lerp(prop.min_size, prop.max_size, rng.next_float());
	...
	void Terrain::Generation_Cancel()
	{
		...
		vz::jobsystem::Wait(generator->workload); // waits until generation thread exits
	...
	void Terrain::UpdateVirtualTexturesCPU()
	{
		...
		for (auto& it : chunks)
		{
			...
			vz::jobsystem::Execute(ctx, [this, &vt](vz::jobsystem::JobArgs args) {
	...
	void Terrain::UpdateVirtualTexturesGPU(CommandList cmd) const
	{
		...
		auto range = vz::profiler::BeginRangeGPU("Terrain - UpdateVirtualTexturesGPU", cmd);
		...
		device->BindComputeShader(vz::renderer::GetShader(vz::enums::CSTYPE_VIRTUALTEXTURE_RESIDENCYUPDATE), cmd);
		for (const VirtualTexture* vt : virtual_textures_in_use)
		{
			if (vt->residency == nullptr)
				continue;
		...
		for (uint32_t map_type = 0; map_type < 3; map_type++)
		{
			...
			{
			case MaterialComponent::BASECOLORMAP:
				device->BindComputeShader(vz::renderer::GetShader(vz::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_BASECOLORMAP), cmd);
				device->BindUAV(&atlas.maps[MaterialComponent::BASECOLORMAP].texture_raw_block, 0, cmd);
				break;
			case MaterialComponent::NORMALMAP:
				device->BindComputeShader(vz::renderer::GetShader(vz::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_NORMALMAP), cmd);
				device->BindUAV(&atlas.maps[MaterialComponent::NORMALMAP].texture_raw_block, 0, cmd);
				break;
			case MaterialComponent::SURFACEMAP:
				device->BindComputeShader(vz::renderer::GetShader(vz::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_SURFACEMAP), cmd);
	...
	void Terrain::AllocateVirtualTextureTileRequestsGPU(CommandList cmd) const
	{
...

		{
			...
			device->BindComputeShader(vz::renderer::GetShader(vz::enums::CSTYPE_VIRTUALTEXTURE_TILEREQUESTS), cmd);
			for (const VirtualTexture* vt : virtual_textures_in_use)
			{
				if (vt->residency == nullptr)
					continue;
...

		{
			...
			device->BindComputeShader(vz::renderer::GetShader(vz::enums::CSTYPE_VIRTUALTEXTURE_TILEALLOCATE), cmd);
			for (const VirtualTexture* vt : virtual_textures_in_use)
			{
				if (vt->residency == nullptr)
					continue;
	...
	void Terrain::Serialize(vz::Archive& archive, vz::ecs::EntitySerializer& seri)
	{
		Generation_Cancel();
			...
			for (size_t i = 0; i < props.size(); ++i)
			{
				...
				if (seri.GetVersion() >= 1)
				{
					...
					if (!prop.data.empty())
					{
						...
						Scene tmp_scene;
						vz::Archive tmp_archive = vz::Archive(prop.data.data());
				...
				else
				{
					...
					tmp_scene.Entity_Serialize(
						...
						seri,
						object_entity,
						vz::scene::Scene::EntitySerializeFlags::RECURSIVE | vz::scene::Scene::EntitySerializeFlags::KEEP_INTERNAL_ENTITY_REFERENCES
					);
