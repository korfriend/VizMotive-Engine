...
namespace vz::gui
{
	...
	{
		vz::graphics::PipelineState PSO_colored;

		InternalState()
		{
			vz::Timer timer;

			static vz::eventhandler::Handle handle = vz::eventhandler::Subscribe(vz::eventhandler::EVENT_RELOAD_SHADERS, [this](uint64_t userdata) { LoadShaders(); });
			LoadShaders();

			vz::backlog::post("vz::gui Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
		}

		void LoadShaders()
		{
			PipelineStateDesc desc;
			desc.vs = vz::renderer::GetShader(vz::enums::VSTYPE_VERTEXCOLOR);
			desc.ps = vz::renderer::GetShader(vz::enums::PSTYPE_VERTEXCOLOR);
			desc.il = vz::renderer::GetInputLayout(vz::enums::ILTYPE_VERTEXCOLOR);
			desc.dss = vz::renderer::GetDepthStencilState(vz::enums::DSSTYPE_DEPTHDISABLED);
			desc.bs = vz::renderer::GetBlendState(vz::enums::BSTYPE_TRANSPARENT);
			desc.rs = vz::renderer::GetRasterizerState(vz::enums::RSTYPE_DOUBLESIDED);
			desc.pt = PrimitiveTopology::TRIANGLESTRIP;
			vz::graphics::GetDevice()->CreatePipelineState(&desc, &PSO_colored);
		}
	};
	...
	void GUI::Update(const vz::Canvas& canvas, float dt)
	{
		if (!visible || vz::backlog::isActive())
		{
			return;
		}

		auto range = vz::profiler::BeginRangeCPU("GUI Update");

		scroll_allowed = true;

		XMFLOAT4 pointer = vz::input::GetPointer();
	...
	void GUI::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		...
		auto range_cpu = vz::profiler::BeginRangeCPU("GUI Render");
		auto range_gpu = vz::profiler::BeginRangeGPU("GUI Render", cmd);
	...
	void GUI::SetColor(vz::Color color, int id)
	{
	...
	void GUI::SetShadowColor(vz::Color color)
	{
		for (auto& widget : widgets)
		{
	...
	void GUI::ExportLocalization(vz::Localization& localization) const
	{
		vz::Localization& section = localization.GetSection("gui");
		for (auto& widget : widgets)
		{
			widget->ExportLocalization(section);
		}
	}
	void GUI::ImportLocalization(const vz::Localization& localization)
	{
		const vz::Localization* section = localization.CheckSection("gui");
		if (section == nullptr)
			return;
		for (auto& widget : widgets)
		{
	...
	Widget::Widget()
	{
		sprites[IDLE].params.color = vz::Color::Booger();
		sprites[FOCUS].params.color = vz::Color::Gray();
		sprites[ACTIVE].params.color = vz::Color::White();
		sprites[DEACTIVATING].params.color = vz::Color::Gray();
		font.params.shadowColor = vz::Color::Shadow();
		font.params.shadow_bolden = 0.2f;
		font.params.shadow_softness = 0.2f;

		tooltipSprite.params.color = vz::Color(255, 234, 165);
		tooltipFont.params.color = vz::Color(25, 25, 25, 255);

		for (int i = IDLE; i < WIDGETSTATE_COUNT; ++i)
		{
			sprites[i].params.blendFlag = vz::enums::BLENDMODE_OPAQUE;
	...
	void Widget::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		scale = vz::math::Max(scale, XMFLOAT3(0.001f, 0.001f, 0.001f));
	...
	void Widget::RenderTooltip(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		if (tooltipTimer > 25)
		{
			...
			tooltipFont.params.h_align = vz::font::WIFALIGN_LEFT;
			tooltipFont.params.v_align = vz::font::WIFALIGN_TOP;
	...
	void Widget::SetSize(const XMFLOAT2& value)
	{
		...
		scale_local = vz::math::Max(scale_local, XMFLOAT3(0.001f, 0.001f, 0.001f));
		UpdateTransform();
	...
	vz::Color Widget::GetColor() const
	{
		return vz::Color::fromFloat4(sprites[GetState()].params.color);
	}
	...
	void Widget::ExportLocalization(vz::Localization& localization) const
	{
		...
		vz::Localization& section = localization.GetSection(GetName());
	...
	void Widget::ImportLocalization(const vz::Localization& localization)
	{
		const vz::Localization* section = localization.CheckSection(GetName());
		if (section == nullptr)
			return;
	...
	void Widget::SetColor(vz::Color color, int id)
	{
		if (id < 0)
		{
	...
	void Widget::SetShadowColor(vz::Color color)
	{
		shadow_color = color;
	}
	void Widget::SetImage(vz::Resource textureResource, int id)
	{
		if (id < 0)
		{
			for (int i = 0; i < arraysize(sprites); ++i)
	...
	void Widget::ApplyScissor(const vz::Canvas& canvas, const Rect rect, CommandList cmd, bool constrain_to_parent) const
	{
		Rect scissor = rect;
	...
	void Widget::HitboxConstrain(vz::primitive::Hitbox2D& hb) const
	{
		float left = hb.pos.x;
	...
	void Button::Create(const std::string& name)
	{
		...
		font.params.h_align = vz::font::WIFALIGN_CENTER;
		font.params.v_align = vz::font::WIFALIGN_CENTER;

		font_description.params = font.params;
		font_description.params.h_align = vz::font::WIFALIGN_RIGHT;
	}
	void Button::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (vz::input::Press(vz::input::MOUSE_BUTTON_LEFT))
			{
				if (state == FOCUS)
				{
					// activate
			...
			if (vz::input::Down(vz::input::MOUSE_BUTTON_LEFT))
			{
				if (state == DEACTIVATING)
				{
		...
		{
		case vz::font::WIFALIGN_LEFT:
			font.params.posX = translation.x + 2;
			break;
		case vz::font::WIFALIGN_RIGHT:
			font.params.posX = translation.x + scale.x - 2;
			break;
		case vz::font::WIFALIGN_CENTER:
		default:
		...
		{
		case vz::font::WIFALIGN_TOP:
			font.params.posY = translation.y + 2;
			break;
		case vz::font::WIFALIGN_BOTTOM:
	...
	void Button::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		if (shadow > 0)
		{
			vz::image::Params fx = sprites[state].params;
			fx.pos.x -= shadow;
			fx.pos.y -= shadow;
			fx.siz.x += shadow * 2;
			fx.siz.y += shadow * 2;
	...
	void ScrollBar::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (IsScrollbarRequired() && hitBox.intersects(pointerHitbox))
			{
				if (clicked)
				{
					...
					grab_pos.x = vz::math::Clamp(grab_pos.x, scrollbar_begin + scrollbar_delta, scrollbar_begin + scrollbar_delta + scrollbar_length);
					grab_pos.y = vz::math::Clamp(grab_pos.y, scrollbar_begin + scrollbar_delta, scrollbar_begin + scrollbar_delta + scrollbar_length);
		...
		scrollbar_delta = vz::math::Clamp(scrollbar_delta, 0, scrollbar_size - scrollbar_length);
		if (scrollbar_begin < scrollbar_end - scrollbar_length)
		{
			scrollbar_value = vz::math::InverseLerp(scrollbar_begin, scrollbar_end - scrollbar_length, scrollbar_begin + scrollbar_delta);
		}
		else
		{
			scrollbar_value = 0;
	...
	void ScrollBar::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		//GraphicsDevice* device = vz::graphics::GetDevice();
		//device->BindScissorRects(1, &scissorRect, cmd);
		//vz::image::Draw(nullptr, vz::image::Params(hitBox.pos.x, hitBox.pos.y, hitBox.siz.x, hitBox.siz.y, vz::Color(255,0,0,100)), cmd);

	}
	void ScrollBar::SetColor(vz::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	void Label::Create(const std::string& name)
	{
		...
		scrollbar.SetColor(vz::Color(80, 80, 80, 100), vz::gui::IDLE);
		scrollbar.sprites_knob[ScrollBar::SCROLLBAR_INACTIVE].params.color = vz::Color(140, 140, 140, 140);
		scrollbar.sprites_knob[ScrollBar::SCROLLBAR_HOVER].params.color = vz::Color(180, 180, 180, 180);
		scrollbar.sprites_knob[ScrollBar::SCROLLBAR_GRABBED].params.color = vz::Color::White();
		scrollbar.SetOverScroll(0.25f);
		scrollbar.knob_inset_border = XMFLOAT2(4, 2);
	}
	void Label::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (scroll_allowed && scrollbar.IsScrollbarRequired() && pointerHitbox.intersects(hitBox))
			{
				...
				scrollbar.Scroll(vz::input::GetPointer().z * 20);
			}
			else
			{
				state = IDLE;
			...
			if (pointerHitbox.intersects(hitBox) && vz::input::Press(vz::input::MOUSE_BUTTON_LEFT))
			{
				Activate();
			}
		}
	...
	void Label::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
	...
	void Label::SetColor(vz::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	vz::SpriteFont TextInputField::font_input;
	int caret_pos = 0;
	int caret_begin = 0;
	int caret_delay = 0;
	bool input_updated = false;
	vz::Timer caret_timer;
	...
	void TextInputField::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (state == ACTIVE)
			{
				if (vz::input::Press(vz::input::KEYBOARD_BUTTON_ENTER))
				{
					// accept input...
				...
				//else if (vz::input::Press(vz::input::KEYBOARD_BUTTON_BACKSPACE))
				//{
				//	// delete input...
				//	DeleteFromInput(-1);
				//}
				else if (vz::input::Press(vz::input::KEYBOARD_BUTTON_DELETE))
				{
					// delete input...
					DeleteFromInput(1);
				}
				else if (vz::input::Press(vz::input::KEYBOARD_BUTTON_LEFT) && caret_pos > 0)
				{
					// caret repositioning left:
					caret_pos--;
					if (!vz::input::Down(vz::input::BUTTON::KEYBOARD_BUTTON_LSHIFT) && !vz::input::Down(vz::input::BUTTON::KEYBOARD_BUTTON_RSHIFT))
					{
						caret_begin = caret_pos;
					}
					caret_timer.record();
				}
				else if (vz::input::Press(vz::input::KEYBOARD_BUTTON_RIGHT) && caret_pos < font_input.GetText().size())
				{
				...
				else if ((clicked && !intersectsPointer) || vz::input::Press(vz::input::KEYBOARD_BUTTON_ESCAPE))
				{
					// cancel input
					font_input.text.clear();
					Deactivate();
				...
				else if (vz::input::Down(vz::input::MOUSE_BUTTON_LEFT))
				{
					...
					{
						XMFLOAT2 size = vz::font::TextSize(str.c_str() + i, 1, font_input.params);
						pos += size.x;
				...
				if (vz::input::Down(vz::input::KEYBOARD_BUTTON_LCONTROL) && vz::input::Down((vz::input::BUTTON)'A'))
				{
					caret_begin = 0;
					caret_pos = (int)font_input.GetText().size();
				}
	...
	void TextInputField::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		if (state == ACTIVE)
		{
			...
			{
				vz::font::Params params = font_input.params;
				XMFLOAT2 size = vz::font::TextSize(font_input.GetText().c_str(), caret_pos, font_input.params);
				params.posX += size.x;
				params.color = vz::Color::lerp(params.color, vz::Color::Transparent(), 0.1f);
				params.size += 4;
				params.posY -= 2;
				params.h_align = vz::font::WIFALIGN_CENTER;
				vz::font::Draw(L"|", params, cmd);
			}
			...
			if(caret_pos != caret_begin)
			{
				...
				const std::wstring& str = font_input.GetText();
				float pos_start = font_input.params.position.x + vz::font::TextSize(str.c_str(), start, font_input.params).x;
				float pos_end = font_input.params.position.x + vz::font::TextSize(str.c_str(), end, font_input.params).x;
				...
				params.blendFlag = vz::enums::BLENDMODE_ALPHA;
				params.color = vz::Color::lerp(font_input.params.color, vz::Color::Transparent(), 0.5f);
				vz::image::Draw(nullptr, params, cmd);
			}
			...
			//GraphicsDevice* device = vz::graphics::GetDevice();
			//device->BindScissorRects(1, &scissorRect, cmd);
			//vz::font::Draw("caret_begin = " + std::to_string(caret_begin) + "\ncaret_pos = " + std::to_string(caret_pos), vz::font::Params(0, 20), cmd);
		}
		else
		{
			font.Draw(cmd);
	...
	{
		input_updated = true;
		if (vz::input::Down(vz::input::KEYBOARD_BUTTON_LCONTROL) || vz::input::Down(vz::input::KEYBOARD_BUTTON_RCONTROL))
			return;
		switch (inputChar)
		{
		case '\b':	// BACKSPACE
	...
	void TextInputField::SetColor(vz::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	void Slider::Create(float start, float end, float defaultValue, float step, const std::string& name)
	{
		...
		sprites[IDLE].params.color = vz::Color(60, 60, 60, 200);
		sprites[FOCUS].params.color = vz::Color(50, 50, 50, 200);
		sprites[ACTIVE].params.color = vz::Color(50, 50, 50, 200);
		sprites[DEACTIVATING].params.color = vz::Color(60, 60, 60, 200);

		font.params.h_align = vz::font::WIFALIGN_RIGHT;
		font.params.v_align = vz::font::WIFALIGN_CENTER;
	}
	...
	void Slider::SetRange(float start, float end)
	{
		this->start = start;
		this->end = end;
		this->value = vz::math::Clamp(this->value, start, end);
	}
	void Slider::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		valueInputField.SetSize(XMFLOAT2(std::max(scale.y, vz::font::TextWidth(valueInputField.GetText(), valueInputField.font.params) + 4), scale.y));
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (dragged)
			{
				EventArgs args;
				args.clickPos = pointerHitbox.pos;
				value = vz::math::InverseLerp(translation.x, translation.x + scale.x, args.clickPos.x);
				value = vz::math::Clamp(value, 0, 1);
				value *= step;
				value = std::floor(value);
				value /= step;
				value = vz::math::Lerp(start, end, value);
				args.fValue = value;
		...
		sprites_knob[state].params.pos.x = vz::math::Lerp(translation.x + knobWidthHalf + 2, translation.x + scale.x - knobWidthHalf - 2, vz::math::Clamp(vz::math::InverseLerp(start, end, value), 0, 1));
	...
	void Slider::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
	...
	void Slider::RenderTooltip(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const
	{
	...
	void Slider::SetColor(vz::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	void CheckBox::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
	...
	void CheckBox::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		if (GetCheck())
		{
			if (!check_text.empty())
			{
				...
				vz::font::Draw(check_text, params, cmd);
			}
			else if (!check_text_global.empty())
			{
				...
				params.scaling = 0.75f;
				params.color = font.params.color;
				vz::font::Draw(check_text_global, params, cmd);
			}
			else
			{
				// simple square:
		...
		else if (!uncheck_text.empty())
		{
			...
			params.scaling = 0.75f;
			params.color = font.params.color;
			vz::font::Draw(uncheck_text, params, cmd);
		}
	...
	{
		vz::helper::StringConvert(text, check_text_global);
	}
	void CheckBox::SetCheckText(const std::string& text)
	{
		vz::helper::StringConvert(text, check_text);
	}
	void CheckBox::SetUnCheckText(const std::string& text)
	{
		vz::helper::StringConvert(text, uncheck_text);
	}
	...
	void ComboBox::Create(const std::string& name)
	{
		...
		selected_font = font;
		selected_font.params.h_align = vz::font::WIFALIGN_CENTER;
		selected_font.params.v_align = vz::font::WIFALIGN_CENTER;
	}
	float ComboBox::GetDropOffset(const vz::Canvas& canvas) const
	{
		float screenheight = canvas.GetLogicalHeight();
	...
	float ComboBox::GetItemOffset(const vz::Canvas& canvas, int index) const
	{
	...
	void ComboBox::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (state == ACTIVE)
			{
					...
					if (combostate != COMBOSTATE_SELECTING && combostate != COMBOSTATE_INACTIVE)
					{
						if (combostate == COMBOSTATE_SCROLLBAR_GRABBED || pointerHitbox.intersects(Hitbox2D(XMFLOAT2(translation.x + scale.x + 1, translation.y + scale.y + drop_offset), XMFLOAT2(scale.y, (float)std::min(maxVisibleItemCount, (int)items.size()) * scale.y))))
						{
							if (click_down)
							{
								...
								scrollbar_delta = vz::math::Clamp(pointerHitbox.pos.y, scrollbar_begin, scrollbar_end) - scrollbar_begin;
								const float scrollbar_value = vz::math::InverseLerp(scrollbar_begin, scrollbar_end, scrollbar_begin + scrollbar_delta);
				...
				{
					scroll_allowed = false;
					int scroll = (int)vz::input::GetPointer().z;
					firstItemVisible -= scroll;
					firstItemVisible = std::max(0, std::min((int)items.size() - maxVisibleItemCount, firstItemVisible));
					if (scroll)
					{
						const float scrollbar_value = vz::math::InverseLerp(0, float(std::max(0, (int)items.size() - maxVisibleItemCount)), float(firstItemVisible));
						scrollbar_delta = vz::math::Lerp(scrollbar_begin, scrollbar_end, scrollbar_value) - scrollbar_begin;
					}
	...
	void ComboBox::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		vz::Color color = GetColor();
		if (combostate != COMBOSTATE_INACTIVE)
		{
			color = vz::Color::fromFloat4(sprites[FOCUS].params.color);
		}
		...
		if (!vb_triangle.IsValid())
		{
			...
			vz::math::ConstructTriangleEquilateral(1, vertices[0].pos, vertices[1].pos, vertices[2].pos);
		...
		if (state == ACTIVE)
		{
			...
			if (HasScrollbar())
			{
				...
				{
					vz::Color col = vz::Color::fromFloat4(sprites[IDLE].params.color);
					if (combostate == COMBOSTATE_SCROLLBAR_HOVER)
					{
						col = vz::Color::fromFloat4(sprites[FOCUS].params.color);
					}
					else if (combostate == COMBOSTATE_SCROLLBAR_GRABBED)
					{
						col = vz::Color::fromFloat4(sprites[ACTIVE].params.color);
					}
					vz::image::Draw(
						vz::texturehelper::getWhite(),
						vz::image::Params(
	...
	{
		vz::helper::StringConvert(text, invalid_selection_text);
	}
	...
	void ComboBox::SetColor(vz::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	void ComboBox::SetTheme(const Theme& theme, int id)
	{
		...
		if (id == WIDGET_ID_COMBO_DROPDOWN)
		{
			drop_color = vz::Color::fromFloat4(theme.image.color);
		}
		theme.font.Apply(selected_font.params);
	}
	void ComboBox::ExportLocalization(vz::Localization& localization) const
	{
		Widget::ExportLocalization(localization);
		if (has_flag(localization_enabled, LocalizationEnabled::Items))
		{
			vz::Localization& section = localization.GetSection(GetName()).GetSection("items");
	...
	void ComboBox::ImportLocalization(const vz::Localization& localization)
	{
	...
	void Window::Create(const std::string& name, WindowControls window_controls)
	{
		...
		if (has_flag(window_controls, WindowControls::RESIZE_TOPLEFT))
		{
			...
			resizeDragger_UpperLeft.OnDrag([this](EventArgs args) {
				...
				this->scale_local = vz::math::Max(this->scale_local, XMFLOAT3(control_size * 3, control_size * 2, 1)); // don't allow resize to negative or too small
		...
		if (has_flag(window_controls, WindowControls::MOVE))
		{
			...
			moveDragger.font.params.h_align = vz::font::WIFALIGN_LEFT;
		...
		if (!has_flag(window_controls, WindowControls::MOVE))
		{
			...
			label.SetText(name);
			label.font.params.h_align = vz::font::WIFALIGN_LEFT;
			AddWidget(&label, AttachmentOptions::NONE);
		}

		scrollbar_horizontal.SetVertical(false);
		scrollbar_horizontal.SetColor(vz::Color(80, 80, 80, 100), vz::gui::IDLE);
		scrollbar_horizontal.sprites_knob[ScrollBar::SCROLLBAR_INACTIVE].params.color = vz::Color(140, 140, 140, 140);
		scrollbar_horizontal.sprites_knob[ScrollBar::SCROLLBAR_HOVER].params.color = vz::Color(180, 180, 180, 180);
		scrollbar_horizontal.sprites_knob[ScrollBar::SCROLLBAR_GRABBED].params.color = vz::Color::White();
		scrollbar_horizontal.knob_inset_border = XMFLOAT2(2, 4);
		AddWidget(&scrollbar_horizontal);

		scrollbar_vertical.SetVertical(true);
		scrollbar_vertical.SetColor(vz::Color(80, 80, 80, 100), vz::gui::IDLE);
		scrollbar_vertical.sprites_knob[ScrollBar::SCROLLBAR_INACTIVE].params.color = vz::Color(140, 140, 140, 140);
		scrollbar_vertical.sprites_knob[ScrollBar::SCROLLBAR_HOVER].params.color = vz::Color(180, 180, 180, 180);
		scrollbar_vertical.sprites_knob[ScrollBar::SCROLLBAR_GRABBED].params.color = vz::Color::White();
	...
	void Window::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (parent == nullptr)
		{
			translation_local.x = vz::math::Clamp(translation_local.x, 0, canvas.GetLogicalWidth() - scale_local.x);
			translation_local.y = vz::math::Clamp(translation_local.y, 0, canvas.GetLogicalHeight() - control_size);
			SetDirty();
		}
		...
		{
			float scroll = vz::input::GetPointer().z * 20;
	...
	void Window::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		//GraphicsDevice* device = vz::graphics::GetDevice();
		//device->BindScissorRects(1, &scissorRect, cmd);
		//vz::image::Draw(nullptr, vz::image::Params(scrollable_area.active_area.pos.x, scrollable_area.active_area.pos.y, scrollable_area.active_area.siz.x, scrollable_area.active_area.siz.y, vz::Color(255,0,255,100)), cmd);
		//Hitbox2D p = scrollable_area.GetPointerHitbox();
		//vz::image::Draw(nullptr, vz::image::Params(p.pos.x, p.pos.y, p.siz.x * 10, p.siz.y * 10, vz::Color(255,0,0,100)), cmd);
		//if (!IsCollapsed())
		//{
		//	vz::image::Draw(nullptr, vz::image::Params(scrollable_area.translation.x, scrollable_area.translation.y, scale.x, 10, vz::Color(255,0,255,100)), cmd);
		//}

		GetDevice()->EventEnd(cmd);
	}
	void Window::RenderTooltip(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const
	{
	...
	void Window::SetColor(vz::Color color, int id)
	{
		Widget::SetColor(color, id);
		for (auto& widget : widgets)
		{
	...
	void Window::SetShadowColor(vz::Color color)
	{
		Widget::SetShadowColor(color);
		for (auto& widget : widgets)
		{
	...
	void Window::ExportLocalization(vz::Localization& localization) const
	{
		Widget::ExportLocalization(localization);
	...
	void Window::ImportLocalization(const vz::Localization& localization)
	{
		Widget::ImportLocalization(localization);
		if (has_flag(localization_enabled, LocalizationEnabled::Children))
		{
	...
	void ColorPicker::Create(const std::string& name, WindowControls window_controls)
	{
		...
		SetColor(vz::Color(100, 100, 100, 100));
		...
		text_H.OnInputAccepted([this](EventArgs args) {
			hue = vz::math::Clamp(args.fValue, 0, 360.0f);
			FireEvents();
			});
		AddWidget(&text_H);
		...
		text_S.OnInputAccepted([this](EventArgs args) {
			saturation = vz::math::Clamp(args.fValue / 100.0f, 0, 1);
			FireEvents();
			});
		AddWidget(&text_S);
		...
		text_V.OnInputAccepted([this](EventArgs args) {
			luminance = vz::math::Clamp(args.fValue / 100.0f, 0, 1);
			FireEvents();
			});
		AddWidget(&text_V);
		...
		text_hex.OnInputAccepted([this](EventArgs args) {
			vz::Color color(args.sValue.c_str());
			SetPickColor(color);
			FireEvents();
			});
		AddWidget(&text_hex);
	...
	void ColorPicker::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			XMFLOAT2 pointer = GetPointerHitbox().pos;
			float distance = vz::math::Distance(center, pointer);
			bool hover_hue = (distance > colorpicker_radius * sca) && (distance < (colorpicker_radius + colorpicker_width)* sca);

			float distTri = 0;
			XMFLOAT4 A, B, C;
			vz::math::ConstructTriangleEquilateral(colorpicker_radius_triangle, A, B, C);
			...
			{
				//hue pick
				const float angle = vz::math::GetAngle(XMFLOAT2(pointer.x - center.x, pointer.y - center.y), XMFLOAT2(colorpicker_radius, 0));
			...
			else if (colorpickerstate == CPS_SATURATION && dragged)
			{
				// saturation pick
				float u, v, w;
				vz::math::GetBarycentric(O, _A, _B, _C, u, v, w, true);
	...
	void ColorPicker::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		...
		static vz::graphics::GPUBuffer vb_hue;
		static vz::graphics::GPUBuffer vb_picker_saturation;
		static vz::graphics::GPUBuffer vb_picker_hue;
		static vz::graphics::GPUBuffer vb_preview;

		static vz::vector<Vertex> vertices_saturation;
		...
		if (!buffersComplete)
		{
			...
			// saturation
			{
				...
				vz::math::ConstructTriangleEquilateral(colorpicker_radius_triangle, vertices_saturation[0].pos, vertices_saturation[1].pos, vertices_saturation[2].pos);
				...
				vertices_saturation.push_back(vertices_saturation[0]); // outer
				vertices_saturation.push_back(vertices_saturation[0]); // inner
				vz::math::ConstructTriangleEquilateral(colorpicker_radius_triangle + 4, vertices_saturation[3].pos, vertices_saturation[5].pos, vertices_saturation[7].pos); // extrude outer
		...
		const vz::Color final_color = GetPickColor();
		...
		if (IsEnabled())
		{
			...
			float u, v, w;
			vz::math::GetBarycentric(inner_point, color_corner, white_corner, black_corner, u, v, w, true);
	...
	{
		vz::gui::Window::ResizeLayout();
		const float padding = 4;
		const float width = GetWidgetAreaSize().x;
		float y = GetWidgetAreaSize().y - control_size;
		float jump = 20;

		auto add = [&](vz::gui::Widget& widget) {
			if (!widget.IsVisible())
				return;
		...
		auto add_right = [&](vz::gui::Widget& widget) {
			if (!widget.IsVisible())
				return;
	...
	vz::Color ColorPicker::GetPickColor() const
	{
		hsv source;
		source.h = hue;
		source.s = saturation;
		source.v = luminance;
		rgb result = hsv2rgb(source);
		return vz::Color::fromFloat4(XMFLOAT4(result.r, result.g, result.b, alphaSlider.GetValue() / 255.0f));
	}
	void ColorPicker::SetPickColor(vz::Color value)
	{
		if (colorpickerstate != CPS_IDLE)
		{
	...
	void TreeList::Create(const std::string& name)
	{
		...
		SetColor(vz::Color(100, 100, 100, 100), vz::gui::IDLE);
	...
	void TreeList::Update(const vz::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (onDelete && state == FOCUS && vz::input::Press(vz::input::KEYBOARD_BUTTON_DELETE))
			{
				int index = 0;
				for (auto& item : items)
				{
			...
			float scroll = vz::input::GetPointer().z * 10;
				...
				for (Item& item : items)
				{
					...
					else
					{
						...
						if (name_box.intersects(pointerHitbox))
						{
							...
							{
								if (!vz::input::Down(vz::input::KEYBOARD_BUTTON_LCONTROL) && !vz::input::Down(vz::input::KEYBOARD_BUTTON_RCONTROL)
									&& !vz::input::Down(vz::input::KEYBOARD_BUTTON_LSHIFT) && !vz::input::Down(vz::input::KEYBOARD_BUTTON_RSHIFT))
								{
									ClearSelection();
								}
								Select(i);
	...
	void TreeList::Render(const vz::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		for (const Item& item : items)
		{
			...
			{
				vz::image::Draw(nullptr
					, vz::image::Params(name_box.pos.x, name_box.pos.y, name_box.siz.x, name_box.siz.y,
			...
			if(DoesItemHaveChildren(i))
			{
				...
				MiscCB cb;
				cb.g_xColor = opener_highlight == i ? vz::Color::White().toFloat4() : sprites[FOCUS].params.color;
	...
	void TreeList::SetColor(vz::Color color, int id)
	{
		Widget::SetColor(color, id);
		scrollbar.SetColor(color, id);
	}
