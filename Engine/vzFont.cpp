...
namespace vz::font
{
	namespace font_internal
	{
		...
		static thread_local vz::Canvas canvas;
		...
		{
			std::string name;
			vz::vector<uint8_t> fontBuffer; // only used if loaded from file, need to keep alive
			...
			void Create(const std::string& newName, const uint8_t* data, size_t size)
			{
				...
				{
					vz::backlog::post("Failed to load font: " + name + " (file was unrecognized, it must be a .ttf file)");
				}
			...
			{
				if (vz::helper::FileRead(newName, fontBuffer))
				{
					Create(newName, fontBuffer.data(), fontBuffer.size());
				}
				else
				{
					vz::backlog::post("Failed to load font: " + name + " (file could not be opened)");
				}
			}
		};
		static vz::vector<std::unique_ptr<FontStyle>> fontStyles;
		...
		static vz::unordered_map<int32_t, Glyph> glyph_lookup;
		static vz::unordered_map<int32_t, vz::rectpacker::Rect> rect_lookup;
		struct Bitmap
		{
			int width;
			int height;
		...
		static vz::unordered_map<int32_t, Bitmap> bitmap_lookup;
		union GlyphHash
		{
			struct
			{
		...
		static vz::unordered_set<uint32_t> pendingGlyphs;
		...
		static thread_local vz::vector<FontVertex> vertexList;
		...
		ParseStatus ParseText(const char* text, size_t text_length, const Params& params)
		{
			...
			vz::helper::StringConvert(char_temp_buffer, wchar_temp_buffer);
	...
	void LoadShaders()
	{
		vz::renderer::LoadShader(ShaderStage::VS, vertexShader, "fontVS.cso");
		vz::renderer::LoadShader(ShaderStage::PS, pixelShader, "fontPS.cso");
		...
		for (int d = 0; d < DEPTH_TEST_MODE_COUNT; ++d)
		{
			...
			vz::graphics::GetDevice()->CreatePipelineState(&desc, &PSO[d]);
		}
	}
	void Initialize()
	{
		...
		static vz::eventhandler::Handle handle1 = vz::eventhandler::Subscribe(vz::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); });
		LoadShaders();

		vz::backlog::post("vz::font Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
	...
	void UpdateAtlas(float upscaling)
	{
		...
		if (!pendingGlyphs.empty())
		{
			...
			static thread_local vz::rectpacker::State packer;
			packer.clear();
			...
			if (packer.pack(4096))
			{
				...
				vz::vector<uint8_t> atlas(size_t(atlasWidth) * size_t(atlasHeight));
				...
				vz::texturehelper::CreateTexture(texture, atlas.data(), atlasWidth, atlasHeight, Format::R8_UNORM);
				GetDevice()->SetName(&texture, "vz::font::texture");
			}
			else
			{
	...
	Cursor Draw_internal(const T* text, size_t text_length, const Params& params, CommandList cmd)
	{
		...
		if (status.quadCount > 0)
		{
			...
			{
				// Asserts will check that a proper canvas was set for this cmd with vz::image::SetCanvas()
	...
	void SetCanvas(const vz::Canvas& current_canvas)
	{
