...
namespace vz::image
{
	...
	void Draw(const Texture* texture, const Params& params, CommandList cmd)
	{
			...
			if (params.isCornerRoundingEnabled())
			{
				...
				for (int i = 0; i < arraysize(params.corners_rounding); ++i)
				{
					...
					for (int j = 0; j < rounding.segments; ++j)
					{
						...
						XMVECTOR bezier = vz::math::GetQuadraticBezierPos(start, B, end, t);
		...
		{
			stencilRef = vz::renderer::CombineStencilrefs(STENCILREF_EMPTY, (uint8_t)stencilRef);
		}
	...
	void LoadShaders()
	{
		vz::renderer::LoadShader(ShaderStage::VS, vertexShader, "imageVS.cso");
		vz::renderer::LoadShader(ShaderStage::PS, pixelShader, "imagePS.cso");
	...
	void Initialize()
	{
		...
		vz::backlog::post("vz::image Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
