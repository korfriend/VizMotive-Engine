...
namespace vz
{
	struct ResourceInternal
	{
		...
		vz::audio::Sound sound;
		std::string script;
		vz::video::Video video;
		vz::vector<uint8_t> filedata;
		int font_style = -1;
	};

	const vz::vector<uint8_t>& Resource::GetFileData() const
	{
	...
	const vz::audio::Sound& Resource::GetSound() const
	{
		const ResourceInternal* resourceinternal = (ResourceInternal*)internal_state.get();
	...
	const vz::video::Video& Resource::GetVideo() const
	{
		const ResourceInternal* resourceinternal = (ResourceInternal*)internal_state.get();
	...
	void Resource::SetFileData(const vz::vector<uint8_t>& data)
	{
	...
	void Resource::SetFileData(vz::vector<uint8_t>&& data)
	{
		if (internal_state == nullptr)
		{
			internal_state = std::make_shared<ResourceInternal>();
	...
	void Resource::SetSound(const vz::audio::Sound& sound)
	{
		if (internal_state == nullptr)
		{
			internal_state = std::make_shared<ResourceInternal>();
	...
	void Resource::SetVideo(const vz::video::Video& video)
	{
		if (internal_state == nullptr)
		{
			internal_state = std::make_shared<ResourceInternal>();
	...
	namespace resourcemanager
	{
		...
		static const vz::unordered_map<std::string, DataType> types = {
		...
		vz::vector<std::string> GetSupportedImageExtensions()
		{
			vz::vector<std::string> ret;
			for (auto& x : types)
			{
		...
		vz::vector<std::string> GetSupportedSoundExtensions()
		{
			vz::vector<std::string> ret;
			for (auto& x : types)
			{
		...
		vz::vector<std::string> GetSupportedVideoExtensions()
		{
			vz::vector<std::string> ret;
			for (auto& x : types)
			{
		...
		vz::vector<std::string> GetSupportedScriptExtensions()
		{
			vz::vector<std::string> ret;
			for (auto& x : types)
			{
		...
		vz::vector<std::string> GetSupportedFontStyleExtensions()
		{
			vz::vector<std::string> ret;
			for (auto& x : types)
			{
		...
		Resource Load(const std::string& name, Flags flags, const uint8_t* filedata, size_t filesize)
		{
			...
			{
				if (resource->filedata.empty() && !vz::helper::FileRead(name, resource->filedata))
				{
					resource.reset();
					return Resource();
				}
					...
					if (!ext.compare("KTX2"))
					{
						...
						if (transcoder.init(filedata, (uint32_t)filesize))
						{
							...
							if (transcoder.start_transcoding())
							{
								...
								vz::vector<uint8_t> transcoded_data(transcoded_data_size);
								
								vz::vector<SubresourceData> InitData;
		...
		void Serialize_READ(vz::Archive& archive, ResourceSerializer& seri)
		{
			...
			vz::vector<TempResource> temp_resources;
			...
			for (size_t i = 0; i < serializable_count; ++i)
			{
				...
				vz::jobsystem::Execute(ctx, [i, &temp_resources, &seri_locker, &seri](vz::jobsystem::JobArgs args) {
		...
		void Serialize_WRITE(vz::Archive& archive, const vz::unordered_set<std::string>& resource_names)
		{
			assert(!archive.IsReadMode());
