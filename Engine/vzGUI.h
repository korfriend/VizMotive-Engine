...
namespace vz::gui
{
	...
	struct EventArgs
	{
		...
		vz::Color color;		// color value of color picker operation
	...
	struct Theme
	{
		// Reduced version of vz::image::Params, excluding position, alignment, etc.
		struct Image
		{
			inline static const vz::image::Params params; // prototype for default values
			XMFLOAT4 color = params.color;
			vz::enums::BLENDMODE blendFlag = params.blendFlag;
			vz::image::SAMPLEMODE sampleFlag = params.sampleFlag;
			vz::image::QUALITY quality = params.quality;
			bool background = params.isBackgroundEnabled();
			bool corner_rounding = params.isCornerRoundingEnabled();
			vz::image::Params::Rounding corners_rounding[arraysize(params.corners_rounding)];

			void Apply(vz::image::Params& params) const
			{
				params.color = color;
			...
			void CopyFrom(const vz::image::Params& params)
			{
				color = params.color;
		...
		// Reduced version of vz::font::Params, excluding position, alignment, etc.
		struct Font
		{
			inline static const vz::font::Params params; // prototype for default values
			vz::Color color = params.color;
			vz::Color shadow_color = params.shadowColor;
			...
			void Apply(vz::font::Params& params) const
			{
				params.color = color;
			...
			void CopyFrom(const vz::font::Params& params)
			{
				color = params.color;
		...
		vz::Color shadow_color = vz::Color::Shadow(); // shadow color for whole widget

		Image tooltipImage;
		Font tooltipFont;
		vz::Color tooltip_shadow_color = vz::Color::Shadow();
	};
	...
	{
	private:
		vz::vector<Widget*> widgets;
		bool focus = false;
		bool visible = true;
	public:

		void Update(const vz::Canvas& canvas, float dt);
		void Render(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const;
		...
		void SetColor(vz::Color color, int id = -1);
		void SetShadowColor(vz::Color color);
		void SetTheme(const Theme& theme, int id = -1);

		void ExportLocalization(vz::Localization& localization) const;
		void ImportLocalization(const vz::Localization& localization);
	};

	class Widget : public vz::scene::TransformComponent
	{
	private:
		int tooltipTimer = 0;
	protected:
		...
		vz::Color shadow_color = vz::Color::Shadow();
		WIDGETSTATE state = IDLE;
		float tooltip_shadow = 1; // shadow radius
		vz::Color tooltip_shadow_color = vz::Color::Shadow();
		mutable vz::Sprite tooltipSprite;
		mutable vz::SpriteFont tooltipFont;
		mutable vz::SpriteFont scripttipFont;
	...
	public:
		...
		vz::Color GetColor() const;
		float GetShadowRadius() const { return shadow; }
		void SetShadowRadius(float value) { shadow = value; }

		virtual void ResizeLayout() {};
		virtual void Update(const vz::Canvas& canvas, float dt);
		virtual void Render(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const {}
		virtual void RenderTooltip(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const;
		...
		virtual void SetColor(vz::Color color, int id = -1);
		virtual void SetShadowColor(vz::Color color);
		virtual void SetImage(vz::Resource textureResource, int id = -1);
		virtual void SetTheme(const Theme& theme, int id = -1);
		virtual const char* GetWidgetTypeName() const { return "Widget"; }

		vz::Sprite sprites[WIDGETSTATE_COUNT];
		vz::SpriteFont font;

		XMFLOAT3 translation = XMFLOAT3(0, 0, 0);
		XMFLOAT3 scale = XMFLOAT3(1, 1, 1);

		vz::primitive::Hitbox2D hitBox;
		...
		void ApplyScissor(const vz::Canvas& canvas, const vz::graphics::Rect rect, vz::graphics::CommandList cmd, bool constrain_to_parent = true) const;
		vz::primitive::Hitbox2D GetPointerHitbox(bool constrained = true) const;

		vz::primitive::Hitbox2D active_area; // Pointer hitbox constrain area
		void HitboxConstrain(vz::primitive::Hitbox2D& hb) const;
		...
		virtual void ExportLocalization(vz::Localization& localization) const;
		virtual void ImportLocalization(const vz::Localization& localization);
	};
	...
	public:
		...
		vz::SpriteFont font_description;
		void SetDescription(const std::string& desc) { font_description.SetText(desc); }

		void Update(const vz::Canvas& canvas, float dt) override;
		void Render(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const override;
	...
	public:
		...
		vz::Sprite sprites_knob[SCROLLBAR_STATE_COUNT];
		XMFLOAT2 knob_inset_border = {};

		void Update(const vz::Canvas& canvas, float dt) override;
		void Render(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const override;
		void SetColor(vz::Color color, int id = -1) override;
		void SetTheme(const Theme& theme, int id = -1) override;
	...
	protected:
		...
		static vz::SpriteFont font_input;
	...
	public:
		...
		vz::Sprite sprites_knob[WIDGETSTATE_COUNT];
		...
		void Update(const vz::Canvas& canvas, float dt) override;
		void Render(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const override;
		void RenderTooltip(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const override;
		void SetColor(vz::Color color, int id = -1) override;
		void SetTheme(const Theme& theme, int id = -1) override;
	...
	protected:
		...
		vz::vector<Item> items;

		vz::Color drop_color = vz::Color::Ghost();
		std::wstring invalid_selection_text;

		float GetDropOffset(const vz::Canvas& canvas) const;
		float GetItemOffset(const vz::Canvas& canvas, int index) const;
	public:
		void Create(const std::string& name);
		...
		vz::SpriteFont selected_font;

		void ExportLocalization(vz::Localization& localization) const override;
		void ImportLocalization(const vz::Localization& localization) override;
	};
	...
	public:
		...
		void RenderTooltip(const vz::Canvas& canvas, vz::graphics::CommandList cmd) const override;
		void SetColor(vz::Color color, int id = -1) override;
		void SetShadowColor(vz::Color color) override;
		void SetTheme(const Theme& theme, int id = -1) override;
	...
	public:
		...
		vz::Color GetPickColor() const;
		void SetPickColor(vz::Color value);
	...
	protected:
		...
		vz::primitive::Hitbox2D GetHitbox_ListArea() const;
		vz::primitive::Hitbox2D GetHitbox_Item(int visible_count, int level) const;
		vz::primitive::Hitbox2D GetHitbox_ItemOpener(int visible_count, int level) const;
...
template<>
struct enable_bitmask_operators<vz::gui::Window::WindowControls> {
	static const bool enable = true;
};

template<>
struct enable_bitmask_operators<vz::gui::Window::AttachmentOptions> {
	static const bool enable = true;
};

template<>
struct enable_bitmask_operators<vz::gui::LocalizationEnabled> {
	static const bool enable = true;
};
