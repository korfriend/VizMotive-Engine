...
namespace vz::graphics
{
	...
	protected:
		...
		vz::vector<VkQueueFamilyProperties2> queueFamilies;
		vz::vector<VkQueueFamilyVideoPropertiesKHR> queueFamiliesVideo;
		uint32_t graphicsFamily = VK_QUEUE_FAMILY_IGNORED;
		uint32_t computeFamily = VK_QUEUE_FAMILY_IGNORED;
		uint32_t copyFamily = VK_QUEUE_FAMILY_IGNORED;
		uint32_t videoFamily = VK_QUEUE_FAMILY_IGNORED;
		vz::vector<uint32_t> families;
		...
		vz::vector<VkDynamicState> pso_dynamicStates;
		...
		struct CommandQueue
		{
			...
			vz::vector<SwapChain> swapchain_updates;
			vz::vector<VkSwapchainKHR> submit_swapchains;
			vz::vector<uint32_t> submit_swapChainImageIndices;
			vz::vector<VkSemaphoreSubmitInfo> submit_waitSemaphoreInfos;
			vz::vector<VkSemaphore> submit_signalSemaphores;
			vz::vector<VkSemaphoreSubmitInfo> submit_signalSemaphoreInfos;
			vz::vector<VkCommandBufferSubmitInfo> submit_cmds;
		...
		struct DescriptorBinder
		{
			...
			vz::vector<VkWriteDescriptorSet> descriptorWrites;
			vz::vector<VkDescriptorBufferInfo> bufferInfos;
			vz::vector<VkDescriptorImageInfo> imageInfos;
			vz::vector<VkBufferView> texelBufferViews;
			vz::vector<VkWriteDescriptorSetAccelerationStructureKHR> accelerationStructureViews;
		...
		struct CommandList_Vulkan
		{
			...
			vz::vector<std::pair<size_t, VkPipeline>> pipelines_worker;
			...
			vz::vector<SwapChain> prev_swapchains;
			bool dirty_pso = {};
			vz::vector<VkMemoryBarrier2> frame_memoryBarriers;
			vz::vector<VkImageMemoryBarrier2> frame_imageBarriers;
			vz::vector<VkBufferMemoryBarrier2> frame_bufferBarriers;
			vz::vector<VkAccelerationStructureGeometryKHR> accelerationstructure_build_geometries;
			vz::vector<VkAccelerationStructureBuildRangeInfoKHR> accelerationstructure_build_ranges;
			RenderPassInfo renderpass_info;
			vz::vector<VkImageMemoryBarrier2> renderpass_barriers_begin;
			vz::vector<VkImageMemoryBarrier2> renderpass_barriers_end;
		...
		vz::vector<std::unique_ptr<CommandList_Vulkan>> commandlists;
		...
		mutable vz::unordered_map<size_t, PSOLayout> pso_layout_cache;
		mutable std::mutex pso_layout_cache_mutex;

		VkPipelineCache pipelineCache = VK_NULL_HANDLE;
		vz::unordered_map<size_t, VkPipeline> pipelines_global;
		...
		vz::vector<VkSampler> immutable_samplers;

	public:
		GraphicsDevice_Vulkan(vz::platform::window_type window, ValidationMode validationMode = ValidationMode::Disabled, GPUPreference preference = GPUPreference::Discrete);
		...
		void* OpenSharedResource(const void* device2, Texture* texture) override;
		void Test(Texture* texture) override;
