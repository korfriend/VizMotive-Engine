...
namespace vz::scene
{
	...
	void Scene::Update(float dt)
	{
		...
		if(vz::renderer::GetOcclusionCullingEnabled() && !vz::renderer::GetFreezeCullingCameraEnabled())
		{
		...
		if (dt > 0)
		{
			...
			vz::jobsystem::Dispatch(ctx, (uint32_t)objects.GetCount(), small_subtask_groupsize, [this](vz::jobsystem::JobArgs args) {
			...
			vz::jobsystem::Dispatch(ctx, (uint32_t)meshes.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
			...
			vz::jobsystem::Dispatch(ctx, (uint32_t)armatures.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
		...
		vz::physics::RunPhysicsUpdateSystem(ctx, *this, dt);

		RunTransformUpdateSystem(ctx);

		vz::jobsystem::Wait(ctx); // dependencies
		...
		if (vz::renderer::GetSurfelGIEnabled())
		{
...

		{
			...
			{
				vz::VoxelGrid& voxelgrid = voxel_grids[voxelgridIndex];
	...
	void Scene::Merge(Scene& other)
	{
		...
		collider_deinterleaved_data.reserve(
			sizeof(vz::primitive::AABB) * colliders.GetCount() +
			sizeof(ColliderComponent) * colliders.GetCount() +
			sizeof(ColliderComponent) * colliders.GetCount()
		);
		aabb_colliders_cpu = (vz::primitive::AABB*)collider_deinterleaved_data.data();
	...
	void Scene::FindAllEntities(vz::unordered_set<vz::ecs::Entity>& entities) const
	{
		for (auto& entry : componentLibrary.entries)
		{
	...
	void Scene::Entity_Remove(Entity entity, bool recursive, bool keep_sorted)
	{
		if (recursive)
		{
			vz::vector<Entity> entities_to_remove;
	...
	bool Scene::Entity_IsDescendant(vz::ecs::Entity entity, vz::ecs::Entity ancestor) const
	{
	...
	)
	{
		...
		if (!filename.empty())
		{
			...
			sound.soundResource = vz::resourcemanager::Load(filename, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
			vz::audio::CreateSoundInstance(&sound.soundResource.GetSound(), &sound.soundinstance);
		}
	...
	)
	{
		...
		if (!filename.empty())
		{
			...
			video.videoResource = vz::resourcemanager::Load(filename, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
			vz::video::CreateVideoInstance(&video.videoResource.GetVideo(), &video.videoinstance);
		}
	...
	void Scene::RunAnimationUpdateSystem(vz::jobsystem::context& ctx)
	{
		auto range = vz::profiler::BeginRangeCPU("Animations");

		vz::jobsystem::Wait(animation_dependency_scan_workload);

		vz::jobsystem::Dispatch(ctx, (uint32_t)animation_queue_count, 1, [&](vz::jobsystem::JobArgs args) {
				...
				for (const AnimationComponent::AnimationChannel& channel : animation.channels)
				{
					...
					else
					{
						...
						case AnimationComponent::AnimationSampler::Mode::STEP:
						{
							...
							const int key = vz::math::InverseLerp(timeLeft, timeRight, animation.timer) > 0.5f ? keyRight : keyLeft;
						...
						case AnimationComponent::AnimationSampler::Mode::LINEAR:
						{
							...
							case AnimationComponent::AnimationChannel::PathDataType::Float:
							{
								...
								float vAnim = vz::math::Lerp(vLeft, vRight, t);
					...
					const bool isRootBone = (animation.IsRootMotion() && animation.rootMotionBone != vz::ecs::INVALID_ENTITY && (target_transform == transforms.GetComponent(animation.rootMotionBone)));
					...
					if (target_mesh != nullptr)
					{
						...
						{
							target_mesh->morph_targets[j].weight = vz::math::Lerp(target_mesh->morph_targets[j].weight, animation.morph_weights_temp[j], t);
						}
					}
					...
					if (target_light != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::LIGHT_COLOR:
						{
							target_light->color = vz::math::Lerp(target_light->color, interpolator.f3, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_INTENSITY:
						{
							target_light->intensity = vz::math::Lerp(target_light->intensity, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_RANGE:
						{
							target_light->range = vz::math::Lerp(target_light->range, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_INNERCONE:
						{
							target_light->innerConeAngle = vz::math::Lerp(target_light->innerConeAngle, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_OUTERCONE:
						{
							target_light->outerConeAngle = vz::math::Lerp(target_light->outerConeAngle, interpolator.f, t);
						}
						break;
						default:
							break;
					...
					if (target_sound != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::SOUND_VOLUME:
						{
							target_sound->volume = vz::math::Lerp(target_sound->volume, interpolator.f, t);
						}
						break;
						default:
							break;
					...
					if (target_emitter != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::EMITTER_EMITCOUNT:
						{
							target_emitter->count = vz::math::Lerp(target_emitter->count, interpolator.f, t);
						}
						break;
						default:
							break;
					...
					if (target_camera != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::CAMERA_FOV:
						{
							target_camera->fov = vz::math::Lerp(target_camera->fov, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::CAMERA_FOCAL_LENGTH:
						{
							target_camera->focal_length = vz::math::Lerp(target_camera->focal_length, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SIZE:
						{
							target_camera->aperture_size = vz::math::Lerp(target_camera->aperture_size, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SHAPE:
						{
							target_camera->aperture_shape = vz::math::Lerp(target_camera->aperture_shape, interpolator.f2, t);
						}
						break;
						default:
							break;
					...
					if (target_material != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::MATERIAL_COLOR:
						{
							target_material->baseColor = vz::math::Lerp(target_material->baseColor, interpolator.f4, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_EMISSIVE:
						{
							target_material->emissiveColor = vz::math::Lerp(target_material->emissiveColor, interpolator.f4, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_ROUGHNESS:
						{
							target_material->roughness = vz::math::Lerp(target_material->roughness, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_METALNESS:
						{
							target_material->metalness = vz::math::Lerp(target_material->metalness, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_REFLECTANCE:
						{
							target_material->reflectance = vz::math::Lerp(target_material->reflectance, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_TEXMULADD:
						{
							target_material->texMulAdd = vz::math::Lerp(target_material->texMulAdd, interpolator.f4, t);
						}
						break;
						default:
							break;
	...
	void Scene::RunTransformUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::jobsystem::Dispatch(ctx, (uint32_t)transforms.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {

			TransformComponent& transform = transforms[args.jobIndex];
			transform.UpdateTransform();
		});
	}
	void Scene::RunHierarchyUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::jobsystem::Dispatch(ctx, (uint32_t)hierarchy.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
	...
	void Scene::RunExpressionUpdateSystem(vz::jobsystem::context& ctx)
	{
		for (size_t i = 0; i < expressions.GetCount(); ++i)
		{
			...
			if (expression_mastering.blink_timer >= 1)
			{
				...
				if (blink >= 0 && blink < expression_mastering.expressions.size())
				{
					...
					float blink_index = std::floor(vz::math::Lerp(0, (float)expression_mastering.blink_count, (expression_mastering.blink_timer - 1) / all_blink_length));
					float blink_trim = 1 + one_blink_length * blink_index;
					float blink_state = vz::math::InverseLerp(0, one_blink_length, expression_mastering.blink_timer - blink_trim);
					if (blink_state < 0.5f)
					{
						// closing
						expression.weight = vz::math::Lerp(0, 1, vz::math::saturate(blink_state * 2));
					}
					else
					{
						// opening
						expression.weight = vz::math::Lerp(1, 0, vz::math::saturate((blink_state - 0.5f) * 2));
					}
					if (expression_mastering.blink_timer >= 1 + all_blink_length)
					{
						expression.weight = 0;
						expression_mastering.blink_timer = -vz::random::GetRandom(0.0f, 1.0f);
					}
					expression.SetDirty();
				}
			}
			...
			if (expression_mastering.look_timer == 0)
			{
				...
				float vertical = vz::random::GetRandom(-1.0f, 1.0f);
				float horizontal = vz::random::GetRandom(-1.0f, 1.0f);
				expression_mastering.look_weights[0] = vz::math::saturate(vertical);
				expression_mastering.look_weights[1] = vz::math::saturate(-vertical);
				expression_mastering.look_weights[2] = vz::math::saturate(horizontal);
				expression_mastering.look_weights[3] = vz::math::saturate(-horizontal);
			}
			...
			if (expression_mastering.look_timer >= 1)
			{
				...
				for (int idx = 0; idx<arraysize(looks); ++idx)
				{
					...
					if (look >= 0 && look < expression_mastering.expressions.size())
					{
						...
						float look_state = vz::math::InverseLerp(0, expression_mastering.look_length * expression_mastering.look_frequency, expression_mastering.look_timer - 1);
						if (look_state < 0.25f)
						{
							expression.weight = vz::math::Lerp(0, weight, vz::math::saturate(look_state * 4));
						}
						else
						{
							expression.weight = vz::math::Lerp(weight, 0, vz::math::saturate((look_state - 0.75f) * 4));
						}
						expression.SetDirty();
					}
				}
				...
				{
					expression_mastering.look_timer = -vz::random::GetRandom(0.0f, 1.0f);
				}
			}
			...
			if(voice_playing || expression_mastering.IsForceTalkingEnabled())
			{
				...
				for (int phoneme = (int)ExpressionComponent::Preset::Aa; phoneme <= (int)ExpressionComponent::Preset::Oh; phoneme++)
				{
					if (phoneme != (int)expression_mastering.talking_phoneme) // don't allow to select the current phoneme next
					{
						...
						expression.weight = vz::math::Lerp(expression.weight, 0, 0.4f); // fade out unused
						expression.SetDirty();
					}
				}
				...
				if (voice_playing)
				{
					...
					vz::audio::SampleInfo info = vz::audio::GetSampleInfo(&sound->soundResource.GetSound());
					uint32_t sample_frequency = info.sample_rate * info.channel_count;
					uint64_t current_sample = vz::audio::GetTotalSamplesPlayed(&sound->soundinstance);
					if (sound->IsLooped())
					{
					...
					if (voice > 0.1f)
					{
						expression.weight = vz::math::Lerp(expression.weight, 1, strength);
					}
					else
					{
						expression.weight = vz::math::Lerp(expression.weight, 0, strength);
					}
				}
				else
				{
				...
				if (prev_slope < 0 && curr_slope > 0)
				{
					...
					expression_mastering.talking_phoneme = unused_phonemes[vz::random::GetRandom(0, (int)arraysize(unused_phonemes) - 1)];
				}
			...
			else if (expression_mastering._flags & ExpressionComponent::TALKING_ENDED)
			{
				...
				for (auto& phoneme : phonemes)
				{
					...
					expression.weight = vz::math::Lerp(expression.weight, 0, 0.4f);
					expression.SetDirty();
			...
			for (int mouth : mouths)
			{
				...
				{
					ExpressionComponent::Expression& expression = expression_mastering.expressions[mouth];
					expression.weight *= 1 - vz::math::saturate(overrideMouthBlend);
				}
			}
			const int blinks[] = {
			...
			for (int blink : blinks)
			{
				...
				{
					ExpressionComponent::Expression& expression = expression_mastering.expressions[blink];
					expression.weight *= 1 - vz::math::saturate(overrideBlinkBlend);
				}
			}
			const int looks[] = {
				expression_mastering.presets[(int)ExpressionComponent::Preset::LookUp],
			...
			for (int look : looks)
			{
				...
				{
					ExpressionComponent::Expression& expression = expression_mastering.expressions[look];
					expression.weight *= 1 - vz::math::saturate(overrideLookBlend);
				}
			}
			...
			for (ExpressionComponent::Expression& expression : expression_mastering.expressions)
			{
				...
				for (const ExpressionComponent::Expression::MorphTargetBinding& morph_target_binding : expression.morph_target_bindings)
				{
					...
					{
						MeshComponent::MorphTarget& morph_target = mesh->morph_targets[morph_target_binding.index];
						morph_target.weight = vz::math::Lerp(morph_target.weight, morph_target_binding.weight, blend);
					}
				}
			}
		}
	}
	void Scene::RunProceduralAnimationUpdateSystem(vz::jobsystem::context& ctx)
	{
		if (dt <= 0)
			return;

		auto range = vz::profiler::BeginRangeCPU("Procedural Animations");
			...
			for (auto& source : sources)
			{
				...
				if (boneIndex < transforms_temp.size())
				{
					...
					if (humanoid.IsLookAtEnabled())
					{
						...
						const float angle_horizontal = vz::math::GetAngle(FORWARD, TARGET_HORIZONTAL, UP, source.rotation_max->x);
						const float angle_vertical = vz::math::GetAngle(FORWARD, TARGET_VERTICAL, SIDE, source.rotation_max->y);

						Q = XMQuaternionNormalize(XMQuaternionRotationRollPitchYaw(angle_vertical, angle_horizontal, 0));

#if 0
						vz::renderer::RenderableLine line;
						...
						vz::renderer::DebugTextParams textparams;
						textparams.flags |= vz::renderer::DebugTextParams::CAMERA_FACING;
						textparams.flags |= vz::renderer::DebugTextParams::CAMERA_SCALING;
						textparams.position = humanoid.lookAt;
						textparams.scaling = 0.8f;
						vz::renderer::DrawDebugText(text.c_str(), textparams);
#endif
					}
		...
		vz::jobsystem::Dispatch(ctx, (uint32_t)colliders.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
		...
		for (size_t i = 0; i < springs.GetCount(); ++i)
		{
			...
			vz::primitive::Sphere tail_sphere;
	...
	void Scene::RunArmatureUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::jobsystem::Dispatch(ctx, (uint32_t)armatures.GetCount(), 1, [&](vz::jobsystem::JobArgs args) {
			...
			for (Entity boneEntity : armature.boneCollection)
			{
				...
				_min = vz::math::Min(_min, boneAABB._min);
				_max = vz::math::Max(_max, boneAABB._max);
	...
	void Scene::RunMeshUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::jobsystem::Dispatch(ctx, (uint32_t)meshes.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
			...
			if (!mesh.streamoutBuffer.IsValid())
			{
				...
				if (softbody != nullptr && vz::physics::IsEnabled())
				{
	...
	void Scene::RunMaterialUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::jobsystem::Dispatch(ctx, (uint32_t)materials.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
	...
	void Scene::RunImpostorUpdateSystem(vz::jobsystem::context& ctx)
	{
		if (dt == 0)
			return;
	...
	void Scene::RunObjectUpdateSystem(vz::jobsystem::context& ctx)
	{
		...
		parallel_bounds.resize((size_t)vz::jobsystem::DispatchGroupCount((uint32_t)objects.GetCount(), small_subtask_groupsize));
		
		vz::jobsystem::Dispatch(ctx, (uint32_t)objects.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
			...
			if (!vz::renderer::GetFreezeCullingCameraEnabled())
			{
			...
			if (object.meshID != INVALID_ENTITY && meshes.Contains(object.meshID) && transforms.Contains(entity))
			{
				...
				{
					const float distsq = vz::math::DistanceSquared(camera.Eye, object.center);
				...
				union SortBits
				{
					struct
					{
						...
						uint32_t blendmode : vz::enums::BLENDMODE_COUNT;
				...
				inst.color = vz::math::CompressColor(object.color);
				inst.emissive = vz::math::Pack_R11G11B10_FLOAT(XMFLOAT3(object.emissiveColor.x * object.emissiveColor.w, object.emissiveColor.y * object.emissiveColor.w, object.emissiveColor.z * object.emissiveColor.w));
				...
				if (TLAS_instancesMapped != nullptr)
				{
					...
					{
						instance.instance_mask &= ~vz::renderer::raytracing_inclusion_mask_shadow;
					}
					if (object.IsNotVisibleInReflections())
					{
						instance.instance_mask &= ~vz::renderer::raytracing_inclusion_mask_reflection;
					}
				...
				if (object.IsLightmapRenderRequested() && dt > 0)
				{
					if (!object.lightmap.IsValid())
					{
						object.lightmapWidth = vz::math::GetNextPowerOfTwo(object.lightmapWidth + 1) / 2;
						object.lightmapHeight = vz::math::GetNextPowerOfTwo(object.lightmapHeight + 1) / 2;
				...
				if (!object.lightmapTextureData.empty() && !object.lightmap.IsValid())
				{
					...
					vz::texturehelper::CreateTexture(object.lightmap, object.lightmapTextureData.data(), object.lightmapWidth, object.lightmapHeight, object.lightmap.desc.format);
	...
	void Scene::RunCameraUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::jobsystem::Dispatch(ctx, (uint32_t)cameras.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
	...
	void Scene::RunDecalUpdateSystem(vz::jobsystem::context& ctx)
	{
	...
	void Scene::RunProbeUpdateSystem(vz::jobsystem::context& ctx)
	{
	...
	void Scene::RunForceUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::jobsystem::Dispatch(ctx, (uint32_t)forces.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
	...
	void Scene::RunLightUpdateSystem(vz::jobsystem::context& ctx)
	{
		aabb_lights.resize(lights.GetCount());

		vz::jobsystem::Dispatch(ctx, (uint32_t)lights.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
		...
		vz::jobsystem::Dispatch(ctx, (uint32_t)emitters.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
			...
			if (TLAS_instancesMapped != nullptr)
			{
				...
				for (int i = 0; i < arraysize(instance.transform); ++i)
				{
					for (int j = 0; j < arraysize(instance.transform[i]); ++j)
					{
						instance.transform[i][j] = vz::math::IDENTITY_MATRIX.m[j][i];
					}
				}
	...
	void Scene::RunWeatherUpdateSystem(vz::jobsystem::context& ctx)
	{
		if (weathers.GetCount() > 0)
		{
			...
			if (!vz::renderer::GetFreezeCullingCameraEnabled() && weather.IsOceanEnabled())
			{
		...
		{
			GraphicsDevice* device = vz::graphics::GetDevice();
			rainEmitter._flags |= vz::EmittedParticleSystem::FLAG_USE_RAIN_BLOCKER;
			rainEmitter.shaderType = vz::EmittedParticleSystem::PARTICLESHADERTYPE::SOFT_LIGHTING;
			...
			rainEmitter.count = vz::math::Lerp(0, (float)target_max_particle_count, weather.rain_amount);
			rainEmitter.life = 1;
			...
			{
				Texture gradientTex = vz::texturehelper::CreateGradientTexture(
					vz::texturehelper::GradientType::Circular,
					32, 32,
					XMFLOAT2(0.5f, 0.5f), XMFLOAT2(0.5f, 0),
					vz::texturehelper::GradientFlags::Smoothstep | vz::texturehelper::GradientFlags::Inverse
				);
				Texture gradientTexBC;
				TextureDesc desc = gradientTex.GetDesc();
				desc.format = Format::BC4_UNORM;
				desc.swizzle = { vz::graphics::ComponentSwizzle::ONE,vz::graphics::ComponentSwizzle::ONE,vz::graphics::ComponentSwizzle::ONE,vz::graphics::ComponentSwizzle::R };
				bool success = device->CreateTexture(&desc, nullptr, &gradientTexBC);
				assert(success);
				vz::renderer::AddDeferredBlockCompression(gradientTex, gradientTexBC);
	...
	void Scene::RunSoundUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::audio::SoundInstance3D instance3D;
		...
		for (size_t i = 0; i < sounds.GetCount(); ++i)
		{
			...
			if (!sound.IsDisable3D())
			{
				...
				if (transform != nullptr)
				{
					...
					vz::audio::Update3D(&sound.soundinstance, instance3D);
				}
			}
			if (sound.IsPlaying())
			{
				vz::audio::Play(&sound.soundinstance);
			}
			else
			{
				vz::audio::Stop(&sound.soundinstance);
			}
			vz::audio::SetVolume(sound.volume, &sound.soundinstance);
		}
	}
	void Scene::RunVideoUpdateSystem(vz::jobsystem::context& ctx)
	{
		for (size_t i = 0; i < videos.GetCount(); ++i)
		{
			...
			if (video.IsPlaying())
			{
				video.videoinstance.flags |= vz::video::VideoInstance::Flags::Playing;
			}
			else
			{
				video.videoinstance.flags &= ~vz::video::VideoInstance::Flags::Playing;
			...
			if (video.IsLooped())
			{
				video.videoinstance.flags |= vz::video::VideoInstance::Flags::Looped;
			}
			else
			{
				video.videoinstance.flags &= ~vz::video::VideoInstance::Flags::Looped;
			...
			video.videoinstance.flags |= vz::video::VideoInstance::Flags::Mipmapped;

		}
	}
	//void Scene::RunScriptUpdateSystem(vz::jobsystem::context& ctx)
	//{
	//	auto range = vz::profiler::BeginRangeCPU("Script Components");
	...
	//				vz::lua::AttachScriptParameters(str, script.filename, vz::lua::GeneratePID(), "local function GetEntity() return " + std::to_string(entity) + "; end;", "");
	//				vz::lua::CompileText(str, script.script);
	//			}
	//			if (!script.script.empty())
	//			{
	//				vz::lua::RunBinaryData(script.script.data(), script.script.size(), script.filename.c_str());
	//			}
	//
	...
	//	vz::profiler::EndRange(range);
	//}
	void Scene::RunSpriteUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::jobsystem::Dispatch(ctx, (uint32_t)sprites.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
	...
	void Scene::RunFontUpdateSystem(vz::jobsystem::context& ctx)
	{
		vz::jobsystem::Dispatch(ctx, (uint32_t)fonts.GetCount(), small_subtask_groupsize, [&](vz::jobsystem::JobArgs args) {
		...
		if (filterMask & FILTER_OBJECT_ALL)
		{
			for (size_t objectIndex = 0; objectIndex < aabb_objects.size(); ++objectIndex)
			{
				...
				auto intersect_triangle = [&](uint32_t subsetIndex, uint32_t indexOffset, uint32_t triangleIndex)
				{
					...
					float distance;
					XMFLOAT2 bary;
					if (vz::math::RayTriangleIntersects(rayOrigin_local, rayDirection_local, p0, p1, p2, distance, bary))
					{
						...
						distance = vz::math::Distance(pos, rayOrigin);
				...
				auto intersect_triangle = [&](uint32_t subsetIndex, uint32_t indexOffset, uint32_t triangleIndex)
				{
					...
					else
					{
						...
						else
						{
							...
							// Edge 0,1
							XMVECTOR Point1 = vz::math::ClosestPointOnLineSegment(p0, p1, LinePlaneIntersection);

							// Edge 1,2
							XMVECTOR Point2 = vz::math::ClosestPointOnLineSegment(p1, p2, LinePlaneIntersection);

							// Edge 2,0
							XMVECTOR Point3 = vz::math::ClosestPointOnLineSegment(p2, p0, LinePlaneIntersection);
					...
					XMVECTOR Center = vz::math::ClosestPointOnLineSegment(A, B, ReferencePoint);
					...
					// Edge 0,1
					XMVECTOR Point1 = vz::math::ClosestPointOnLineSegment(p0, p1, Center);
					...
					// Edge 1,2
					XMVECTOR Point2 = vz::math::ClosestPointOnLineSegment(p1, p2, Center);
					...
					// Edge 2,0
					XMVECTOR Point3 = vz::math::ClosestPointOnLineSegment(p2, p0, Center);
	...
	void Scene::VoxelizeObject(size_t objectIndex, vz::VoxelGrid& grid, bool subtract, uint32_t lod)
	{
		if (objectIndex >= objects.GetCount() || objectIndex >= aabb_objects.size())
			return;
		if (aabb_objects[objectIndex].intersects(grid.get_aabb()) == vz::primitive::AABB::OUTSIDE)
			return;
		const ObjectComponent& object = objects[objectIndex];
		const MeshComponent* mesh = meshes.GetComponent(object.meshID);
		if (mesh == nullptr)
	...
	void Scene::VoxelizeScene(vz::VoxelGrid& voxelgrid, bool subtract, uint32_t filterMask, uint32_t layerMask, uint32_t lod)
	{
		vz::jobsystem::context ctx;
		if ((filterMask & FILTER_COLLIDER))
		{
			for (size_t i = 0; i < collider_count_cpu; ++i)
			{
				...
				case ColliderComponent::Shape::Sphere:
				{
					...
					vz::jobsystem::Execute(ctx, [&voxelgrid, subtract, sphere](vz::jobsystem::JobArgs args) {
				...
				case ColliderComponent::Shape::Capsule:
				{
					...
					// TODO: fix heap allocating lambda capture!
					vz::jobsystem::Execute(ctx, [&voxelgrid, subtract, capsule](vz::jobsystem::JobArgs args) {
				...
				case ColliderComponent::Shape::Plane:
				{
					...
					// TODO: fix heap allocating lambda capture!
					vz::jobsystem::Execute(ctx, [&voxelgrid, subtract, P0, P1, P2, P3](vz::jobsystem::JobArgs args) {
		...
		if (filterMask & FILTER_OBJECT_ALL)
		{
			for (size_t i = 0; i < objects.GetCount(); ++i)
			{
				...
				if ((layerMask & aabb.layerMask) == 0)
					continue;
				// TODO: fix heap allocating lambda capture!
				vz::jobsystem::Execute(ctx, [this, &voxelgrid, subtract, lod, i](vz::jobsystem::JobArgs args) {
	...
	XMFLOAT3 Scene::GetPositionOnSurface(vz::ecs::Entity objectEntity, int vertexID0, int vertexID1, int vertexID2, const XMFLOAT2& bary) const
	{
	...
	void Scene::PutWaterRipple(const std::string& image, const XMFLOAT3& pos)
	{
		...
		img.params.pos = pos;
		img.params.rotation = (vz::random::GetRandom(0, 1000) * 0.001f) * 2 * 3.1415f;
		img.params.siz = XMFLOAT2(1, 1);
		img.params.quality = vz::image::QUALITY_ANISOTROPIC;
	...
	{
		vz::Sprite img;
		img.textureResource.SetTexture(*vz::texturehelper::getWaterRipple());
		img.params.enableExtractNormalMap();
		img.params.blendFlag = BLENDMODE_ADDITIVE;
		img.anim.fad = 0.01f;
		img.anim.scaleX = 0.1f;
	...
	{
		vz::Archive archive(fileName, true);
		if (archive.IsOpen())
		{
	...
	PickResult Pick(const vz::primitive::Ray& ray, uint32_t filterMask, uint32_t layerMask, const Scene& scene, uint32_t lod)
	{
		return scene.Intersects(ray, filterMask, layerMask, lod);
	}
	SceneIntersectSphereResult SceneIntersectSphere(const vz::primitive::Sphere& sphere, uint32_t filterMask, uint32_t layerMask, const Scene& scene, uint32_t lod)
	{
		return scene.Intersects(sphere, filterMask, layerMask, lod);
	}
	SceneIntersectCapsuleResult SceneIntersectCapsule(const vz::primitive::Capsule& capsule, uint32_t filterMask, uint32_t layerMask, const Scene& scene, uint32_t lod)
	{
	...
	XMMATRIX Scene::FindBoneRestPose(vz::ecs::Entity bone) const
	{
		if (bone != INVALID_ENTITY)
		{
	...
	void Scene::ScanAnimationDependencies()
	{
		...
		vz::jobsystem::Execute(animation_dependency_scan_workload, [&](vz::jobsystem::JobArgs args) {
			auto range = vz::profiler::BeginRangeCPU("Animation Dependencies");
