...
namespace vz::scene
{
	...
	{
		void Serialize(vz::Archive& archive, vz::ecs::EntitySerializer& seri) { /*this never serialized any data*/ }
	};
	...
	void LayerComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> layerMask;
	...
	void MaterialComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		...
		if (archive.IsReadMode())
		{
			...
			vz::jobsystem::Execute(seri.ctx, [&](vz::jobsystem::JobArgs args) {
				CreateRenderData();
			});
		}
		else
		{
			...
			archive << vz::helper::GetPathRelative(dir, textures[BASECOLORMAP].name);
			archive << vz::helper::GetPathRelative(dir, textures[SURFACEMAP].name);
			archive << vz::helper::GetPathRelative(dir, textures[NORMALMAP].name);
			archive << vz::helper::GetPathRelative(dir, textures[DISPLACEMENTMAP].name);

			if (archive.GetVersion() >= 24)
			{
				archive << vz::helper::GetPathRelative(dir, textures[EMISSIVEMAP].name);
			}

			if (archive.GetVersion() >= 28)
			{
				archive << vz::helper::GetPathRelative(dir, textures[OCCLUSIONMAP].name);
			...
			{
				archive << transmission;
				archive << vz::helper::GetPathRelative(dir, textures[TRANSMISSIONMAP].name);
				archive << textures[TRANSMISSIONMAP].uvset;
			}
			...
			if (archive.GetVersion() >= 61)
			{
				archive << sheenColor;
				archive << sheenRoughness;
				archive << vz::helper::GetPathRelative(dir, textures[SHEENCOLORMAP].name);
				archive << vz::helper::GetPathRelative(dir, textures[SHEENROUGHNESSMAP].name);
				archive << textures[SHEENCOLORMAP].uvset;
				archive << textures[SHEENROUGHNESSMAP].uvset;

				archive << clearcoat;
				archive << clearcoatRoughness;
				archive << vz::helper::GetPathRelative(dir, textures[CLEARCOATMAP].name);
				archive << vz::helper::GetPathRelative(dir, textures[CLEARCOATROUGHNESSMAP].name);
				archive << vz::helper::GetPathRelative(dir, textures[CLEARCOATNORMALMAP].name);
				archive << textures[CLEARCOATMAP].uvset;
				archive << textures[CLEARCOATROUGHNESSMAP].uvset;
				archive << textures[CLEARCOATNORMALMAP].uvset;
			}
			...
			if (archive.GetVersion() >= 68)
			{
				archive << vz::helper::GetPathRelative(dir, textures[SPECULARMAP].name);
				archive << textures[SPECULARMAP].uvset;
			}
			...
			if (seri.GetVersion() >= 2)
			{
				archive << anisotropy_strength;
				archive << anisotropy_rotation;
				archive << vz::helper::GetPathRelative(dir, textures[ANISOTROPYMAP].name);
				archive << textures[ANISOTROPYMAP].uvset;
			}

			if (seri.GetVersion() >= 2)
			{
				archive << vz::helper::GetPathRelative(dir, textures[TRANSPARENCYMAP].name);
				archive << textures[TRANSPARENCYMAP].uvset;
			}
		}
	}
	void MeshComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ImpostorComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ObjectComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void RigidBodyPhysicsComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void SoftBodyPhysicsComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ArmatureComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void LightComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
			...
			vz::jobsystem::Execute(seri.ctx, [&](vz::jobsystem::JobArgs args) {
				...
				for (size_t i = 0; i < lensFlareNames.size(); ++i)
				{
					if (!lensFlareNames[i].empty())
					{
						...
						lensFlareRimTextures[i] = vz::resourcemanager::Load(lensFlareNames[i], vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
					}
				}
			});
		}
		else
		{
			...
			vz::vector<std::string> lensFlareNamesRelative = lensFlareNames;
			if (!dir.empty())
			{
				for (size_t i = 0; i < lensFlareNamesRelative.size(); ++i)
				{
					vz::helper::MakePathRelative(dir, lensFlareNamesRelative[i]);
				}
			}
	...
	void EnvironmentProbeComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
		...
		else
		{
			...
			{
				archive << resolution;
				archive << vz::helper::GetPathRelative(dir, textureName);
			}
		}
	}
	void ForceFieldComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void DecalComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void AnimationComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void AnimationDataComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void WeatherComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		...
		if (archive.IsReadMode())
		{
			...
			if (archive.GetVersion() >= 32)
			{
				...
				if (!skyMapName.empty())
				{
					...
					skyMap = vz::resourcemanager::Load(skyMapName, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
				}
			}
			...
			if (archive.GetVersion() >= 62)
			{
				...
				if (!colorGradingMapName.empty())
				{
					...
					colorGradingMap = vz::resourcemanager::Load(colorGradingMapName, vz::resourcemanager::Flags::IMPORT_COLORGRADINGLUT | vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
				}
			}
			...
			if (archive.GetVersion() >= 86)
			{
				...
				if (!volumetricCloudsWeatherMapFirstName.empty())
				{
					...
					volumetricCloudsWeatherMapFirst = vz::resourcemanager::Load(volumetricCloudsWeatherMapFirstName, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
				}
			}
			...
			if (archive.GetVersion() >= 88)
			{
				...
				if (!volumetricCloudsWeatherMapSecondName.empty())
				{
					...
					volumetricCloudsWeatherMapSecond = vz::resourcemanager::Load(volumetricCloudsWeatherMapSecondName, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
				}
		...
		else
		{
			...
			if (archive.GetVersion() >= 32)
			{
				archive << vz::helper::GetPathRelative(dir, skyMapName);
			}
			if (archive.GetVersion() >= 40)
			{
				archive << windSpeed;
			...
			if (archive.GetVersion() >= 62)
			{
				archive << vz::helper::GetPathRelative(dir, colorGradingMapName);
			}
			...
			if (archive.GetVersion() >= 86)
			{
				archive << vz::helper::GetPathRelative(dir, volumetricCloudsWeatherMapFirstName);
			}

			if (archive.GetVersion() >= 88)
			{
				archive << vz::helper::GetPathRelative(dir, volumetricCloudsWeatherMapSecondName);
	...
	void SoundComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
		...
		if (archive.IsReadMode())
		{
			...
			vz::jobsystem::Execute(seri.ctx, [&](vz::jobsystem::JobArgs args) {
				if (!filename.empty())
				{
					...
					soundResource = vz::resourcemanager::Load(filename, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
		...
		else
		{
			...
			archive << _flags;
			archive << vz::helper::GetPathRelative(dir, filename);
			archive << volume;
	...
	void VideoComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
		...
		if (archive.IsReadMode())
		{
			...
			vz::jobsystem::Execute(seri.ctx, [&](vz::jobsystem::JobArgs args) {
				...
				{
					filename = dir + filename;
					videoResource = vz::resourcemanager::Load(filename, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
					vz::video::CreateVideoInstance(&videoResource.GetVideo(), &videoinstance);
				}
			});
		}
		else
	...
	void InverseKinematicsComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
	...
	void SpringComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ColliderComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ScriptComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
	...
	void ExpressionComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void HumanoidComponent::Serialize(vz::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void Scene::Serialize(vz::Archive& archive)
	{
		...
		vz::resourcemanager::ResourceSerializer resource_seri;
		if (archive.IsReadMode() && archive.GetVersion() >= 63)
		{
			vz::resourcemanager::Serialize_READ(archive, resource_seri);
			if (archive.GetVersion() >= 90)
			{
		...
		else
		{
			...
			ComponentManager<vz::primitive::AABB> aabbs_tmp; // no longer needed from serializer
		...
		vz::jobsystem::Wait(seri.ctx); // This is needed before emitter material fixup that is below, because material CreateRenderDatas might be pending!
		...
		if (archive.GetVersion() >= 90)
		{
			...
			else
			{
				...
				vz::resourcemanager::Serialize_WRITE(archive, seri.resource_registration);
				archive.PatchUnknownJumpPosition(jump_after);
			}
		}

		vz::backlog::post("Scene serialize took " + std::to_string(timer.elapsed_seconds()) + " sec");
	}

	void Scene::DDGI::Serialize(vz::Archive& archive)
	{
		...
		else
		{
			...
			{
				bool success = vz::helper::saveTextureToMemory(color_texture[0], data);
				assert(success);
			}
			archive << data;
			...
			{
				bool success = vz::helper::saveTextureToMemory(depth_texture[0], data);
				assert(success);
			}
			archive << data;
			...
			if(offset_buffer.IsValid())
			{
				...
				desc.usage = vz::graphics::Usage::READBACK;
				desc.bind_flags = {};
				desc.misc_flags = {};
				GPUBuffer staging;
		...
		else
		{
			...
			if (archive.IsReadMode())
			{
				...
				}
				{
					...
					if (component_exists)
					{
						auto component = vz::primitive::AABB(); // no longer needed to be serialized
						component.Serialize(archive, seri);
					}
				}
				{
