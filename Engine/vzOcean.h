...
namespace vz
{
	...
	public:
		...
		void UpdateDisplacementMap(const OceanParameters& params, vz::graphics::CommandList cmd) const;
		void Render(const vz::scene::CameraComponent& camera, const OceanParameters& params, vz::graphics::CommandList cmd) const;

		const vz::graphics::Texture* getDisplacementMap() const;
		const vz::graphics::Texture* getGradientMap() const;
		...
		mutable int occlusionQueries[vz::graphics::GraphicsDevice::GetBufferCount() + 1];
	...
	protected:
		vz::graphics::Texture displacementMap;		// (RGBA32F)
		vz::graphics::Texture gradientMap;			// (RGBA16F)
		...
		vz::graphics::GPUBuffer buffer_Float2_H0;

		// Angular frequency
		vz::graphics::GPUBuffer buffer_Float_Omega;

		// Height field H(t), choppy field Dx(t) and Dy(t) in frequency domain, updated each frame.
		vz::graphics::GPUBuffer buffer_Float2_Ht;

		// Height & choppy buffer in the space domain, corresponding to H(t), Dx(t) and Dy(t)
		vz::graphics::GPUBuffer buffer_Float_Dxyz;


		vz::graphics::GPUBuffer immutableCB;
		vz::graphics::GPUBuffer perFrameCB;
		mutable vz::graphics::GPUBuffer indexBuffer;
	};
}
