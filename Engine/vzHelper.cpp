...
namespace vz::helper
{
	...
	void messageBox(const std::string& msg, const std::string& caption)
	{
		...
		vz::eventhandler::Subscribe_Once(vz::eventhandler::EVENT_THREAD_SAFE_POINT, [=](uint64_t userdata) {
	...
	bool saveTextureToMemory(const vz::graphics::Texture& texture, vz::vector<uint8_t>& texturedata)
	{
	...
	bool saveTextureToMemoryFile(const vz::graphics::Texture& texture, const std::string& fileExtension, vz::vector<uint8_t>& filedata)
	{
		using namespace vz::graphics;
		TextureDesc desc = texture.GetDesc();
	...
	bool saveTextureToMemoryFile(const vz::vector<uint8_t>& texturedata, const vz::graphics::TextureDesc& desc, const std::string& fileExtension, vz::vector<uint8_t>& filedata)
	{
		using namespace vz::graphics;
		const uint32_t data_stride = GetFormatStride(desc.format);
		...
		if (is_png)
		{
			if (desc.format == Format::R16_UNORM || desc.format == Format::R16_UINT)
			{
				...
				vz::vector<uint8_t> src_bigendian = texturedata;
		...
		stbi_write_func* func = [](void* context, void* data, int size) {
			vz::vector<uint8_t>& filedata = *(vz::vector<uint8_t>*)context;
	...
	bool saveTextureToFile(const vz::vector<uint8_t>& texturedata, const vz::graphics::TextureDesc& desc, const std::string& fileName)
	{
		using namespace vz::graphics;
...
#define ToNativeString(x) (x)
	...
	bool FileRead_Impl(const std::string& fileName, vector_interface<uint8_t, std::allocator<uint8_t>>& data)
	{
		...
		auto async_helper = [&]() -> IAsyncAction {
			...
			catch (winrt::hresult_error const& ex)
			{
				...
				{
				case E_ACCESSDENIED:
					vz::backlog::post("Opening file failed: " + fileName + " | Reason: Permission Denied!");
					break;
				default:
					break;
				}
		...
		vz::backlog::post("File not found: " + fileName, vz::backlog::LogLevel::Warning);
		return false;
	}
	...
	void GetFileNamesInDirectory(const std::string& directory, std::function<void(std::string fileName)> onSuccess, const std::string& filter_extension)
	{
		...
		for (const auto& entry : std::filesystem::directory_iterator(directory_path))
		{
			...
			if (filter_extension.empty() || vz::helper::toUpper(vz::helper::GetExtensionFromFileName(filename)).compare(filter_extension) == 0)
			{
	...
	void OpenUrl(const std::string& url)
	{
		...
		vz::backlog::post("vz::helper::OpenUrl(" + url + ") returned status: " + std::to_string(status));
		return;
#endif // PLATFORM_WINDOWS_DESKTOP
		...
		vz::backlog::post("vz::helper::OpenUrl(" + url + "): not implemented for this operating system!", vz::backlog::LogLevel::Warning);
	}
