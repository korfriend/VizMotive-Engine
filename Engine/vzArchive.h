...
namespace vz
{
	...
	private:
		...
		vz::vector<uint8_t> DATA; // data suitable for read/write operations
	...
	public:
		...
		void WriteData(vz::vector<uint8_t>& dest) const { dest.resize(pos); std::memcpy(dest.data(), data_ptr, pos); }
		...
		template<typename T>
		inline Archive& operator<<(const vz::vector<T>& data)
		{
		...
		template<typename T>
		inline Archive& operator>>(vz::vector<T>& data)
		{
			// Here we will use the >> operator so that non-specified types will have compile error!
			size_t count;
			(*this) >> count;
