...
namespace tinygltf
{
	...
	bool FileExists(const std::string& abs_filename, void*) {
		return vz::helper::FileExists(abs_filename);
	}
	...
	bool ReadWholeFile(std::vector<unsigned char>* out, std::string* err,
		...
		return vz::helper::FileRead(filepath, *out);
	}

	bool WriteWholeFile(std::string* err, const std::string& filepath,
		const std::vector<unsigned char>& contents, void*) {
		return vz::helper::FileWrite(filepath, contents.data(), contents.size());
	}
		...
		const unsigned char *bytes, int size, void *userdata)
	{
		...
		if (image->uri.empty())
		{
			...
			do {
				ss.clear();
				ss += "gltfimport_" + std::to_string(vz::random::GetRandom(std::numeric_limits<uint32_t>::max())) + ".png";
			} while (vz::resourcemanager::Contains(ss)); // this is to avoid overwriting an existing imported image
			image->uri = ss;
		}

		auto resource = vz::resourcemanager::Load(
			image->uri,
			vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA | vz::resourcemanager::Flags::IMPORT_DELAY,
			(const uint8_t*)bytes,
			(size_t)size
		);
		...
		vz::resourcemanager::ResourceSerializer* seri = (vz::resourcemanager::ResourceSerializer*)userdata;
...
struct LoaderState
{
	...
	Scene* scene;
	vz::unordered_map<int, Entity> entityMap;  // node -> entity
	Entity rootEntity = INVALID_ENTITY;

	//Export states
	vz::unordered_map<std::string, int> textureMap; // path -> textureid
	vz::unordered_map<Entity, int> nodeMap;// entity -> node
	vz::unordered_map<size_t, TransformComponent> transforms_original; // original transform states
};
...
void FlipZAxis(LoaderState& state)
{
	...
	if(!state_restore)
	{
		vz::unordered_map<vz::ecs::Entity, vz::ecs::Entity> hierarchy_list;
		vz::unordered_map<size_t, vz::scene::TransformComponent> correction_queue;
	...
	for(size_t i = 0; i < wiscene.animations.GetCount(); ++i)
	{
		...
		for(auto& channel : animation.channels)
		{
			...
			if(channel.path == vz::scene::AnimationComponent::AnimationChannel::Path::TRANSLATION)
			{
			...
			if(channel.path == vz::scene::AnimationComponent::AnimationChannel::Path::ROTATION)
			{
...
void ImportModel_GLTF(const std::string& fileName, Scene& scene)
{
	...
	std::string extension = vz::helper::toUpper(vz::helper::GetExtensionFromFileName(name));
	...
	vz::resourcemanager::ResourceSerializer seri; // keep this alive to not delete loaded images while importing gltf
	...
	vz::vector<uint8_t> filedata;
	bool ret = vz::helper::FileRead(fileName, filedata);
	...
	if (!ret)
	{
		vz::helper::messageBox(err, "GLTF error!");
	}
	...
	for (auto& x : state.gltfModel.materials)
	{
		...
		if (alphaMode != x.additionalValues.end())
		{
			...
			{
				material.userBlendMode = vz::enums::BLENDMODE_ALPHA;
			}
		...
		if (ext_specular != x.extensions.end())
		{
			...
			if (ext_specular->second.Has("specularTexture"))
			{
				...
				{
					vz::backlog::post("[KHR_materials_specular warning] specularTexture must be either in surfaceMap.a or specularColorTexture.a! specularTexture discarded!", vz::backlog::LogLevel::Warning);
				}
			}
					...
					if (accessor.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT)
					{
						if (accessor.type == TINYGLTF_TYPE_VEC3)
						{
							for (size_t i = 0; i < vertexCount; ++i)
							{
								...
								uint32_t rgba = vz::math::CompressColor(color);
					...
					else if (accessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE)
					{
						if (accessor.type == TINYGLTF_TYPE_VEC3)
						{
							for (size_t i = 0; i < vertexCount; ++i)
							{
								...
								vz::Color color = vz::Color(r, g, b, a);
					...
					else if (accessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT)
					{
						if (accessor.type == TINYGLTF_TYPE_VEC3)
						{
							for (size_t i = 0; i < vertexCount; ++i)
							{
								...
								uint32_t rgba = vz::math::CompressColor(XMFLOAT3(r / 65535.0f, g / 65535.0f, b / 65535.0f));
						...
						else if (accessor.type == TINYGLTF_TYPE_VEC4)
						{
							for (size_t i = 0; i < vertexCount; ++i)
							{
								...
								uint32_t rgba = vz::math::CompressColor(XMFLOAT4(r / 65535.0f, g / 65535.0f, b / 65535.0f, a / 65535.0f));
	...
	if (env != state.gltfModel.extensions.end())
	{
		...
		for (int i = 0; i < (int)lights.ArrayLen(); ++i)
		{
			...
			if (light.Has("specularImages"))
			{
				...
				vz::vector<vz::vector<XMFLOAT3SE>> hdr_datas(mip_count * 6);
				...
				for (int m = 0; m < mip_count; ++m)
				{
					...
					for (int f = 0; f < face_count; ++f)
					{
						...
						vz::Resource res = vz::resourcemanager::Load(image.uri, vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA);
						auto& imagefiledata = res.GetFileData();
						const stbi_uc* filedata = imagefiledata.data();
						size_t filesize = imagefiledata.size();
						int width, height, bpp;
						vz::Color* rgba = (vz::Color*)stbi_load_from_memory(filedata, (int)filesize, &width, &height, &bpp, 4);
						vz::vector<XMFLOAT3SE>& hdr_data = hdr_datas[idx];
						...
						for (int y = 0; y < height; ++y)
						{
							for (int x = 0; x < width; ++x)
							{
								...
								vz::Color color = rgba[x + y_flip * width];
				...
				weather.skyMapName = vz::helper::RemoveExtension(vz::helper::GetFileNameFromPath(fileName)) + "/EXT_lights_image_based_" + std::to_string(counter++) + ".dds";
				weather.skyMap = vz::resourcemanager::Load(
					weather.skyMapName,
					vz::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA,
					dds.data(),
					dds.size()
				);
			...
			if (blendShapeMaster.Has("blendShapeGroups"))
			{
				...
				for (size_t blendShapeGroup_index = 0; blendShapeGroup_index < blendShapeGroups.ArrayLen(); ++blendShapeGroup_index)
				{
					...
					if (blendShapeGroup.Has("presetName"))
					{
						...
						std::string presetName = vz::helper::toUpper(value.Get<std::string>());
						...
						else if (!vz::helper::toUpper(expression.name).compare("SURPRISED")) // wtf vroid
						{
							expression.preset = ExpressionComponent::Preset::Surprised;
						}
				...
				for (size_t boneGroup_index = 0; boneGroup_index < boneGroups.ArrayLen(); ++boneGroup_index)
				{
					...
					if (boneGroup.Has("bones"))
					{
						...
						for (size_t bone_index = 0; bone_index < bones.ArrayLen(); ++bone_index)
						{
							...
							vz::vector<int> stack = state.gltfModel.nodes[node].children;
							while (!stack.empty())
							{
			...
			if (humanoid.Has("humanBones"))
			{
				...
				for (size_t bone_index = 0; bone_index < humanBones.ArrayLen(); ++bone_index)
				{
					...
					if (humanBone.Has("bone"))
					{
						...
						const std::string& type = vz::helper::toUpper(value.Get<std::string>());
				...
				if (material.Has("name"))
				{
					...
					if (component != nullptr)
					{
						if (material.Has("shader"))
						{
							...
							const std::string& shaderName = vz::helper::toUpper(value.Get<std::string>());
			...
			for (auto& expression_type : expression_types)
			{
				if (expressions.Has(expression_type))
				{
					...
					for (auto& name : names.Keys())
					{
						...
						{
							std::string presetName = vz::helper::toUpper(name);
		...
		if (ext_vrm->second.Has("humanoid"))
		{
			...
			if (humanoid.Has("humanBones"))
			{
				...
				for (const auto& key : keys)
				{
					...
					const std::string& type = vz::helper::toUpper(key);
					if (!type.compare("NECK"))
					{
						component.bones[size_t(HumanoidComponent::HumanoidBone::Neck)] = boneID;
					}
...
vz::vector<std::string> original_texture_extension_iterator = 
{
	"png",
	"jpg",
	"jpeg",
...
inline std::string _ExportHelper_GetOriginalTexture(std::string texture_file)
{
	...
	{
		std::string target_file = vz::helper::ReplaceExtension(texture_file, ext);
		vz::backlog::post(target_file);
		if (vz::helper::FileExists(target_file))
		{
			return target_file;
		}
	}
...
inline tinygltf::TextureInfo _ExportHelper_StoreMaterialTexture(LoaderState& state, const std::string& gltf_dir, const MaterialComponent& material, MaterialComponent::TEXTURESLOT slot)
{
	...
	if(find_texture_id == state.textureMap.end())
	{
		...
		std::string src_extension = vz::helper::toUpper(vz::helper::GetExtensionFromFileName(texture_file));
		...
		vz::helper::MakePathRelative(gltf_dir, texture_file);
...
void ExportModel_GLTF(const std::string& filename, Scene& scene)
{
	...
	for (size_t i = 0; i < scene.materials.GetCount(); ++i)
	{
		...
		if (material.transmission > 0 || material.textures[vz::scene::MaterialComponent::TRANSMISSIONMAP].resource.IsValid())
		{
	...
	if (vzscene.materials.GetCount() == 0)
	{
		state.gltfModel.materials.emplace_back().name = "dummyMaterial";
	}

	// Write Materials
	for(size_t mt_id = 0; mt_id < vzscene.materials.GetCount(); ++mt_id)
	{
		auto& material = vzscene.materials[mt_id];
		auto materialEntity = vzscene.materials.GetEntity(mt_id);
		auto nameComponent = vzscene.names.GetComponent(materialEntity);
		...
		// Textures
		if(material.textures[vz::scene::MaterialComponent::BASECOLORMAP].resource.IsValid())
		{
			material_builder.pbrMetallicRoughness.baseColorTexture = _ExportHelper_StoreMaterialTexture(
				state, 
				vz::helper::GetDirectoryFromPath(filename), 
				material,
				vz::scene::MaterialComponent::BASECOLORMAP
			);
		}
		if(material.textures[vz::scene::MaterialComponent::NORMALMAP].resource.IsValid())
		{
			auto normalTexInfo_pre = _ExportHelper_StoreMaterialTexture(
				state, 
				vz::helper::GetDirectoryFromPath(filename), 
				material,
				vz::scene::MaterialComponent::NORMALMAP
			);
		...
		if(material.textures[vz::scene::MaterialComponent::OCCLUSIONMAP].resource.IsValid())
		{
			auto occlTexInfo_pre = _ExportHelper_StoreMaterialTexture(
				state, 
				vz::helper::GetDirectoryFromPath(filename), 
				material,
				vz::scene::MaterialComponent::OCCLUSIONMAP
			);
		...
		if(material.textures[vz::scene::MaterialComponent::EMISSIVEMAP].resource.IsValid())
		{
			material_builder.emissiveTexture = _ExportHelper_StoreMaterialTexture(
				state, 
				vz::helper::GetDirectoryFromPath(filename), 
				material,
				vz::scene::MaterialComponent::EMISSIVEMAP
			);
		}
		if(material.textures[vz::scene::MaterialComponent::SURFACEMAP].resource.IsValid())
		{
			material_builder.pbrMetallicRoughness.metallicRoughnessTexture = _ExportHelper_StoreMaterialTexture(
				state, 
				vz::helper::GetDirectoryFromPath(filename), 
				material,
				vz::scene::MaterialComponent::SURFACEMAP
			);				
		}
		// Values
		...
		// Values
		if (material.shaderType == vz::scene::MaterialComponent::SHADERTYPE_UNLIT)
		{
		...
		if (material.transmission > 0 || material.textures[vz::scene::MaterialComponent::TRANSMISSIONMAP].resource.IsValid())
		{
			...
			// Textures
			if (material.textures[vz::scene::MaterialComponent::TRANSMISSIONMAP].resource.IsValid())
			{
				auto transmissionTexInfo_pre = _ExportHelper_StoreMaterialTexture(
					state,
					vz::helper::GetDirectoryFromPath(filename),
					material,
					vz::scene::MaterialComponent::TRANSMISSIONMAP
				);
		...
		if(material.shaderType == vz::scene::MaterialComponent::SHADERTYPE_PBR_CLOTH || material.shaderType == vz::scene::MaterialComponent::SHADERTYPE_PBR_CLOTH_CLEARCOAT)
		{
			// Values
			...
			// Textures
			if(material.textures[vz::scene::MaterialComponent::SHEENCOLORMAP].resource.IsValid())
			{
				auto sheencolorTexInfo_pre = _ExportHelper_StoreMaterialTexture(
					state, 
					vz::helper::GetDirectoryFromPath(filename), 
					material,
					vz::scene::MaterialComponent::SHEENCOLORMAP
				);
			...
			if(material.textures[vz::scene::MaterialComponent::SHEENROUGHNESSMAP].resource.IsValid())
			{
				auto sheenRoughTexInfo_pre = _ExportHelper_StoreMaterialTexture(
					state, 
					vz::helper::GetDirectoryFromPath(filename), 
					material,
					vz::scene::MaterialComponent::SHEENROUGHNESSMAP
				);
		...
		if (material.shaderType == MaterialComponent::SHADERTYPE_PBR_CLEARCOAT || material.shaderType == MaterialComponent::SHADERTYPE_PBR_CLOTH_CLEARCOAT)
		{
			...
			// Textures
			if (material.textures[vz::scene::MaterialComponent::CLEARCOATMAP].resource.IsValid())
			{
				auto clearcoatTexInfo_pre = _ExportHelper_StoreMaterialTexture(
					state,
					vz::helper::GetDirectoryFromPath(filename),
					material,
					vz::scene::MaterialComponent::CLEARCOATMAP
				);
			...
			if (material.textures[vz::scene::MaterialComponent::CLEARCOATNORMALMAP].resource.IsValid())
			{
				auto clearcoatNormTexInfo_pre = _ExportHelper_StoreMaterialTexture(
					state,
					vz::helper::GetDirectoryFromPath(filename),
					material,
					vz::scene::MaterialComponent::CLEARCOATNORMALMAP
				);
			...
			if (material.textures[vz::scene::MaterialComponent::CLEARCOATROUGHNESSMAP].resource.IsValid())
			{
				auto clearcoatRoughTexInfo_pre = _ExportHelper_StoreMaterialTexture(
					state,
					vz::helper::GetDirectoryFromPath(filename),
					material,
					vz::scene::MaterialComponent::CLEARCOATROUGHNESSMAP
				);
		...
		if(material.textures[vz::scene::MaterialComponent::SPECULARMAP].resource.IsValid())
		{
			auto specularTexInfo_pre = _ExportHelper_StoreMaterialTexture(
				state, 
				vz::helper::GetDirectoryFromPath(filename), 
				material,
				vz::scene::MaterialComponent::SPECULARMAP
			);
		...
		if (material.shaderType == MaterialComponent::SHADERTYPE_PBR_ANISOTROPIC)
		{
			...
			if (material.textures[vz::scene::MaterialComponent::ANISOTROPYMAP].resource.IsValid())
			{
				auto specularTexInfo_pre = _ExportHelper_StoreMaterialTexture(
					state,
					vz::helper::GetDirectoryFromPath(filename),
					material,
					vz::scene::MaterialComponent::ANISOTROPYMAP
				);
	...
	// Write Meshes
	for(size_t m_id = 0; m_id < vzscene.meshes.GetCount(); ++m_id)
	{
		auto& mesh = vzscene.meshes[m_id];
		auto meshEntity = vzscene.meshes.GetEntity(m_id);
		auto nameComponent = vzscene.names.GetComponent(meshEntity);
		...
		auto bound = vz::primitive::AABB(mesh.vertex_positions[0], mesh.vertex_positions[0]);
		for(auto& vpos : mesh.vertex_positions)
		{
			bound = vz::primitive::AABB::Merge(bound, vz::primitive::AABB(vpos, vpos));
		}
		...
		vz::vector<std::pair<size_t,bool>> morphs_pos_accessors;
		vz::vector<std::pair<size_t,bool>> morphs_norm_accessors;
		...
		for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset; ++subsetIndex)
		{
			...
			primitive_builder.material = std::max(0, std::min((int)vzscene.materials.GetIndex(subset.materialID), (int)vzscene.materials.GetCount() - 1));
	...
	// Write Lights
	for(size_t l_id = 0; l_id < vzscene.lights.GetCount(); ++l_id)
	{
		auto& light = vzscene.lights[l_id];
		auto lightEntity = vzscene.lights.GetEntity(l_id);

		auto nameComponent = vzscene.names.GetComponent(lightEntity);
	...
	// Write Cameras
	for(size_t cam_id = 0; cam_id < vzscene.cameras.GetCount(); ++cam_id)
	{
		auto& camera = vzscene.cameras[cam_id];
		auto cameraEntity = vzscene.cameras.GetEntity(cam_id);

		auto nameComponent = vzscene.names.GetComponent(cameraEntity);
	...
	// Compose Node
	for(size_t t_id = 0; t_id < vzscene.transforms.GetCount(); ++t_id)
	{
		auto& transformComponent = vzscene.transforms[t_id];
		auto transformEntity = vzscene.transforms.GetEntity(t_id);
		auto nameComponent = vzscene.names.GetComponent(transformEntity);

		auto light_forward_flip = vzscene.lights.Contains(transformEntity);
		if(light_forward_flip)
			transformComponent.RotateRollPitchYaw(XMFLOAT3(-XM_PIDIV2,0,0));

		auto objectComponent = vzscene.objects.GetComponent(transformEntity);
		...
		{
			if(objectComponent->meshID != vz::ecs::INVALID_ENTITY)
			{
				node_builder.mesh = (int)vzscene.meshes.GetIndex(objectComponent->meshID);
				if(vzscene.meshes[node_builder.mesh].armatureID != vz::ecs::INVALID_ENTITY)
				{
					node_builder.skin = (int)vzscene.armatures.GetIndex(vzscene.meshes[node_builder.mesh].armatureID);
				}
			}
		}

		if(vzscene.lights.Contains(transformEntity))
		{
			tinygltf::Value::Object node_light_extension_builder;
			node_light_extension_builder["light"] = tinygltf::Value(int(vzscene.lights.GetIndex(transformEntity)));
			node_builder.extensions["KHR_lights_punctual"] = tinygltf::Value(node_light_extension_builder);
		}

		if(vzscene.cameras.Contains(transformEntity))
		{
			node_builder.camera = (int)vzscene.cameras.GetIndex(transformEntity);
		}
	...
	// Write Armature
	for(size_t arm_id = 0; arm_id < vzscene.armatures.GetCount(); ++arm_id)
	{
		auto& armatureComponent = vzscene.armatures[arm_id];
		auto armatureEntity = vzscene.armatures.GetEntity(arm_id);

		auto nameComponent = vzscene.names.GetComponent(armatureEntity);
		...
		//// Analysis prep
		//vz::jobsystem::context analysis_ctx;
	...
	// Write Animations
	vz::unordered_map<Entity, std::vector<size_t>> animation_datasets;
	if(vzscene.animations.GetCount() > 0)
	{
		// Find accessor types first!
		vz::unordered_map<Entity, size_t> animdata_vectype; 
		for(size_t anim_id = 0; anim_id < vzscene.animations.GetCount(); ++anim_id)
		{
			auto& animation = vzscene.animations[anim_id];
			for(auto& channel : animation.channels)
			{
		...
		tinygltf::Buffer buffer_builder;
		int buffer_index = (int)state.gltfModel.buffers.size();
		for(size_t animdata_id = 0; animdata_id < vzscene.animation_datas.GetCount(); ++animdata_id)
		{
			auto& animdata = vzscene.animation_datas[animdata_id];
			auto animdataEntity = vzscene.animation_datas.GetEntity(animdata_id);
	...
	// Compose hierarchy
	for(size_t h_id = 0; h_id < vzscene.hierarchy.GetCount(); ++h_id)
	{
		auto& hierarchyComponent = vzscene.hierarchy[h_id];
		auto hierarchyEntity = vzscene.hierarchy.GetEntity(h_id);
		if(vzscene.transforms.Contains(hierarchyComponent.parentID) && vzscene.transforms.Contains(hierarchyEntity))
		{
			int node_index = (int)vzscene.transforms.GetIndex(hierarchyEntity);
			size_t parent_node_index = vzscene.transforms.GetIndex(hierarchyComponent.parentID);
	...
	auto file_extension = vz::helper::toUpper(vz::helper::GetExtensionFromFileName(filename));
