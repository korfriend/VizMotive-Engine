...
namespace vz
{
	...
	void RenderPath2D::Render() const
	{
		...
		vz::renderer::ProcessDeferredTextureRequests(cmd);

		if (GetGUIBlurredBackground() != nullptr)
		{
			vz::image::SetBackground(*GetGUIBlurredBackground());
		}
			...
			for (auto& x : layers)
			{
				for (auto& y : x.items)
				{
					if (y.type == RenderItem2D::TYPE::SPRITE &&
						y.sprite != nullptr &&
						y.sprite->params.stencilComp != vz::image::STENCILMODE_DISABLED)
					{
						y.sprite->Draw(cmd);
					}
				}
		...
		if (GetDepthStencil() != nullptr)
		{
			if (rtStenciled.IsValid())
			{
				...
				if (rtStenciled.GetDesc().sample_count > 1)
				{
					vz::image::Draw(&rtStenciled_resolved, fx, cmd);
				}
				else
				{
					vz::image::Draw(&rtStenciled, fx, cmd);
				}
				device->EventEnd(cmd);
			}
			else
		...
		for (auto& x : layers)
		{
			for (auto& y : x.items)
			{
				...
				default:
				case RenderItem2D::TYPE::SPRITE:
					if (y.sprite != nullptr && y.sprite->params.stencilComp == vz::image::STENCILMODE_DISABLED)
					{
						y.sprite->Draw(cmd);
					}
					break;
	...
	void RenderPath2D::Compose(CommandList cmd) const
	{
		...
		vz::image::Draw(&GetRenderResult(), fx, cmd);

		RenderPath::Compose(cmd);
	}


	void RenderPath2D::AddSprite(vz::Sprite* sprite, const std::string& layer)
	{
		for (auto& x : layers)
		{
	...
	void RenderPath2D::RemoveSprite(vz::Sprite* sprite)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	int RenderPath2D::GetSpriteOrder(vz::Sprite* sprite)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::AddFont(vz::SpriteFont* font, const std::string& layer)
	{
		for (auto& x : layers)
		{
			if (!x.name.compare(layer))
	...
	void RenderPath2D::RemoveFont(vz::SpriteFont* font)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	int RenderPath2D::GetFontOrder(vz::SpriteFont* font)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::SetSpriteOrder(vz::Sprite* sprite, int order)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::SetFontOrder(vz::SpriteFont* font, int order)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::CleanLayers()
	{
		for (auto& x : layers)
		{
			...
			vz::vector<RenderItem2D> itemsToRetain(0);
			for (auto& y : x.items)
			{
