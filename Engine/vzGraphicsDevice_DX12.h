...
namespace vz::graphics
{
	...
	protected:
		...
		struct CommandQueue
		{
			...
			vz::vector<ID3D12CommandList*> submit_cmds;
		} queues[QUEUE_COUNT];
		...
		struct CommandList_DX12
		{
			...
			vz::vector<D3D12_RESOURCE_BARRIER> frame_barriers;
			struct Discard
			{
				ID3D12Resource* resource = nullptr;
				D3D12_DISCARD_REGION region = {};
			};
			vz::vector<Discard> discards;
			D3D_PRIMITIVE_TOPOLOGY prev_pt = {};
			vz::vector<std::pair<size_t, Microsoft::WRL::ComPtr<ID3D12PipelineState>>> pipelines_worker;
			...
			vz::vector<const SwapChain*> swapchains;
			bool dirty_pso = {};
			vz::vector<D3D12_RAYTRACING_GEOMETRY_DESC> accelerationstructure_build_geometries;
			RenderPassInfo renderpass_info;
			vz::vector<D3D12_RESOURCE_BARRIER> renderpass_barriers_begin;
			vz::vector<D3D12_RESOURCE_BARRIER> renderpass_barriers_begin_after_discards;
			vz::vector<D3D12_RESOURCE_BARRIER> renderpass_barriers_end;
			...
			vz::vector<D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS> resolve_subresources[D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT] = {};
			vz::vector<D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS> resolve_subresources_dsv = {};
			vz::vector<D3D12_RESOURCE_BARRIER> resolve_src_barriers;
		...
		vz::vector<std::unique_ptr<CommandList_DX12>> commandlists;
		...
		vz::unordered_map<size_t, Microsoft::WRL::ComPtr<ID3D12PipelineState>> pipelines_global;
	...
	public:
		...
		struct AllocationHandler
		{
			...
			struct DescriptorAllocator
			{
				...
				vz::vector<Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>> heaps;
				uint32_t descriptor_size = 0;
				vz::vector<D3D12_CPU_DESCRIPTOR_HANDLE> freelist;
			...
			vz::vector<int> free_bindless_res;
			vz::vector<int> free_bindless_sam;
