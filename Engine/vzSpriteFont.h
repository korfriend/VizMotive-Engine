...
namespace vz
{
	...
	public:
		...
		vz::Resource fontStyleResource;
		std::string fontStyleName;

		SpriteFont() = default;
		SpriteFont(const std::string& value, const vz::font::Params& params = vz::font::Params(), const std::string& fontStyleName = "") :params(params), fontStyleName(fontStyleName)
		{
			SetText(value);
			if (!fontStyleName.empty())
			{
				fontStyleResource = vz::resourcemanager::Load(fontStyleName);
			}
		}
