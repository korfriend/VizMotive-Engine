...
namespace vz
{
	...
	// This can be used as a Engine application
	...
	protected:
		...
		vz::eventhandler::Handle swapChainVsyncChangeEvent;
		...
		vz::graphics::Texture rendertarget;
	...
	public:
		...
		vz::Canvas canvas;
		vz::platform::window_type window;
		...
		void ActivatePath(RenderPath* component, float fadeSeconds = 0, vz::Color fadeColor = vz::Color(0, 0, 0, 255));
		...
		virtual void Compose(vz::graphics::CommandList cmd);

		// You need to call this before calling Run() or Initialize() if you want to render
		void SetWindow(vz::platform::window_type);
