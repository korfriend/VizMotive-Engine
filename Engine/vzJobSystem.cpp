...
namespace vz::jobsystem
{
	...
	inline void work(uint32_t startingQueue)
	{
		...
		for (uint32_t i = 0; i < internal_state.numThreads; ++i)
		{
			...
			while (job_queue.pop_front(job))
			{
				...
				{
					thread_local static vz::vector<uint8_t> shared_allocation_data;
	...
	void Initialize(uint32_t maxThreadCount)
	{
		...
		for (uint32_t threadID = 0; threadID < internal_state.numThreads; ++threadID)
		{
			...
			// Name the thread:
			std::wstring wthreadname = L"vz::jobsystem_" + std::to_wstring(threadID);
...
#define handle_error_en(en, msg) \
			...
			// Name the thread
			std::string thread_name = "vz::job::" + std::to_string(threadID);
...
#undef handle_error_en
		...
		vz::backlog::post("vz::jobsystem Initialized with [" + std::to_string(internal_state.numCores) + " cores] [" + std::to_string(internal_state.numThreads) + " threads] (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
