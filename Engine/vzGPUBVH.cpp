...
namespace vz
{
	...
	void GPUBVH::Update(const vz::scene::Scene& scene)
	{
	...
	void GPUBVH::Build(const Scene& scene, CommandList cmd) const
	{
		...
		auto range = vz::profiler::BeginRangeGPU("BVH Rebuild", cmd);
		...
		vz::gpusortlib::Sort(primitiveCount, primitiveMortonBuffer, primitiveCounterBuffer, 0, primitiveIDBuffer, cmd);
		device->EventEnd(cmd);
		...
		vz::profiler::EndRange(range); // BVH rebuild
	...
	namespace GPUBVH_Internal
	{
		void LoadShaders()
		{
			vz::renderer::LoadShader(ShaderStage::CS, computeShaders[CSTYPE_BVH_PRIMITIVES], "bvh_primitivesCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, computeShaders[CSTYPE_BVH_HIERARCHY], "bvh_hierarchyCS.cso");
			vz::renderer::LoadShader(ShaderStage::CS, computeShaders[CSTYPE_BVH_PROPAGATEAABB], "bvh_propagateaabbCS.cso");
		}
	}
	...
	void GPUBVH::Initialize()
	{
		...
		static vz::eventhandler::Handle handle = vz::eventhandler::Subscribe(vz::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { GPUBVH_Internal::LoadShaders(); });
		GPUBVH_Internal::LoadShaders();

		vz::backlog::post("vz::GPUBVH Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
	}
}
