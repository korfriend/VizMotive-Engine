...
int main(int, char**)
{
...

	{
		...
		slicer_curved->SetCurvedPlaneHeight(5.f);
		{
			...
			vzm::VzActorStaticMesh* actor_cslicer_helper = vzm::NewActorStaticMesh("actor: geometry helper for curved slicer", 
				geometry_cslicer_helper->GetVID(), material_curvedslicer0->GetVID());
		...
		axis_helper->EnablePickable(false);
		...
		using namespace vzm;
		{
			...
			ImGui::Begin("3D Viewer Multi-Test");
			{
				...
				if (renderer3D1->GetSharedRenderTarget(g_pd3dDevice, g_pd3dSrvDescHeap, 3, srt, &w, &h)) {
					...
					if (belongto == 1)
						view3D_gizmo.ApplyGizmo(camera1->GetVID(), pos, size, ImGui::GetWindowDrawList());
				}
			}
			ImGui::End();
